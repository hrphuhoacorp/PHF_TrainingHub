...
function cleanName(value) { return text(value).slice(0, MAX_NAME_CHARS); }

/* HOTFIX (2026-08-18) - "Thu nhap hien tai cua Nguyen Thien Truc la bao
 * nhieu?" tung KHONG tra loi duoc: get_employee_income/get_employee_competency_status
 * TRUOC DAY chi nhan employeeCode CHINH XAC, khong nhan ten - kien truc
 * tool-calling chi co 1 vong (xem ghi chu dau lib/ai-sandbox.js), nen model
 * KHONG THE goi search_employees de tim ma roi goi tiep income tool trong
 * cung 1 luot (khong biet ma truoc khi tim), dan den vong 2 (text-only,
 * khong con tool) co the "leak" 1 lan goi tool gia lap va bi loi chung
 * chung. Fix: cho phep goi 2 tool nay THANG bang ten (name), tu resolve ra
 * employeeCode qua orgDirectory.getEmployeeProfile() (lib/org-directory.js -
 * CUNG nguon/logic ambiguous:true+candidates da dung cho get_employee_manager/
 * get_direct_reports, KHONG tao logic resolve moi, KHONG mo permission moi -
 * day la thong tin co cau to chuc DUNG CHUNG, khong nhay cam). Neu ten khop
 * NHIEU nguoi -> tra ambiguous_name kem candidates, KHONG tu doan 1 nguoi,
 * KHONG fallback ve "chinh minh". Permission thu nhap/nang luc THAT (income_view/
 * view_people) van chay HOAN TOAN o getKnlEmployeeIncome/getKnlEmployeeCompetencyAssignment
 * NHU CU, sau khi co employeeCode - resolve ten CHI la buoc tim ma, khong
 * lien quan/khong bypass permission gate that. */
async function resolveEmployeeCodeByName(session, nameQuery) {
  const resolved = await orgDirectory.getEmployeeProfile(session, { name: nameQuery });
  if (resolved.ambiguous) {
    return {
      employeeCode: '', ambiguous: true,
      candidates: (resolved.candidates || []).map(p => ({
        employeeCode: p.employeeCode || '', employeeName: p.employeeName || '',
        title: p.title || '', department: p.department || '', branch: p.branch || ''
      }))
    };
  }
  if (resolved.found && resolved.person) return { employeeCode: text(resolved.person.employeeCode).toUpperCase(), ambiguous: false, candidates: null };
  return { employeeCode: '', ambiguous: false, candidates: null };
}

// resolveEmployeeCodeForAi: employeeCode CHINH XAC (uu tien) HOAC name (tra
// ve qua orgDirectory) HOAC mac dinh chinh nguoi hoi (selfCode, GIU NGUYEN
// hanh vi cu khi KHONG truyen ca employeeCode lan name). Tra ve
// {employeeCode, ambiguous, candidates, notFoundByName} de ham goi phan
// biet ro 3 truong hop: da resolve dung / can hoi lai (ambiguous) / khong
// tim thay theo ten (KHONG duoc am tham fallback ve self trong truong hop
// nay - nguoi dung hoi ve 1 nguoi CU THE, tra loi nham nguoi khac la sai).
async function resolveEmployeeCodeForAi(session, input, selfCodeFn) {
  const explicitCode = cleanEmployeeCode(input.employeeCode);
  if (explicitCode) return { employeeCode: explicitCode, ambiguous: false, candidates: null, notFoundByName: false };
  const nameQuery = cleanName(input.name || input.employeeName);
  if (nameQuery) {
    const byName = await resolveEmployeeCodeByName(session, nameQuery);
    if (byName.ambiguous) return { employeeCode: '', ambiguous: true, candidates: byName.candidates, notFoundByName: false };
    if (!byName.employeeCode) return { employeeCode: '', ambiguous: false, candidates: null, notFoundByName: true };
    return { employeeCode: byName.employeeCode, ambiguous: false, candidates: null, notFoundByName: false };
  }
  const selfCode = await selfCodeFn(session);
  return { employeeCode: selfCode, ambiguous: false, candidates: null, notFoundByName: false };
}
...
async function getEmployeeIncomeForAi(session, args) {
  ...
  const resolvedRef = await resolveEmployeeCodeForAi(session, input, selfEmployeeCodeFromOrg);
  if (resolvedRef.ambiguous) return { available: false, asOf, employeeCode: '', reason: 'ambiguous_name', candidates: resolvedRef.candidates };
  if (resolvedRef.notFoundByName) return { available: false, asOf, employeeCode: '', reason: 'employee_not_found' };
  let employeeCode = resolvedRef.employeeCode;
