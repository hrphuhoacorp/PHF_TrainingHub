...
async function saveKnlFrameworkAssignment(session,input={}){
  ...
  const {data:version,error:vError}=await db.from('knl_framework_versions').select('id,framework_id,status').eq('id',versionId).maybeSingle();throwDb(vError);if(!version)fail('Version KNL không tồn tại.',404,'KNL_ASSIGNMENT_VERSION_NOT_FOUND');
  ...
  const row={assignment_key:assignmentKey,version_id:versionId,target_type:resolved.targetType,target_ref:resolved.targetRef,employee_code:resolved.employeeCode,position_ref:resolved.positionRef,organization_snapshot:resolved.snapshot,is_primary:input.isPrimary===true,status:input.status==='inactive'?'inactive':'active',valid_from:text(input.validFrom)||new Date().toISOString().slice(0,10),valid_to:text(input.validTo)||null,reason,updated_by:a.id,updated_by_name:a.name,updated_at:new Date().toISOString()};
