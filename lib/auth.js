...
async function ensureLearnerEmployeeLink(account){
  if (!account || account.role !== 'learner' || String(account.employeeId || '').trim()) return account;
  const employeeId = await resolveUniqueEmployeeIdByPhone(account.phone);
  if (!employeeId) return account;
...
async function updateAccountByAdmin(accountId, input){
  ...
  if (supabase) {
    ...
    const labels = ['Email này đã được dùng cho tài khoản khác.','Số điện thoại này đã được liên kết với tài khoản khác.','Mã nhân viên này đã được dùng cho tài khoản khác.'];
    for (let i=0;i<checks.length;i++) {
      if (checks[i].error) throw checks[i].error;
      if (checks[i].data && checks[i].data.length) {
        const error = new Error(labels[i]);
        error.statusCode = 409;
        error.code = i===0 ? 'EMAIL_DUPLICATE' : (i===1 ? 'PHONE_DUPLICATE' : 'EMPLOYEE_CODE_DUPLICATE');
        throw error;
      }
    }
  } else {
...
async function createAccountByAdmin(input){
  ...
  if (!phone || phone.replace(/\D/g,'').length < 9 || phone.replace(/\D/g,'').length > 11) {
  ...
  const duplicatePhone = existing.find(a => cleanPhone(a.phone) && cleanPhone(a.phone) === phone);
  if (duplicatePhone) {
  ...
  const account = normalizeAccount({
    ...
    trainingAudience:String(data.trainingAudience || ''),
    defaultProgram:String(data.defaultProgram || ''),
    mustChangePassword:true,
    tempPassword:temporaryPassword,
    metadata:{passwordVersion:1, createdByAdmin:true}
  });
