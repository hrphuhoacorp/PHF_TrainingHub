...
function builtIns(){
  ...
  const allowTest = String(process.env.PHF_ENABLE_TEST_ACCOUNTS || '').toLowerCase() === 'true';

  if (production && !allowTest) {
    const email = cleanEmail(process.env.PHF_BOOTSTRAP_ADMIN_EMAIL);
    const password = String(process.env.PHF_BOOTSTRAP_ADMIN_PASSWORD || '');
    if (!email || !password) return [];
    return [{
      id:'acct-bootstrap-admin',
      name:String(process.env.PHF_BOOTSTRAP_ADMIN_NAME || 'PHF Admin'),
      email,
      phone:cleanPhone(process.env.PHF_BOOTSTRAP_ADMIN_PHONE),
      role:'admin',
      status:'active',
      password,
      position:'Quản trị đào tạo',
      branch:'',
      department:'HCNS',
...
function ensureStore(){
  ...
  if(!fs.existsSync(ACCOUNTS_FILE)){
    ...
    const production = String(process.env.NODE_ENV || '').toLowerCase() === 'production';
    if (production && accounts.length === 0) {
      throw new Error(
        'Thiếu tài khoản Admin khởi tạo. Hãy cấu hình PHF_BOOTSTRAP_ADMIN_EMAIL và PHF_BOOTSTRAP_ADMIN_PASSWORD.'
      );
    }
