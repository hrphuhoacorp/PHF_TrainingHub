...
async function createEmployeeProfileForAccount(account){
  ...
  const row = {
    ...
    program_id: String(account.defaultProgram || '').trim() && String(account.defaultProgram) !== 'none'
      ? String(account.defaultProgram).trim()
      : 'new_sales',
...
async function listAccountsForAdmin(){
  if (supabase) {
    ...
    for (const account of accounts) {
      try { repaired.push(await ensureLearnerEmployeeLink(account)); }
      catch (linkError) {
...
async function updateAccountByAdmin(accountId, input){
  ...
  const resolvedEmployeeId = isSystemAccount ? '' : await resolveEmployeeIdForAccount({
  ...
  }, {createIfMissing:true, rejectInvalidDirect:true});
...
async function createAccountByAdmin(input){
  ...
  const account = normalizeAccount({
    ...
    defaultProgram:isSystemAccount ? 'none' : String(data.defaultProgram || ''),
