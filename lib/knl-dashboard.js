...
/*
 * Salary-grade distribution uses the exact-period assignment snapshot. It never
 * joins current ladder/grade masters, never reads a grade from another period,
 * and receives only people already accepted by incomeScopeAllows().
 */
function buildCompensationGradeMatrix(period, incomePeople, compensationRows) {
  ...
  (compensationRows || []).forEach(row => {
    const code = employeeCodeOf(row);
    if (!code || text(row.payroll_period) !== text(period) || exactAssignments.has(code)) return;
    exactAssignments.set(code, row);
  });
  ...
  uniquePeople(incomePeople).forEach(person => {
    ...
    const assignment = exactAssignments.get(code) || null;
...
/*
 * Batch 1A.2 — "kỳ hoàn chỉnh" (period completeness), DETERMINISTIC, không
 * threshold/tỷ lệ tự suy. Schema knl_employee_compensation_assignments không
 * có cờ publish/official (đã trace ở 1A.1), và resolver "current compensation"
 * hiện hữu (getKnlEmployeeIncome/getKnlEmployeeNextCompensationGrade ở
 * lib/knl-foundation.js — latest-row-wins, carry-forward) trả lời câu hỏi
 * KHÁC ("thu nhập hiện tại của 1 người là gì") chứ không phải "kỳ này đã đủ
 * dữ liệu để làm kỳ chuẩn chưa" — và Dashboard đã CHỐT ở Gate 2 là exact-period,
 * không carry-forward (xem comment periodsPresent/buildCompensationGradeMatrix
 * phía trên), nên không thể tái dùng resolver đó cho việc này.
 *
 ...
 * coveredCount = |expectedCodes ∩ actual codes có row đúng payroll_period|;
 * giao tập rõ ràng để 1 row "lạ" ngoài expected population (case-variant/đã
 * rời scope) không thể làm kỳ trông "đủ hơn" thực tế.
 */
function computePeriodCoverage(period, compensationRows, expectedCodes) {
  ...
  (compensationRows || []).forEach(row => {
    if (text(row.payroll_period) !== text(period)) return;
    const code = employeeCodeOf(row);
    if (expectedCodes.has(code)) actualCodes.add(code);
  });
...
async function getKnlDashboardOverview(session, filters = {}) {
  ...
  // Batch 1A.2: default (không truyền filters.period) KHÔNG được là "kỳ lớn
  // nhất có record" — phải là kỳ gần nhất KHÔNG future và coverageStatus
  // === 'complete' (xem computePeriodCoverage ở trên, deterministic, không %).
  // Nếu user CHỦ ĐỘNG chọn 1 kỳ cụ thể (kể cả kỳ partial/future/hoàn toàn
  // chưa có dữ liệu), request đó luôn được tôn trọng nguyên văn — giữ đúng
  // hành vi hiện hữu "cho xem nếu flow cho phép", chỉ default tự động mới cần
  // né kỳ chưa đủ. Nếu KHÔNG có kỳ nào complete, fallback kỹ thuật an toàn =
  // kỳ gần nhất có data (để còn xem được) nhưng KHÔNG gọi là complete —
  // currentPeriodState phản ánh đúng coverageStatus thật của nó.
  // previousPeriod chỉ được gán khi cả kỳ hiện tại lẫn kỳ trước đó đều
  // complete; nếu không, previousPeriod=null khiến mọi so sánh (deltaOf) tự
  // động trả null thay vì % gây hiểu nhầm — không cần sửa từng nơi tiêu thụ
  // deltaOf. expectedCodes = incomeByEmployeeCodes (population thật của
  // compensation KPI, KHÔNG lọc theo UI filter phòng ban/chi nhánh/chức
  // danh/bậc — completeness là thuộc tính của toàn incomeScope).
  const periodsPresent = [...new Set(compensationRows.map(r => text(r.payroll_period)))].sort().reverse();
  const periodCoverage = new Map(periodsPresent.map(p => [p, computePeriodCoverage(p, compensationRows, incomeByEmployeeCodes)]));
  const isPeriodComplete = p => { const s = periodCoverage.get(p); return Boolean(s && s.isComplete); };
  const requestedPeriod = text(filters.period);
  const currentPeriod = requestedPeriod
    || periodsPresent.find(isPeriodComplete)
    || periodsPresent[0] || null;
  const currentIdx = currentPeriod ? periodsPresent.indexOf(currentPeriod) : -1;
  const currentPeriodState = currentPeriod
    ? (periodCoverage.get(currentPeriod) || computePeriodCoverage(currentPeriod, compensationRows, incomeByEmployeeCodes))
    : null;
  const previousPeriod = (currentPeriodState && currentPeriodState.isComplete && currentIdx >= 0)
    ? (periodsPresent.slice(currentIdx + 1).find(isPeriodComplete) || null)
    : null;
...
module.exports = { getKnlDashboardOverview, computePeriodCoverage };
