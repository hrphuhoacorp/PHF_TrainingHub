...
async function deleteMonthlyFormException(session,input={}){
 ...
 if(reason.length<10)fail('Lý do xóa cần tối thiểu 10 ký tự.',409,'CHECKLIST_RECOVERY_REASON_REQUIRED');
 if(idempotencyKey.length<8||idempotencyKey.length>120)fail('Mã chống gửi lặp không hợp lệ.',400,'CHECKLIST_RECOVERY_IDEMPOTENCY_REQUIRED');
 if(!expectedUpdatedAt)fail('Thiếu phiên bản phiếu cần xóa.',409,'CHECKLIST_RECOVERY_EXPECTED_UPDATED_AT_REQUIRED');
 if(input.confirmDelete!==true)fail('Chưa xác nhận dữ liệu đánh giá của phiếu sẽ bị xóa.',409,'CHECKLIST_RECOVERY_DELETE_CONFIRM_REQUIRED');
