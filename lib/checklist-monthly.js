...
function normalizeMarketingKpiRows(rows){
 ...
 return rows.slice(0,100).map((row,index)=>{
  const source=row&&typeof row==='object'&&!Array.isArray(row)?row:{},code=t(source.code),name=t(source.name),target=t(source.target),weight=Number(source.weight);
  ...
  if(!target)fail('Tiêu chí '+code+' chưa có mục tiêu.',409,'CHECKLIST_MARKETING_KPI_TARGET_REQUIRED');
  if(!Number.isFinite(weight)||weight<0||weight>100)fail('Tỷ trọng '+code+' phải từ 0 đến 100.',409,'CHECKLIST_MARKETING_KPI_WEIGHT_INVALID');
...
async function reconcileMissingMonthlyReviewers(session,periodMonth){
 ...
 for(const form of got.data||[]){
  if(t(form.reviewer_id)||t(form.reviewer_code))continue;
  const current=byCode.get(t(form.employee_code).toUpperCase())||{},reviewerId=t(current.manager_id),reviewerCode=t(current.manager_code).toUpperCase(),reviewerName=t(current.manager_name);
  if(!reviewerId&&!reviewerCode){warnings.push({code:'MISSING_REVIEWER',employeeCode:t(form.employee_code).toUpperCase(),employeeName:t(form.employee_name),message:'Chưa có người thẩm định trong phân công hiệu lực của kỳ.'});continue;}
  ...
  const reason='Hệ thống bổ sung người thẩm định còn thiếu khi đồng bộ kỳ '+m+'.';
  ...
  if(rpc.error){warnings.push({code:'REVIEWER_RECONCILE_FAILED',employeeCode:t(form.employee_code).toUpperCase(),employeeName:t(form.employee_name),message:t(rpc.error.message)||'Không thể bổ sung người thẩm định.'});continue;}
  ...
  else warnings.push({code:t(result.code)||'REVIEWER_RECONCILE_BLOCKED',employeeCode:t(form.employee_code).toUpperCase(),employeeName:t(form.employee_name),message:t(result.message)||'Không thể bổ sung người thẩm định.'});
 }
