...
async function myMonthlyForm(session,input={}){if(!db)fail('Supabase chưa được cấu hình.',503,'SUPABASE_NOT_CONFIGURED');const a=actor(session);if(!a.employeeCode&&!a.employeeId)fail('Tài khoản chưa liên kết mã nhân viên.',403,'CHECKLIST_MONTHLY_IDENTITY_REQUIRED');let q=db.from('checklist_monthly_forms').select('*').in('status',['waiting_self','waiting_review','reviewed','locked']).order('period_month',{ascending:false}).limit(12);q=a.employeeCode?q.eq('employee_code',a.employeeCode):q.eq('employee_id',a.employeeId);if(input.month)q=q.eq('period_month',month(input.month));const found=await q;if(found.error)throw found.error;const forms=found.data||[];if(!forms.length)return {form:null,period:null};const ids=[...new Set(forms.map(x=>x.period_id))],pq=await db.from('checklist_monthly_periods').select('*').in('id',ids);if(pq.error)throw pq.error;const periods=pq.data||[],form=forms.find(f=>{const p=periods.find(x=>x.id===f.period_id);return f.pilot_opened_at||p&&['open','locked'].includes(p.status);});const refreshed=form?await refreshUnlockedChecklistScore(form):null,histories=refreshed?await formHistories([refreshed.id]):new Map();return {form:refreshed?withScoreSummary({...refreshed,history:histories.get(refreshed.id)||[]}):null,period:form?periods.find(x=>x.id===form.period_id)||null:null};}
...
async function myMonthlyReviews(session,input={}){
 if(!db)fail('Supabase chưa được cấu hình.',503,'SUPABASE_NOT_CONFIGURED');const a=actor(session);if(!a.employeeCode&&!a.employeeId)fail('Tài khoản chưa liên kết mã nhân viên.',403,'CHECKLIST_MONTHLY_IDENTITY_REQUIRED');
 const access=await getChecklistMonthlyReviewAccess(session);if(session?.role!=='admin'&&!access.canReview)return {forms:[]};
 const allowedIds=new Set((access.people||[]).map(x=>t(x.employeeId)).filter(Boolean)),allowedCodes=new Set((access.people||[]).map(x=>t(x.employeeCode).toUpperCase()).filter(Boolean));
 let assigned=db.from('checklist_monthly_forms').select('*').in('status',['draft','waiting_self','waiting_review','reviewed','locked']).order('period_month',{ascending:false}).order('self_submitted_at',{ascending:true}).limit(200),filters=[];
 if(a.employeeCode)filters.push('reviewer_code.eq.'+a.employeeCode);if(a.employeeId)filters.push('reviewer_id.eq.'+a.employeeId);assigned=filters.length?assigned.or(filters.join(',')):assigned.eq('id','00000000-0000-0000-0000-000000000000');if(input.month)assigned=assigned.eq('period_month',month(input.month));
 const aq=await assigned;if(aq.error)throw aq.error;
 const visible=(aq.data||[]).filter(form=>!(form.admin_exception_open&&form.status==='waiting_review')).filter(form=>session?.role==='admin'||allowedIds.has(t(form.employee_id))||allowedCodes.has(t(form.employee_code).toUpperCase()));
 const unique=new Map(visible.map(x=>[x.id,x])),uniqueForms=[...unique.values()],histories=await formHistories([...unique.keys()]);
 /* Chỉ trả phiếu được giao đúng người đăng nhập và còn nằm trong phạm vi
    thẩm định hiện hành. Phiếu lịch sử của cấp dưới do người khác thẩm định
    không còn bị trộn vào màn “Tôi cần thẩm định”. */
 const refreshedForms=await mapWithConcurrency(uniqueForms,6,refreshUnlockedChecklistScore);
 const forms=refreshedForms.map(form=>withScoreSummary({...form,history:histories.get(form.id)||[],view_only_current_manager:false}));
 return {forms};
}
async function saveMonthlyReview(session,input={}){
 ...
 if(!assigned&&!adminOverride)fail('Bạn không phải người được phân công thẩm định phiếu này.',403,'CHECKLIST_MONTHLY_REVIEW_FORBIDDEN');
 if(assigned&&session?.role!=='admin'){const access=await getChecklistMonthlyReviewAccess(session),allowed=(access.people||[]).some(person=>(t(form.employee_id)&&t(person.employeeId)===t(form.employee_id))||(t(form.employee_code)&&t(person.employeeCode).toUpperCase()===t(form.employee_code).toUpperCase()));if(!access.canReview||!allowed)fail('Quyền thẩm định của bạn đã thay đổi hoặc nhân viên không còn thuộc phạm vi được cấp. Vui lòng liên hệ Admin để phân công lại phiếu.',403,'CHECKLIST_MONTHLY_REVIEW_SCOPE_REVOKED');}
