...
async function myMonthlyForm(session,input={}){
 ...
 return {form:refreshed?withScoreSummary({...refreshed,history:histories.get(refreshed.id)||[],self_edit_window:selfWindow}):null,period:periodData};
}
...
async function getChecklistAssessmentProfile(session,input={}){
 ...
 /* Batch 1: chỉ xem chính mình. targetEmployeeCode/targetEmployeeId từ client
    (nếu có) bị bỏ qua hoàn toàn — không đọc input.targetEmployeeCode ở đâu
    trong hàm này. */
 ...
 const target={employeeId:a.employeeId,employeeCode:a.employeeCode,employeeName:'',department:'',title:'',branch:''};
 let standard={status:'error',data:null},currentScore={status:'none',data:null},resolvedForm=null;
 try{
  const resolved=await resolveAssessmentStandard(a.employeeCode,a.employeeId,selectedMonth);
 ...
 try{
  const h=await myAssessmentScoreHistory(a.employeeCode,a.employeeId,year);
