...
async function refreshUnlockedChecklistScore(form){
 ...
 if(isLive&&scoreChanged){
  ...
  const summary=scoreSummary(calculated),stamp=new Date().toISOString(),patch={checklist_score:breakdown.score,self_total_score:summary.selfTotalScore,review_total_score:summary.reviewTotalScore,score_formula_version:SCORE_FORMULA_VERSION,updated_at:stamp};
...
function scoreSummary(form){
 ...
 rows.forEach(r=>{const automatic=isAutomaticSource(r.source,r.name);selfActual[r.code]=automatic?numeric(form.checklist_score):numeric(self[r.code]?.value);reviewActual[r.code]=automatic?numeric(form.checklist_review_score==null?form.checklist_score:form.checklist_review_score):numeric(review[r.code]?.value);});
...
async function saveMyMonthly(session,input={}){
 ...
 const periodCheck=await db.from('checklist_monthly_periods').select('status').eq('id',form.period_id).maybeSingle();if(periodCheck.error)throw periodCheck.error;if(!form.pilot_opened_at&&!(periodCheck.data&&['open','locked'].includes(periodCheck.data.status)))fail('Phiếu chưa được mở cho tài khoản này.',403,'CHECKLIST_MONTHLY_NOT_OPEN');const breakdown=await checklistBreakdown(form.employee_code,form.period_month);form={...form,checklist_score:breakdown.score,checklist_breakdown:breakdown};const allRows=monthlyRows(form),automaticOver=allRows.find(r=>isAutomaticSource(r.source,r.name)&&breakdown.score>numeric(r.target));if(automaticOver)fail('Điểm tự động vượt Mục tiêu cấu hình tại '+automaticOver.code+'.',409,'CHECKLIST_MONTHLY_AUTOMATIC_OVER_TARGET');const rows=allRows.filter(r=>!isAutomaticSource(r.source,r.name)),allowed=new Set(rows.map(r=>r.code)),answers=input.answers&&typeof input.answers==='object'&&!Array.isArray(input.answers)?input.answers:{},clean={};
 ...
 const summary=scoreSummary({...form,self_answers:clean}),now=new Date().toISOString(),patch={self_answers:clean,self_note:t(input.note).slice(0,3000),self_total_score:summary.selfTotalScore,score_formula_version:SCORE_FORMULA_VERSION,self_saved_at:now,updated_at:now};if(submit){patch.status='waiting_review';patch.self_submitted_at=now;}
...
const MONTHLY_REVIEW_SUMMARY_FIELDS='id,period_id,period_month,employee_id,employee_code,employee_name,department,title,branch,reviewer_id,reviewer_code,reviewer_name,status,checklist_score,checklist_review_score,self_total_score,review_total_score,final_score,score_formula_version,self_submitted_at,review_submitted_at,reviewed_by,reviewed_by_code,reviewed_by_name,updated_at,admin_exception_open,pilot_opened_at';
