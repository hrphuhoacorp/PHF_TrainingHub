...
async function searchEmployees(session, args) {
  ...
  // title la field rieng (khac query tu do) de cau hoi kieu "Truong ca Phu
  // Loi la ai" match dung chuc danh that thay vi doan gan giong tu keyword.
  const filtered = people.filter(p => {
    ...
    if (title && !normalizeText(p.title).includes(normalizeText(title))) return false;
    if (query) {
