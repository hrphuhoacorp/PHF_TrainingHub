...
async function getChecklistScorePeriodReport(session,input={}){
  ...
  const [formsRead,violationsRead]=await Promise.all([
  ...
  if(formsRead.error)throw formsRead.error;if(violationsRead.error)throw violationsRead.error;
  ...
  const employees=scopedPeople.map(p=>{
    ...
    months.forEach(m=>{
      ...
      if(!form){periodsOut[m]={hasForm:false,formId:null,status:'',checklistScore:null,selfTotalScore:null,reviewTotalScore:null,finalScore:null,department:p.department,branch:p.branch,title:p.title,reviewerName:'',reviewSubmittedAt:'',templateId:'',templateVersion:''};return;}
      periodsOut[m]={hasForm:true,formId:form.id,status:t(form.status),...scoreFieldsOf(form,code+'|'+m),department:t(form.department),branch:t(form.branch),title:t(form.title),reviewerName:t(form.reviewer_name),reviewSubmittedAt:form.review_submitted_at||'',templateId:t(form.template_id),templateVersion:t(form.template_version)};
    });
...
async function getChecklistAnnualResultReport(session,input={}){
  ...
  const employees=filtered.map(p=>{
    ...
    months.forEach(m=>{
      ...
      const resultState=t(row.result_state),score=resultState==='SCORED'?round(row.score):null;
