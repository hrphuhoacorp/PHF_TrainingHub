...
const { getChecklistReportAccess, getChecklistMonthlyReviewAccess } = require('./checklist-permissions');
...
/**
 * "Tình trạng xử lý ghi nhận lỗi" trong menu Báo cáo. Đây là dashboard XỬ LÝ
 * (workflow), khác hẳn "Nhật ký lỗi" (view capability + viewScope) - dashboard
 * này chỉ dành cho người có quyền thẩm định (review_monthly capability),
 * dùng đúng reviewScope, không phải viewScope/reportScope. Tái sử dụng
 * nguyên getChecklistMonthlyReviewAccess (đã dùng cho luồng thẩm định phiếu
 * tháng) làm nguồn chuẩn duy nhất cho canReview + danh sách nhân sự trong
 * phạm vi - không có logic phân quyền riêng ở đây, không hard-code preset.
 *
 ...
 * - Đang giải trình: task.status IN (waiting_reviewer, waiting_admin) - nhân
 *   viên đã gửi giải trình, đang chờ người ghi lỗi/Admin xử lý.
 * - Quá hạn phản hồi: task.due_at < hiện tại và task.status chưa
 *   completed/cancelled (không loại trừ hai nhóm trên - đúng theo cách
 *   summary "overdue" hiện có trong lib/checklist-tasks.js tính, một task có
 *   thể vừa "chờ nhân viên" vừa "quá hạn").
 */
async function getChecklistViolationWorkflowSummary(session,input={}){
  if(!db)fail('Supabase chưa được cấu hình.',503,'SUPABASE_NOT_CONFIGURED');
  const access=await getChecklistMonthlyReviewAccess(session);
  const generatedAt=new Date().toISOString();
  if(!access.canReview)return {canReview:false,month:'',summary:null,scope:null,generatedAt};

  const allowedCodes=[...new Set((access.people||[]).map(row=>t(row.employeeCode).toUpperCase()).filter(Boolean))];
  const selectedMonth=input.month?month(input.month):new Date().toISOString().slice(0,7);
  const emptySummary={total:0,official:0,cancelled:0,waitingEmployee:0,inExplanation:0,overdue:0};
  if(!allowedCodes.length){
    return {canReview:true,month:selectedMonth,summary:emptySummary,scope:{role:access.role,presetCode:access.grant?.presetCode||'',count:0},generatedAt};
  }

  const bounds=periodBounds(selectedMonth);
  const {data:records,error:recordsError}=await db.from('checklist_violation_records')
    .select('id,record_status')
    .eq('is_test',false)
    .in('record_status',['official','cancelled'])
    .gte('occurred_date',bounds.start).lt('occurred_date',bounds.next)
    .in('employee_code',allowedCodes)
    .limit(5000);
  ...
  if(officialIds.length){
    const {data:taskRows,error:tasksError}=await db.from('checklist_violation_tasks')
      .select('violation_id,status,due_at')
      .in('violation_id',officialIds)
      .limit(5000);
  ...
  const now=Date.now();
  let waitingEmployee=0,inExplanation=0,overdue=0;
  for(const id of officialIds){
    const task=taskByViolation.get(id);
    if(!task)continue;
    if(task.status==='waiting_employee'||task.status==='waiting_employee_result')waitingEmployee++;
    else if(task.status==='waiting_reviewer'||task.status==='waiting_admin')inExplanation++;
    const due=Date.parse(task.due_at||'');
    if(!Number.isNaN(due)&&due<now&&task.status!=='completed'&&task.status!=='cancelled')overdue++;
  }
  ...
  return {
    canReview:true,
    month:selectedMonth,
    summary:{total,official,cancelled,waitingEmployee,inExplanation,overdue},
    scope:{role:access.role,presetCode:access.grant?.presetCode||'',count:allowedCodes.length},
    generatedAt
  };
}

module.exports={getChecklistMonthlyReport,reportScoreSummary,getChecklistViolationWorkflowSummary};
