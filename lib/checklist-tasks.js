...
async function getChecklistViolationDetail(session,input={}){
  ...
  const {data:violations,error}=await supabase.from('checklist_violation_records')
    .select('id,employee_id,employee_code,employee_name,department,branch,title,template_id,template_version,template_name,criterion_code,criterion_name,criterion_group,points,occurred_date,occurred_time,location,note,record_status,is_test,created_by,created_by_name,created_at,cancelled_by_name,cancelled_at,cancel_reason')
    .in('id',ids);
  ...
  const {data:taskRows,error:taskError}=await supabase.from(TABLE)
    .select('id,violation_id,status,current_assignee_id,current_assignee_code,current_assignee_type,current_assignee_name,due_at,created_by,created_by_name,created_at')
