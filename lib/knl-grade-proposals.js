...
// Bulk-đọc employee_code của người tạo (step_index 0, action 'propose' — luôn
// tồn tại đúng 1 dòng/proposal) + title của subject/creator từ Organization
// Master — THUẦN presentation cho "Danh sách" (mục 3 batch giám sát Admin),
// KHÔNG đổi permission/scope filter, KHÔNG đổi field nào publicProposal() đã
// có. created_by (cột proposal) là account id, không phải employee_code, nên
// phải lấy qua steps — không suy đoán/parse từ tên.
async function attachListPresentation(rows, orgByCode) {
  if (!rows.length) return [];
  const stepsResult = await db.from(STEPS_TABLE).select('proposal_id,actor_employee_code').eq('step_index', 0).eq('action', 'propose').in('proposal_id', rows.map(r => r.id));
  throwDb(stepsResult.error);
  const creatorCodeByProposalId = new Map((stepsResult.data || []).map(s => [s.proposal_id, code(s.actor_employee_code)]));
  return rows.map(row => {
    const p = publicProposal(row);
    const creatorCode = creatorCodeByProposalId.get(row.id) || '';
