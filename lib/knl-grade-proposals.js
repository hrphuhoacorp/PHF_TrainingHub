...
const { resolveCompetencyStandardForGradeCode } = require('./knl-competency');
...
async function getGradePromotionCriteriaStandard(session, input = {}) {
  ...
  const proposedGradeCode = code(input.proposedGradeCode || input.gradeCode);
  if (!proposedGradeCode) fail('Thiếu mã bậc đề xuất.', 400, 'KNL_PROPOSAL_GRADE_REQUIRED');
  ...
  const mapping = await resolveCompetencyStandardForGradeCode(subjectEmployeeCode, proposedGradeCode);
...
async function createGradePromotionProposal(session, input = {}) {
  ...
  // qua grade_code của compensation grade vừa chọn, BLOCK nếu không resolve
  // được mapping hoặc thiếu/sai đánh giá (không cho proposal rỗng lọt xuống DB).
  const criteriaMapping = requireCriteriaMapping(await resolveCompetencyStandardForGradeCode(subjectEmployeeCode, proposedGrade.grade_code));
