 ...
 *
 * Admin-only cho MỌI hàm. previewTransitionImport/confirmTransitionImport
 * (T08 Transition Import, 2026-08-19) là entrypoint route/UI ĐẦU TIÊN gọi
 * tới nhóm hàm này - xem server.js/api/data.js action previewChecklist
 * TransitionImport/confirmChecklistTransitionImport.
 */
...
// T08 Transition Import (2026-08-19) - batch này CHỈ phục vụ đúng kỳ T08/2026
// (2 chi nhánh Lái Thiêu/Ngô Quyền live, các đơn vị còn lại nhập tay). Không
// tin periodMonth từ file Admin upload - server luôn ép về đúng hằng số này
// cho MỌI dòng, bất kể file chứa gì (xem forcePeriodMonth()).
const T08_TRANSITION_PERIOD_MONTH = '2026-08';
...
async function loadActiveEmployeeIndex() {
  const { data, error } = await db.from('employee_profiles')
    .select('employee_code,full_name,employment_status,branch')
  ...
  (data || []).forEach(row => {
    ...
    if (!code) return;
    // branch chỉ được lib/checklist-monthly-results.js#buildPreviewRow() đọc khi
    // options.source==='TRANSITION_IMPORT' (guard SKIP_LT_NQ_LIVE) - vô hại với
    // BASELINE_IMPORT/MANUAL_IMPORT, không đổi hành vi các nguồn khác.
    index.set(code, { employeeCode: code, employeeName: t(row.full_name), employmentStatus: t(row.employment_status), branch: t(row.branch) });
  });
  return index;
}
...
async function previewMonthlyResultImport(session, input = {}) {
  ...
  const preview = buildPreviewBatch(rows, employeeIndex, existingIndex, { source });
...
async function confirmMonthlyResultImport(session, input = {}) {
  ...
  const revalidated = buildPreviewBatch(rows, employeeIndex, existingIndex, { source });
...
/*
 * forcePeriodMonth(rows, periodMonth) — T08 Transition Import (2026-08-19):
 * "periodMonth should be server/context controlled" - KHÔNG tin periodMonth
 * per-row từ file Admin upload. Mọi dòng bị ép về đúng T08_TRANSITION_
 * PERIOD_MONTH bất kể file gốc ghi gì ở cột đó (file Admin thậm chí không
 * cần có cột kỳ tháng).
 */
function forcePeriodMonth(rows, periodMonth) {
  return (Array.isArray(rows) ? rows : []).map(r => ({ ...(r && typeof r === 'object' ? r : {}), periodMonth }));
}

/*
 * previewTransitionImport(session, {rows}) / confirmTransitionImport(session,
 * {rows, batchId}) — entrypoint DUY NHẤT cho T08 Transition Import: source
 * LUÔN LÀ 'TRANSITION_IMPORT' (client không spoof được dù cố truyền source
 * khác, giống hệt nguyên tắc confirmBaselineImport()), periodMonth LUÔN bị
 * ép về T08_TRANSITION_PERIOD_MONTH (client không chọn tháng khác qua đường
 * này). Reuse NGUYÊN VẸN previewMonthlyResultImport/confirmMonthlyResultImport
 * - không có logic ghi/đọc DB thứ hai nào ở đây.
 */
async function previewTransitionImport(session, input = {}) {
  return previewMonthlyResultImport(session, { rows: forcePeriodMonth(input.rows, T08_TRANSITION_PERIOD_MONTH), source: 'TRANSITION_IMPORT' });
}
async function confirmTransitionImport(session, input = {}) {
  return confirmMonthlyResultImport(session, { rows: forcePeriodMonth(input.rows, T08_TRANSITION_PERIOD_MONTH), source: 'TRANSITION_IMPORT', batchId: input.batchId });
}
...
module.exports = {
  ...
  previewTransitionImport,
  confirmTransitionImport,
  loadActiveEmployeeIndex,
  loadExistingResultIndex,
  T08_TRANSITION_PERIOD_MONTH
};
