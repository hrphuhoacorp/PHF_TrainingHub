...
/* person: string (employeeCode, backward-compatible với call site/test cũ -
 * chỉ đủ để chấm type 'employees'/'all_company') HOẶC object đầy đủ
 * {employeeCode,department,branch,title} để chấm được cả department/branch/
 * title (2026-08-11 KNL Permission Clean). Không đọc field nào khác ngoài 4
 * field này - income scope hoàn toàn độc lập với people_scope/reservedEmployees. */
function incomeScopeAllows(resolved,person){if(resolved.source==='admin_recovery')return true;if(resolved.capabilities.income_view!==true)return false;const scope=resolved.row&&resolved.row.capabilities&&(resolved.row.capabilities.incomeScope||resolved.row.capabilities.income_scope);if(!scope||typeof scope!=='object')return false;if(scope.type==='all_company')return true;const p=typeof person==='string'?{employeeCode:person}:(person||{});const values=Array.isArray(scope.values)?scope.values.map(v=>text(v).toUpperCase()):[];if(!values.length)return false;if(scope.type==='employees')return values.includes(text(p.employeeCode).toUpperCase());if(scope.type==='department')return values.includes(text(p.department).toUpperCase());if(scope.type==='branch')return values.includes(text(p.branch).toUpperCase());if(scope.type==='title')return values.includes(text(p.title).toUpperCase());return false;}
...
async function getKnlEmployeeNextCompensationGrade(session,input={}){
  ...
  const {data:row,error}=await db.from('knl_employee_compensation_assignments').select('*').eq('employee_code',employeeCode).order('payroll_period',{ascending:false}).limit(1).maybeSingle();
  throwDb(error);
