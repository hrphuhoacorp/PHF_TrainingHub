...
async function createConversation(session, input = {}) {
  ...
  const rows = messages.map(m => ({ conversation_id: conv.id, role: m.role, content: m.content, result: m.result, actions: m.actions, created_at: nowIso }));
...
async function appendMessages(session, input = {}) {
  ...
  const nowIso = new Date().toISOString();
  const rows = messages.map(m => ({ conversation_id: id, role: m.role, content: m.content, result: m.result, actions: m.actions, created_at: nowIso }));
  const insertMsgs = await supabase.from(MESSAGES).insert(rows);
  if (insertMsgs.error) throw insertMsgs.error;
