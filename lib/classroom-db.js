...
async function listAttendance(session, sessionId) {
  ...
  return {classroomClass:cls,session:ses,attendance:rows,canManage:canManageAttendance(session,cls,sessionId)};
}
async function saveAttendance(session, payload) {
  assertStorage();
  const sessionId=text(payload?.sessionId), items=Array.isArray(payload?.attendance)?payload.attendance:[];
  const {cls}=await attendanceContext(session,sessionId);
