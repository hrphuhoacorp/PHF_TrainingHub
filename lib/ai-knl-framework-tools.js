...
const { resolveActorGrant } = require('./knl-permissions');
...
async function findFrameworkByCode(session, frameworkCode) {
  ...
  return (frameworks || []).find(f => norm(f.code) === code || norm(f.name) === code || norm(f.name).includes(code) || code.includes(norm(f.code))) || null;
}
...
async function findVersionIdByAssignment(session, { employeeCode, title, department }) {
  ...
  if (employeeCode) {
    ...
    const direct = active.filter(a => a.targetType === 'employee' && norm(a.employeeCode) === code);
    if (direct.length) {
...
async function getKnlFrameworkForAi(session, args) {
  ...
  if (!versionId) {
    return { available: false, asOf, reason: 'not_found', frameworkCode, employeeCode, title, department };
  }
...
async function getKnlGradeRequirementsForAi(session, args) {
  ...
  if (!versionId) return { available: false, asOf, reason: 'not_found', frameworkCode, employeeCode, title, department };

  const [bundle, matrix] = await Promise.all([loadVersionBundle(session, versionId), getKnlGradeMatrix(session, { versionId })]);
