...
function buildStructuredResult(toolName, result) {
  ...
  if (toolName === 'search_employees') {
    ...
    return {
      ...
      evidence: buildEvidence('VERIFIED', ['checklist_employee_assignments'], result.asOf || nowIso, SCOPE_NOTE_ORG_DIRECTORY, {
        isCompletePopulation: false,
        groundingReplacement: `Trong phạm vi dữ liệu hiện có, hệ thống tìm thấy ${total} người khớp điều kiện tìm kiếm. ${SCOPE_NOTE_ORG_DIRECTORY}`
      }),
      data: {
        columns: [
          { key: 'employeeName', label: 'Nhân viên', align: 'left' },
