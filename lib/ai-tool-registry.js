...
const AI_TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Lấy thu nhập THAM CHIẾU hiện tại (kỳ lương đang áp dụng gần nhất) của MỘT nhân viên (mặc định chính người đang hỏi nếu để trống cả employeeCode lẫn name), gồm Bậc lương (compensation grade - ĐỘC LẬP với Bậc năng lực Khung năng lực, xem get_employee_competency_status), lương cơ bản/HQCV, các khoản phụ cấp (chuyên môn/quản lý/ăn trưa/khác) và tổng thu nhập tham chiếu. Chấp nhận employeeCode CHÍNH XÁC nếu đã biết, HOẶC name (tên nhân viên tự do, ví dụ "Nguyễn Thiên Trúc") nếu CHƯA biết mã - gọi THẲNG bằng name, KHÔNG cần tra mã bằng tool khác trước. Nếu tên khớp nhiều người, kết quả trả về ambiguous:true kèm candidates - PHẢI hỏi lại người dùng để xác định đúng người, không tự chọn đại. Đây là DỮ LIỆU NHẠY CẢM - chỉ gọi tool này khi câu hỏi thực sự cần số liệu thu nhập/lương/phụ cấp của một người cụ thể, KHÔNG gọi cho câu hỏi kiến thức chung không liên quan thu nhập. Tài khoản không đủ quyền xem thu nhập người khác sẽ bị tool từ chối - đây là đúng thiết kế, không suy đoán số liệu thay thế.',
      parameters: {
        type: 'object',
        properties: {
          employeeCode: { type: 'string', description: 'Mã nhân viên, ví dụ PHF079. Để trống nếu chưa biết mã (dùng name) hoặc hỏi về chính người đang hỏi.' },
          name: { type: 'string', description: 'Tên nhân viên nếu chưa biết mã, ví dụ "Nguyễn Thiên Trúc" - hệ thống tự resolve ra mã, gọi thẳng bằng tên, không cần tool khác trước.' }
        },
        required: []
      }
    }
  },
  {
    ...
    function: {
      ...
      description: 'Lấy Bậc năng lực Khung năng lực (KNL) hiện tại của MỘT nhân viên (mặc định chính người đang hỏi nếu để trống cả employeeCode lẫn name) và trạng thái Tạm thời (PROVISIONAL - mới gán/mới đổi, chưa được xác nhận chính thức) hay Đã xác nhận (CONFIRMED). Chấp nhận employeeCode CHÍNH XÁC nếu đã biết, HOẶC name (tên tự do) nếu chưa biết mã - gọi THẲNG bằng name, không cần tra mã trước; tên khớp nhiều người trả về ambiguous:true kèm candidates, phải hỏi lại người dùng, không tự chọn đại. Đây là "Bậc KNL" - ĐỘC LẬP HOÀN TOÀN với "Bậc lương/compensation grade" lấy từ get_employee_income - hai con số này CÓ THỂ KHÁC NHAU, không được đồng nhất hay suy ra cái này từ cái kia. Khi câu hỏi chỉ nói chung chung "đang bậc mấy" mà không rõ là hỏi Bậc lương hay Bậc năng lực, hãy gọi CẢ HAI tool (get_employee_income và get_employee_competency_status, cùng employeeCode/name) và trình bày rõ ràng, tách biệt từng nguồn - không tự chọn 1 nguồn rồi trả lời như thể đó là câu trả lời duy nhất.',
      parameters: {
        type: 'object',
        properties: {
          employeeCode: { type: 'string', description: 'Mã nhân viên, ví dụ PHF079. Để trống nếu chưa biết mã (dùng name) hoặc hỏi về chính người đang hỏi.' },
          name: { type: 'string', description: 'Tên nhân viên nếu chưa biết mã - hệ thống tự resolve ra mã, gọi thẳng bằng tên, không cần tool khác trước.' }
        },
        required: []
      }
    }
...
function buildStructuredResult(toolName, result) {
  ...
  if (toolName === 'get_employee_income') {
    if (!result.available) {
      // HOTFIX (2026-08-18) - ten khop nhieu nguoi (goi tool bang name) ->
      // dung LAI DUNG buildAmbiguousCard() da co san (cung pattern
      // get_employee_manager/get_direct_reports), KHONG tu doan 1 nguoi.
      if (result.reason === 'ambiguous_name') return buildAmbiguousCard(result.candidates, 'Nhiều nhân sự trùng tên - cần xác định người cần hỏi');
      return null; // thieu input hoac khong tim thay nhan vien - de model tu dien giai tu raw JSON
    }
