...
function buildStructuredResult(toolName, result) {
  ...
  if (toolName === 'get_employee_income') {
    ...
    if (c.hasProfessionalAllowance) allowanceItems.push({ label: 'Phụ cấp chuyên môn', value: round2(c.professionalAllowance) });
    if (c.hasManagementAllowance) allowanceItems.push({ label: 'Phụ cấp quản lý', value: round2(c.managementAllowance) });
    if (c.hasMealAllowance) allowanceItems.push({ label: 'Phụ cấp ăn trưa', value: round2(c.mealAllowance) });
    if (c.probationAmount) allowanceItems.push({ label: 'Hỗ trợ thử việc', value: round2(c.probationAmount) });
    (c.extraAllowances || []).forEach(e => allowanceItems.push({ label: e.label || e.name || 'Khoản khác', value: round2(e.amount) }));
    return {
      type: 'summary',
      ...
      data: {
        metrics: [
          ...
          { label: 'Lương cơ bản', value: round2(c.baseSalary) },
          { label: 'HQCV', value: round2(c.hqcv) },
          { label: 'Tổng thu nhập tham chiếu', value: round2(c.totalReferenceIncome) }
        ],
