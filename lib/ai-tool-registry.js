...
const AI_TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Tìm nhân viên theo tên, mã, chức danh, phòng ban hoặc chi nhánh. Dùng khi được hỏi tìm một hoặc vài nhân viên theo điều kiện (kể cả câu hỏi dạng "X hiện có bao nhiêu người / là những ai", "Trưởng ca chi nhánh Y là ai" - dùng tham số title cho đúng chức danh, không đoán từ query chung chung), không biết trước mã chính xác. Kết quả chỉ trong phạm vi nhân sự đang được phân công theo dõi Checklist, KHÔNG phải toàn bộ nhân viên PHF - luôn đọc evidence.note trước khi trả lời.',
      parameters: {
        type: 'object',
        properties: {
...
function buildStructuredResult(toolName, result) {
  ...
  if (toolName === 'search_employees') {
    ...
    return {
      type: 'ranking',
      title: `Kết quả tìm nhân viên (${total})`,
      scope: '',
      count: total,
      asOf: result.asOf || nowIso,
  ...
  if (toolName === 'get_employee_profile' || toolName === 'get_knl_profile') {
    ...
    return {
      ...
      scope: result.employeeCode || '',
      asOf: result.asOf || nowIso,
      evidence: buildEvidence('VERIFIED', ['checklist_employee_assignments'], result.asOf || nowIso, note, { isCompletePopulation: false }),
      data: {
