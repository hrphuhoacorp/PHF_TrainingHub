...
async function resolveActorGrant(session) {
  ...
  if (a.role === 'admin') {
    const incomeView = await resolveAdminIncomeViewOverride(a.id);
    return {
      source: 'admin_recovery',
      presetCode: 'ADMIN_RECOVERY',
      capabilities: capabilities({ access_knl: true, view_people: true, propose: true, agree_proposal: true, approve: true, manage_framework: true, manage_permissions: true, income_view: incomeView }),
      peopleScope: { type: 'all_company', values: [], reservedEmployees: [] },
      row: null,
      identity: a
    };
