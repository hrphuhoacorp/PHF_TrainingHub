...
/*
 * buildPreviewRow(rawRow, currentEmployeeIndex, existingResultIndex)
 ...
 *   READY | SKIP_NOT_CURRENT_EMPLOYEE | SKIP_INACTIVE | MISSING_CODE |
 *   INVALID_SCORE | NEED_REVIEW | DUPLICATE | CONFLICT
 */
...
function buildPreviewRow(rawRow, currentEmployeeIndex, existingResultIndex) {
  ...
  if (existing) {
    // DUPLICATE = đã có đúng batch/source này (double-run an toàn, không
    // ghi lại). CONFLICT = đã có authoritative result KHÁC nguồn/khác giá
    // trị - KHÔNG tự ghi đè, cần Admin quyết định tường minh.
    const sameSourceFamily = existing.source === 'BASELINE_IMPORT';
    return {
      ...base, status: sameSourceFamily ? 'DUPLICATE' : 'CONFLICT',
      reason: sameSourceFamily
        ? 'Đã có kết quả BASELINE_IMPORT cho nhân sự/tháng này - không ghi trùng.'
        : 'Đã có kết quả authoritative khác nguồn (' + existing.source + ') cho nhân sự/tháng này - không tự động ghi đè, cần Admin xác nhận riêng.',
...
const PREVIEW_STATUSES = ['READY', 'SKIP_NOT_CURRENT_EMPLOYEE', 'SKIP_INACTIVE', 'MISSING_CODE', 'INVALID_SCORE', 'NEED_REVIEW', 'DUPLICATE', 'CONFLICT'];

function buildPreviewBatch(rawRows, currentEmployeeIndex, existingResultIndex) {
  const rows = (Array.isArray(rawRows) ? rawRows : []).map(r => buildPreviewRow(r, currentEmployeeIndex, existingResultIndex));
