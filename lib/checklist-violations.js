...
/**
 * Phân loại "Tình trạng xử lý" của một bản ghi lỗi theo đúng bucket nghiệp vụ
 * đã chốt ở Dashboard "Tình trạng xử lý ghi nhận lỗi" (xem docblock
 * getChecklistViolationWorkflowSummary trong lib/checklist-reports.js). Dùng
 * chung cho cả summary KPI lẫn danh sách/badge Nhật ký lỗi - không định nghĩa
 * lại bucket ở nơi khác.
 */
function classifyViolationWorkflowStatus(recordStatus, task, now = Date.now()) {
  if (recordStatus === 'cancelled') {
    return { key: 'cancelled', waitingEmployee: false, inExplanation: false, overdue: false };
  }
  ...
  const overdue = !!task && !Number.isNaN(due) && due < now && task.status !== 'completed' && task.status !== 'cancelled';
  const key = overdue ? 'overdue' : waitingEmployee ? 'waiting_employee' : inExplanation ? 'in_explanation' : 'official';
  return { key, waitingEmployee, inExplanation, overdue };
}
...
async function listChecklistViolations(session, input = {}) {
   ...
   * cùng nguồn với getChecklistViolationWorkflowSummary). waiting_employee/
   * in_explanation/overdue chỉ thuộc bản ghi official, is_test=false nên cần
   * một lượt lấy id khớp task trước khi lọc trang chính - đúng mẫu truy vấn
   * 2 bước đã dùng ở getChecklistViolationWorkflowSummary (giới hạn 5000).
   */
  const workflowStatus = ['waiting_employee', 'in_explanation', 'overdue', 'official', 'cancelled'].includes(t(input.workflowStatus))
    ? t(input.workflowStatus)
    : '';
  let workflowMatchedIds = null;
  if (workflowStatus === 'waiting_employee' || workflowStatus === 'in_explanation' || workflowStatus === 'overdue') {
    ...
    if (candidateIds.length) {
      ...
      matchedIds = candidateIds.filter(id => {
        const classification = classifyViolationWorkflowStatus('official', taskByViolation.get(id) || null);
