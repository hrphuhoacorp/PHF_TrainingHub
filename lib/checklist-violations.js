...
async function resolveViolationPermission(session) {
  const currentActor = actor(session);
  ...
  /* Fail-closed: chỉ capability record_violation=true mới tạo quyền ghi lỗi.
     Không fallback sang quyền xem, preset, route hay bảng quyền cũ. */
  const grant = (rows || []).find(row => row.capabilities && row.capabilities.record_violation === true) || null;
  if (!grant) {
    return {
      allowed: false,
...
function assertScope(permission, row) {
  if (permission.scopeType === 'all') return;
  const values = (permission.scopeValues || []).map(value => normalizeText(value));
  const assignmentBranch = row._scope_branch || row.assignment_branch || row.branch || row.location;

  if (
    permission.scopeType === 'employee' &&
    values.includes(normalizeText(row.employee_code))
  ) return;

  if (permission.scopeType === 'direct_reports') {
    const managerId = normalizeText(row._scope_manager_id || row.assignment_manager_id || row.manager_id);
    const managerCode = t(row._scope_manager_code || row.assignment_manager_code || row.manager_code).toUpperCase();
    const actorId = normalizeText(permission.actorId);
    const actorCode = t(permission.actorEmployeeCode).toUpperCase();
    if ((actorId && managerId === actorId) || (actorCode && managerCode === actorCode)) return;
  }

  if (
    permission.scopeType === 'branch' &&
    values.includes(normalizeText(assignmentBranch))
  ) return;

  if (permission.scopeType === 'department') {
    const department = normalizeText(row._scope_department || row.assignment_department || row.department);
    if (values.includes(department)) return;
  }

  if (permission.scopeType === 'department_branch') {
    const department = normalizeText(row._scope_department || row.assignment_department || row.department);
    const branch = normalizeText(assignmentBranch);
    const departments = values.filter(value => value.includes('ban hang') || value.includes('phong') || value.includes('bo phan'));
    const branches = values.filter(value => !departments.includes(value));
    const departmentOk = departments.length ? departments.includes(department) : department === normalizeText('Bán hàng');
    const branchOk = branches.length ? branches.includes(branch) : false;
    if (departmentOk && branchOk) return;
  }

  fail(
    'Nhân sự hoặc chi nhánh nằm ngoài phạm vi được cấp quyền.',
    403,
...
async function requireViolationPermission(session, action, rows = []) {
  const permission = await resolveViolationPermission(session);
  ...
  if (!allowed) {
    fail(
      'Tài khoản chưa được cấp quyền phù hợp cho thao tác này.',
      403,
      'CHECKLIST_VIOLATION_PERMISSION_DENIED'
    );
  }
  if (permission.source !== 'admin' && permission.scopeType === 'none') {
    fail(
      'Quyền Ghi nhận lỗi chưa có phạm vi áp dụng.',
      403,
      'CHECKLIST_VIOLATION_SCOPE_REQUIRED'
    );
  }
  for (const row of rows) assertScope(permission, row);
  return permission;
}
...
async function normalizeCanonical(row, session, mode, index, resolver, options = {}) {
  ...
  const payload = {
    ...
    has_evidence: row.hasEvidence === true || row.has_evidence === true,
...
async function saveChecklistViolations(session, rows) {
  ...
  const fingerprints = [
    ...new Set(payload.map(row => row.duplicate_fingerprint).filter(Boolean))
  ];
  let duplicates = [];
  ...
  return {
    ...
    duplicateSkipped: payload.length - saved.length,
