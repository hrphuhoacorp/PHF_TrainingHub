...
// (xem lib/checklist-permissions.js PRESETS.TRO_LY_GD); các hàm scope trong
// file này (assignmentMatchesPermission, permissionEmployees,
// requireViolationPermission, listChecklistViolations) chỉ so khớp với 'all'.
// Chuẩn hóa một lần tại đây để toàn bộ downstream chỉ cần biết 'all'.
function normalizeViolationScopeType(type) {
  const value = t(type) || 'none';
  return value === 'all_company' ? 'all' : value;
}
...
async function resolveViolationPermission(session, action = 'view') {
  ...
  return {
    ...
    canEditTest: false,
    canCancel: false,
    scopeType: normalizeViolationScopeType(resolvedScope.type),
...
async function listChecklistViolations(session, input = {}) {
  ...
  if (['waiting_employee', 'in_explanation', 'waiting_admin', 'overdue', 'due_soon', 'open'].includes(workflowStatus)) {
    let idQuery = supabase
      ...
      .eq('record_status', 'official')
      .limit(5000);
    if (permission.scopeType !== 'all') idQuery = idQuery.in('employee_code', allowedCodes);
