...
async function listChecklistEvidence(session, payload) {
  ...
  const items = await Promise.all((data || []).map(async row => {
    const signed = await db().storage.from(BUCKET).createSignedUrl(row.storage_path, SIGNED_URL_TTL_SECONDS);
    return { ...mapEvidence(row), url: signed.error ? '' : signed.data.signedUrl };
  }));
  return { items };
}
