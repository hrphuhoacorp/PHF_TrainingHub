...
async function callDeepSeekWithTools(session, cleanedMessages) {
  ...
  // DEDUPE (Batch 2 bounded orchestration) - neu model lo goi lai CUNG 1
  // tool+args trong CUNG 1 vong (vd do tu suy nghi lai giua nhieu tool
  // call), dung lai ket qua da co thay vi thuc thi adapter lan 2 - van
  // phai tra loi DU tool_call_id (API DeepSeek doi hoi 1 tool message cho
  // moi tool_call_id da phat ra), chi khong lam viec 2 lan.
  const payloadByKey = new Map();
  for (const call of toolCalls) {
    const toolName = String(call?.function?.name || '');
    const dedupeKey = toolName + '|' + String(call?.function?.arguments || '');
    let toolPayload;
    if (payloadByKey.has(dedupeKey)) {
      toolPayload = payloadByKey.get(dedupeKey);
    } else {
      toolPayload = await executeToolCall(session, call);
      payloadByKey.set(dedupeKey, toolPayload);
    }
    if (!structuredResult) {
