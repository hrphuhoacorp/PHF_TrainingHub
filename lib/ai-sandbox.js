...
const { AI_TOOLS, ALLOWED_TOOL_NAMES, executeToolCall, buildStructuredResult, buildGradeComparisonResult, buildAction } = require('./ai-tool-registry');
...
const SYSTEM_PROMPT =
  ...
  'thích đây là hai hệ bằng chứng khác nhau. Nếu hỏi so sánh/tra cứu Bậc mà\n' +
  'KHÔNG nêu tên bộ KNL (vd "B3 khác B2 ở đâu?"), get_knl_framework/\n' +
  'get_knl_grade_requirements có thể không tự xác định được bộ KNL (tài\n' +
  'khoản đang hỏi có thể chưa có assignment để tự suy) - nếu kết quả trả về\n' +
  'kèm danh sách "các bộ KNL hiện có", HÃY liệt kê đúng danh sách đó và hỏi\n' +
  'lại người dùng muốn xem bộ nào, KHÔNG tự chọn đại một bộ và KHÔNG trả\n' +
  'lời rằng không hỗ trợ.\n\n' +
  ...
  'toán thu nhập từ nguồn khác. Nếu câu hỏi nêu TÊN nhân viên (chưa biết mã),\n' +
  'gọi THẲNG get_employee_income/get_employee_competency_status với tham số\n' +
  'name - hai tool này tự resolve ra mã, KHÔNG cần gọi search_employees/\n' +
  'get_employee_profile để tìm mã trước rồi mới hỏi lại; nếu tên khớp nhiều\n' +
  'người, kết quả trả về ambiguous kèm danh sách candidates - hỏi lại người\n' +
  'dùng xác định đúng người, không tự chọn đại. Khi câu hỏi rõ ràng cần số\n' +
  'liệu thu nhập của một người, đây là tool phải gọi trực tiếp - không chỉ\n' +
  'dừng ở việc tra thông tin cơ cấu tổ chức cơ bản (search_employees/\n' +
  'get_employee_profile) rồi coi như đã trả lời xong.\n\n' +
...
async function callDeepSeekWithTools(session, cleanedMessages) {
  ...
  // HOTFIX polish (2026-08-18) - gom rieng cac ket qua get_knl_grade_requirements
  // THAT (available:true, co grade cu the) de sau vong lap co the dung
  // buildGradeComparisonResult() render 1 the "so sanh" khi model goi tool
  // nay >=2 lan trong CUNG 1 luot (vd "B3 khac B2 o dau") - KHONG doi cach
  // model goi tool, KHONG them truy van nao, chi doc lai payload da co san.
  const gradeRequirementPayloads = [];
  // HOTFIX (2026-08-18) - "Thu nhap hien tai cua Nguyen Thien Truc..." tung
  // hien the org/co ban (search_employees/get_employee_profile...) thay vi
  // the thu nhap du get_employee_income cung duoc goi thanh cong trong CUNG
  // luot, vi vong lap CU chi giu the CUA LAN GOI DAU (if (!structuredResult)).
  // Gio VAN giu dung logic "lan dau thang" o duoi (khong doi hanh vi mac
  // dinh cho moi to hop tool khac), nhung gom THEM builtResults (MOI card
  // xay dung thanh cong, khong chi cai dau) de sau vong lap co the UU TIEN
  // dung card income/nang luc THAT SU tra loi cau hoi hon la card tra cuu
  // danh tinh - dua tren TEN TOOL (generic), khong hard-code theo nhan vien.
  const builtResults = [];

  for (const call of toolCalls) {
    const toolName = String(call?.function?.name || '');
    const toolPayload = payloadByKey.get(dedupeKeyOf(call));
    if (toolName === 'get_knl_grade_requirements') gradeRequirementPayloads.push(toolPayload);
    const built = buildStructuredResult(toolName, toolPayload);
    if (built) {
      builtResults.push({ toolName, built });
      if (!structuredResult) { structuredResult = built; structuredToolName = toolName; }
    }
  ...
  if (gradeRequirementPayloads.length >= 2) {
    const comparison = buildGradeComparisonResult(gradeRequirementPayloads);
    if (comparison) { structuredResult = comparison; structuredToolName = 'get_knl_grade_requirements'; }
  }

  // Uu tien trinh bay (2026-08-18): neu cau hoi co ca ket qua tra cuu DANH
  // TINH co ban (search_employees/get_employee_profile/get_knl_profile/
  // search_knl_people - KHONG nhay cam) LAN ket qua thu nhap/bac nang luc
  // THAT SU tra loi duoc (get_employee_income/get_employee_competency_status),
  // card thu nhap/nang luc PHAI la card hien thi chinh - tra cuu danh tinh
  // chi ho tro resolve, khong duoc "lan at" cau tra loi that su. Sinh dua
  // tren TEN TOOL (generic cho moi nhan vien), KHONG doi khi chi co 1 loai
  // ket qua (giu nguyen "lan dau thang" cho moi truong hop khac).
  const IDENTITY_LOOKUP_TOOLS = new Set(['search_employees', 'get_employee_profile', 'get_knl_profile', 'search_knl_people']);
  const PRIORITY_ANSWER_TOOLS = new Set(['get_employee_income', 'get_employee_competency_status']);
  if (IDENTITY_LOOKUP_TOOLS.has(structuredToolName)) {
    const priorityBuilt = builtResults.find(b => PRIORITY_ANSWER_TOOLS.has(b.toolName));
    if (priorityBuilt) { structuredResult = priorityBuilt.built; structuredToolName = priorityBuilt.toolName; }
  }
