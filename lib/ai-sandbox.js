...
const SYSTEM_PROMPT =
  ...
  'CƠ CẤU TỔ CHỨC PHF - THÔNG TIN DÙNG CHUNG - nhóm tool search_employees,\n' +
  ...
  '(gọi chung là "Cơ cấu tổ chức") trả về thông tin dùng chung cho TẤT CẢ\n' +
  'tài khoản đã đăng nhập, bất kể vai trò: họ tên, mã nhân viên, chức danh,\n' +
...
/* FAIL-CLOSED GUARD - DSML/tool-call protocol leak (P0 Production). Mot so
   phan hoi DeepSeek co the "lo" cu phap goi tool duoi dang TEXT trong
   ...
   noi bo) lot thang ra nguoi dung, nhu da xay ra tren Production. 2 lop:
   1) looksLikeLeakedToolProtocol() - nhan dien tin hieu cu phap noi bo
      trong bat ky doan text nao SAP hien cho nguoi dung.
   2) extractLeakedToolCall() - CO GANG parse ra 1 tool call hop le tu
      chinh doan text do (ke ca khi bi cat ngang, thieu the dong) de VAN
      THUC THI DUNG tool thay vi vut bo - dung yeu cau "parse/execute neu
      la valid tool call, hoac tra loi sach/natural fallback". KHONG BAO
      GIO hien nguyen van cho nguoi dung du parse thanh cong hay khong. */
const LEAK_SIGNATURE_RE = /<｜[^>]*(?:tool|invoke|dsml)[^>]*｜>|<\|[^>]*(?:tool_calls|function_call|invoke)[^>]*\|>|<invoke\s+name=|<\/invoke>|<parameter\s+name=|tool[▁_]calls[▁_](?:begin|end)|"tool_calls"\s*:\s*\[/i;
...
function extractLeakedToolCall(text) {
  ...
  const closedRe = /parameter\s+name=["'](\w+)["'][^>]*>([\s\S]*?)<\/parameter>/gi;
  let matched = false;
  let m;
  while ((m = closedRe.exec(raw))) { args[m[1]] = m[2].trim(); matched = true; }
  if (!matched) {
    // The bi cat ngang (khong co </parameter>) - lay phan sau ">" toi
    // cuoi dong/het chuoi, khong doi the dong day du.
...
// Chan cuoi cung truoc khi bat ky doan text nao duoc coi la "cau tra loi
// cuoi cung" (khong con co hoi parse/execute lai) - neu van con dau hieu
// leak, KHONG BAO GIO hien nguyen van, thay bang fallback sach + log de
// van hanh dieu tra.
function sanitizeFinalReply(text) {
  if (!looksLikeLeakedToolProtocol(text)) return text;
...
async function callDeepSeekWithTools(session, cleanedMessages) {
  ...
  if (!toolCalls.length) {
    ...
    if (looksLikeLeakedToolProtocol(rawContent)) {
      ...
      return { reply: rawContent, result: null, actions: null };
    }
  }
