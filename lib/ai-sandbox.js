...
const SYSTEM_PROMPT =
  ...
  'không được suy số Bậc từ số Mức hay ngược lại) - phần đánh giá/gap năng\n' +
  'lực chi tiết của Khung năng lực CHƯA có trong hệ thống, chỉ có hồ sơ tổ\n' +
  'chức cơ bản; Training Hub dùng để phát triển kiến thức/kỹ năng qua bài\n' +
  'học, KHÔNG phải điểm đánh giá. Điểm Checklist thấp KHÔNG đồng nghĩa với\n' +
  'năng lực kém - đây là hai loại bằng chứng khác nhau, không được đồng\n' +
  'nhất hay suy luận cái này từ cái kia. Với framework Bán hàng, các chức\n' +
  'danh trong tuyến quản lý bán hàng (Nhân viên bán hàng, Ca trưởng, Quản\n' +
  'lý chuỗi...) dùng CHUNG một framework career ladder Bán hàng - không tự\n' +
  'suy đoán có framework Khung năng lực riêng cho từng chức danh quản lý\n' +
  'này nếu tool không xác nhận.\n\n' +
...
async function callDeepSeekWithTools(session, cleanedMessages) {
  ...
  if (!toolCalls.length) {
    ...
    if (looksLikeLeakedToolProtocol(rawContent)) {
      ...
      return { reply: sanitizeFinalReply(rawContent), result: null, actions: null };
    }
  }
