 ...
 * (xem scripts/PHF_KNL_EMPLOYEE_COMPETENCY_ASSIGNMENT_1.52.0_DRAFT.sql —
...
function throwDb(error) {
  ...
  if (code === 'PGRST205' || code === 'PGRST202' || code === '42P01' || code === '42883' || /Could not find the table|Could not find the function|relation .* does not exist|function .* does not exist/i.test(message)) {
    fail('Schema KNL Employee Competency Assignment chưa được cài đặt. Hãy chạy scripts/PHF_KNL_EMPLOYEE_COMPETENCY_ASSIGNMENT_1.52.0_DRAFT.sql (sau khi Technical Lead duyệt).', 503, 'KNL_COMPETENCY_SCHEMA_MISSING');
  }
  ...
  if (/KNL_COMPETENCY_RETROACTIVE_REASON_REQUIRED/.test(message)) {
