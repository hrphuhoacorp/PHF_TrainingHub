...
/* resolveCompetencyStandardForCompensationGrade — KNL Grade Promotion
 * Proposal Phase 2 (2026-08-19, sửa sau DEV rehearsal thật trên PHF-HR-DEV
 * phát hiện lỗi thiết kế). Bridge 2 KHÔNG GIAN "bậc" ĐỘC LẬP: compensation
 * grade (public.knl_compensation_grades, dùng cho workflow đề xuất nâng bậc
 * ở lib/knl-grade-proposals.js) và competency grade (public.knl_grade_
 * definitions, dùng cho tiêu chí năng lực).
 *
 * KHÔNG so khớp grade_code (string) như bản trước — check constraint THẬT
 * trên 2 bảng loại trừ nhau tuyệt đối: knl_grade_definitions.grade_code chỉ
 * nhận '^B[1-9][0-9]*$' (vd "B1"), knl_compensation_grades.grade_code bắt
 * buộc có tiền tố + gạch ngang '^[A-Z0-9][A-Z0-9_-]{1,19}-B[1-9][0-9]*$' (vd
 * "NSGQ-B1") — 2 regex không có giao nhau, so chuỗi trực tiếp LUÔN thất bại
 * với dữ liệu thật (phát hiện qua DEV rehearsal, không phải giả định). KHÔNG
 * fallback grade_number, KHÔNG fallback string-parse kiểu "NSGQ-B1"->"B1" —
 * business decision đã chốt: 2 hệ ĐỘC LẬP, chỉ nối qua mapping TƯỜNG MINH.
 *
 * Nguồn mapping DUY NHẤT: public.knl_compensation_competency_grade_map
 * (framework_version_id, compensation_grade_id) -> competency_grade_id,
 * UNIQUE theo (framework_version_id, compensation_grade_id) — xem
 * scripts/PHF_KNL_COMPENSATION_COMPETENCY_GRADE_MAP_1.63.0.sql. Không có
 * mapping cho đúng framework_version_id đang active của subject -> BLOCK
 * (reason='grade_not_mapped'), KHÔNG suy đoán bằng bất kỳ cách nào khác.
 *
 * THUẦN READ, không có permission check riêng ở đây — caller
 * (createGradePromotionProposal) đã enforce requirePropose()/
 * creationAuthorized() trước khi gọi tới hàm này. */
async function resolveCompetencyStandardForCompensationGrade(employeeCode, compensationGradeId) {
  ensureDb();
  const code = text(employeeCode).toUpperCase();
  const gradeId = text(compensationGradeId);
  const { data: assignmentRow, error: aErr } = await db.from('knl_employee_competency_assignments')
    .select('framework_version_id').eq('employee_code', code).eq('is_active', true).maybeSingle();
  throwDb(aErr);
  if (!assignmentRow) return { ok: false, reason: 'no_framework_assignment' };

  const { data: fv, error: fvErr } = await db.from('knl_framework_versions').select('id,framework_id,version_number').eq('id', assignmentRow.framework_version_id).maybeSingle();
  throwDb(fvErr);
  if (!fv) return { ok: false, reason: 'no_framework_assignment' };

  const { data: mapRow, error: mErr } = await db.from('knl_compensation_competency_grade_map')
    .select('competency_grade_id').eq('framework_version_id', fv.id).eq('compensation_grade_id', gradeId).maybeSingle();
  throwDb(mErr);
  if (!mapRow) return { ok: false, reason: 'grade_not_mapped' };

  const { data: gradeRow, error: gErr } = await db.from('knl_grade_definitions').select('id,grade_code,grade_number,sort_order,label').eq('id', mapRow.competency_grade_id).eq('version_id', fv.id).maybeSingle();
  throwDb(gErr);
  if (!gradeRow) return { ok: false, reason: 'grade_not_mapped' };

  const standard = await buildCompetencyStandard(fv.id, gradeRow);
  if (!standard || !standard.groups.length) return { ok: false, reason: 'no_requirements' };

  const { data: fw, error: fwErr } = await db.from('knl_frameworks').select('code,name').eq('id', fv.framework_id).maybeSingle();
  throwDb(fwErr);

  return {
    ok: true,
    frameworkVersionId: fv.id, competencyGradeId: gradeRow.id, gradeCode: gradeRow.grade_code, gradeLabel: gradeRow.label,
    framework: { code: fw ? fw.code : '', name: fw ? fw.name : '', versionNumber: fv.version_number },
    standard
  };
}
...
module.exports = { getKnlEmployeeCompetencyAssignment, listKnlEmployeeCompetencyHistory, getKnlEmployeeCompetencyStandard, getKnlEmployeeCompetencyGradeStandard, setKnlEmployeeCompetencyAssignment, listKnlEmployeeCompetencyAssignmentsInScope, buildCompetencyStandard, resolveCompetencyStandardForCompensationGrade };
