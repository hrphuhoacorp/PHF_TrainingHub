...
async function getKnlEmployeeCompetencyStandard(session, input = {}) {
  ...
  /* Bậc bổ sung ngoài current/next — chỉ trả code/number/label (rẻ, đã có
   * sẵn sortedGrades), KHÔNG build full standard cho từng bậc ở đây để tránh
   * query thừa khi user không bấm "Xem thêm". Lấy standard đầy đủ qua
   * getKnlEmployeeCompetencyGradeStandard khi cần. */
  const furtherGrades = nextGradeRow
    ? sortedGrades.filter(g => Number(g.sort_order) > Number(nextGradeRow.sort_order)).map(g => ({ code: g.grade_code, number: g.grade_number, label: g.label }))
    : [];
...
module.exports = { getKnlEmployeeCompetencyAssignment, listKnlEmployeeCompetencyHistory, getKnlEmployeeCompetencyStandard, getKnlEmployeeCompetencyGradeStandard, setKnlEmployeeCompetencyAssignment };
