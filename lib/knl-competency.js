...
/* resolveCompetencyStandardForGradeCode — KNL Grade Promotion Proposal Phase
 * 2 (2026-08-19). Bridge 2 KHÔNG GIAN "bậc" tách biệt (audit Phase 1, mục 3):
 * compensation grade_code (public.knl_compensation_grades, dùng cho workflow
 * đề xuất nâng bậc ở lib/knl-grade-proposals.js) KHÔNG có FK trực tiếp tới
 * competency grade (public.knl_grade_definitions, dùng cho tiêu chí năng
 * lực) — 2 bảng chỉ liên kết GIÁN TIẾP qua grade_code text trùng nhau. Hàm
 * này là NƠI DUY NHẤT thực hiện việc bridge đó: lấy framework KNL ACTIVE của
 * subject (knl_employee_competency_assignments.is_active=true) rồi tìm đúng
 * knl_grade_definitions theo grade_code trong CHÍNH framework_version_id đó
 * — nếu subject không có framework active, hoặc grade_code không khớp bậc
 * nào trong framework đó, hoặc bậc đó chưa có knl_grade_requirements nào
 * (framework rỗng — nhiều framework hiện chưa có grade matrix thật), trả về
 * {ok:false, reason} để caller (lib/knl-grade-proposals.js) BLOCK tạo/gửi
 * proposal — KHÔNG cho phép proposal rỗng lọt xuống DB (business decision đã
 * chốt Phase 2). THUẦN READ, không có permission check riêng ở đây — caller
 * (createGradePromotionProposal) đã enforce requirePropose()/
 * creationAuthorized() trước khi gọi tới hàm này. */
async function resolveCompetencyStandardForGradeCode(employeeCode, proposedGradeCode) {
  ensureDb();
  const code = text(employeeCode).toUpperCase();
  const targetGradeCode = text(proposedGradeCode).toUpperCase();
  ...
  const { data: gradeRow, error: gErr } = await db.from('knl_grade_definitions').select('id,grade_code,grade_number,sort_order,label').eq('version_id', fv.id).eq('grade_code', targetGradeCode).maybeSingle();
  throwDb(gErr);
...
module.exports = { getKnlEmployeeCompetencyAssignment, listKnlEmployeeCompetencyHistory, getKnlEmployeeCompetencyStandard, getKnlEmployeeCompetencyGradeStandard, setKnlEmployeeCompetencyAssignment, listKnlEmployeeCompetencyAssignmentsInScope, buildCompetencyStandard, resolveCompetencyStandardForGradeCode };
