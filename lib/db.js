...
function mapCommitmentRecord(row) {
  ...
  return {
    ...
    confirmedName: row.confirmed_name || snapshot.signName || snapshot.fullName || '',
    confirmedPhone: row.confirmed_phone || snapshot.signPhone || snapshot.phone || '',
    confirmedAt: row.confirmed_at || snapshot.signedAt || snapshot.confirmDate || '',
...
async function saveDataToSupabase(payload) {
  ...
  if (payload.confidentialityCommitment) {
    const r = payload.confidentialityCommitment;
    const commitmentId = r.id || createRecordId('bmtt');
    const documentVersion = r.documentVersion || 'PHF-BMTT-2026-06-06';
    const snapshot = {
      id: commitmentId,
      documentCode: r.documentCode || 'PHF-BMTT',
      documentVersion,
      documentTitle: r.documentTitle || 'Cam kết bảo mật thông tin',
      fullName: r.fullName || '',
      birthday: r.birthday || '',
      cccd: r.cccd || '',
      cccdDate: r.cccdDate || '',
    ...
    const commitmentSave = await supabase.from('commitment_records').upsert({
      ...
      confirmed_name: snapshot.signName || snapshot.fullName,
      confirmed_phone: snapshot.signPhone || snapshot.phone,
      confirmed_at: snapshot.signedAt || now,
      status: r.status || 'active',
      created_at: r.createdAt || now,
      updated_at: now,
      metadata: { source: 'PHF Training Hub' }
    }, {
      onConflict: 'employee_id,commitment_type,document_version'
    });

    if (commitmentSave.error) {
      if (isMissingCommitmentTableError(commitmentSave.error)) {
        console.error('[PHF Supabase] Chưa có bảng commitment_records. Hãy chạy file SQL nâng cấp:', commitmentSave.error.message || commitmentSave.error);
        throw new Error('Chưa thể lưu cam kết BMTT vì cơ sở dữ liệu chưa được nâng cấp. Vui lòng chạy file SQL BMTT.');
      }
      console.error('[PHF Supabase] Không lưu được hồ sơ BMTT:', commitmentSave.error.message || commitmentSave.error);
...
async function deleteEmployeeFromSupabase(payload) {
  ...
  for (const [table, column, optionalDuringUpgrade] of steps) {
    ...
    if (result.error) {
      if (optionalDuringUpgrade && isMissingCommitmentTableError(result.error)) {
        console.warn('[PHF Supabase] Bỏ qua commitment_records vì bảng chưa được nâng cấp.');
        continue;
      }
