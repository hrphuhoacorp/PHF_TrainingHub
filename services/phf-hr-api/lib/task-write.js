...
// ---------------------------------------------------------------------------
// Gate 12 — Category CRUD. Dịch NGUYÊN VĂN từ createTaskCategory/
// renameTaskCategory/setTaskCategoryActive/reorderTaskCategory/
// deleteTaskCategory trong api/_lib/task-core.js + RPC
// task_delete_category_if_unused (scripts/PHF_TASK_CATEGORY_CREATE_
// FOUNDATION_1.70.0.sql dòng 321-349, verbatim: advisory lock trên
// hashtextextended('task-category-delete|'||code, 0), check task.tasks
// còn tham chiếu category_code hay không, DELETE nếu không dùng).
//
// KHÔNG kiểm tra authorization (Admin-only) ở đây — main app đã
// requireTaskAdmin() TRƯỚC khi gọi (đúng nguyên tắc đã CLOSED, giống mọi
// operation khác trong file này). KHÔNG duplicate validate format
// category_code/display_name đầy đủ (regex, độ dài) — main app đã làm, DB
// CHECK constraint (task_categories_code_ck/name_ck) là backstop cuối cùng
// nếu main app có bug; ở đây chỉ chặn thiếu tham số cơ bản (presence check),
// đúng mức defensive-boundary đã có tiền lệ ở setTaskPermissionAssignment.
// ---------------------------------------------------------------------------
async function createTaskCategory(config, params) {
  const { categoryCode, displayName, actorAccountId, actorEmployeeCode } = params;
  const normCode = categoryCode && String(categoryCode).trim() !== '' ? String(categoryCode).trim().toUpperCase() : null;
  if (normCode === null) throw taskWriteError('TASK_CATEGORY_CODE_INVALID');
  const normName = displayName && String(displayName).trim() !== '' ? String(displayName).trim() : null;
  if (normName === null) throw taskWriteError('TASK_CATEGORY_NAME_REQUIRED');
  const normActorAccountId = actorAccountId && String(actorAccountId).trim() !== '' ? String(actorAccountId).trim() : null;
  const normActorEmployeeCode = actorEmployeeCode && String(actorEmployeeCode).trim() !== '' ? String(actorEmployeeCode).trim().toUpperCase() : null;
    ...
    try {
      inserted = await client.query(
        `INSERT INTO task.categories (
           category_code, display_name, is_active,
           created_by_account_id, created_by_employee_code,
           updated_by_account_id, updated_by_employee_code, updated_at
         ) VALUES ($1, $2, true, $3, $4, $3, $4, now())
         RETURNING *`,
        [normCode, normName, normActorAccountId, normActorEmployeeCode]
      );
    } catch (err) {
      if (err && err.code === '23505') throw taskWriteError('TASK_CATEGORY_CODE_EXISTS');
      throw err;
    }
...
async function renameTaskCategory(config, params) {
  const { categoryCode, displayName, actorAccountId, actorEmployeeCode } = params;
  const normCode = categoryCode && String(categoryCode).trim() !== '' ? String(categoryCode).trim().toUpperCase() : null;
  if (normCode === null) throw taskWriteError('TASK_CATEGORY_CODE_INVALID');
  const normName = displayName && String(displayName).trim() !== '' ? String(displayName).trim() : null;
  ...
  return withTaskWriteTransaction(config, async (client) => {
    const updated = await client.query(
      `UPDATE task.categories
          SET display_name = $2, updated_by_account_id = $3, updated_by_employee_code = $4, updated_at = now()
        WHERE category_code = $1
        RETURNING *`,
      [normCode, normName, normActorAccountId, normActorEmployeeCode]
    );
    // TASK_CATEGORY_RESOURCE_NOT_FOUND, NOT TASK_CATEGORY_NOT_FOUND — deliberately
    // distinct from createDraftTask's category-lookup code (Batch 3, 400,
    // untouched). Here the category IS the resource being addressed by :code
    // (route layer), so "not found" is 404 semantics, not a bad-input-field
    // 400 — Technical Lead decision (Option C), see route-wiring report.
    if (updated.rowCount === 0) throw taskWriteError('TASK_CATEGORY_RESOURCE_NOT_FOUND');
    return updated.rows[0];
  });
}

async function setTaskCategoryActive(config, params) {
  const { categoryCode, isActive, actorAccountId, actorEmployeeCode } = params;
  const normCode = categoryCode && String(categoryCode).trim() !== '' ? String(categoryCode).trim().toUpperCase() : null;
  if (normCode === null) throw taskWriteError('TASK_CATEGORY_CODE_INVALID');
  if (typeof isActive !== 'boolean') throw taskWriteError('TASK_CATEGORY_ACTIVE_INVALID');
  const normActorAccountId = actorAccountId && String(actorAccountId).trim() !== '' ? String(actorAccountId).trim() : null;
  const normActorEmployeeCode = actorEmployeeCode && String(actorEmployeeCode).trim() !== '' ? String(actorEmployeeCode).trim().toUpperCase() : null;
  ...
  return withTaskWriteTransaction(config, async (client) => {
    const updated = await client.query(
      `UPDATE task.categories
          SET is_active = $2, updated_by_account_id = $3, updated_by_employee_code = $4, updated_at = now()
        WHERE category_code = $1
        RETURNING *`,
      [normCode, isActive, normActorAccountId, normActorEmployeeCode]
    );
    if (updated.rowCount === 0) throw taskWriteError('TASK_CATEGORY_RESOURCE_NOT_FOUND');
    return updated.rows[0];
  });
...
async function reorderTaskCategory(config, params) {
  const { categoryCode, sortOrder, actorAccountId, actorEmployeeCode } = params;
  const normCode = categoryCode && String(categoryCode).trim() !== '' ? String(categoryCode).trim().toUpperCase() : null;
  if (normCode === null) throw taskWriteError('TASK_CATEGORY_CODE_INVALID');
  const normSortOrder = normalizeInteger(sortOrder);
  if (normSortOrder === undefined || normSortOrder < 1) throw taskWriteError('TASK_CATEGORY_SORT_ORDER_INVALID');
  const normActorAccountId = actorAccountId && String(actorAccountId).trim() !== '' ? String(actorAccountId).trim() : null;
  const normActorEmployeeCode = actorEmployeeCode && String(actorEmployeeCode).trim() !== '' ? String(actorEmployeeCode).trim().toUpperCase() : null;
  ...
  return withTaskWriteTransaction(config, async (client) => {
    const updated = await client.query(
      `UPDATE task.categories
          SET sort_order = $2, updated_by_account_id = $3, updated_by_employee_code = $4, updated_at = now()
        WHERE category_code = $1
        RETURNING *`,
      [normCode, normSortOrder, normActorAccountId, normActorEmployeeCode]
    );
    if (updated.rowCount === 0) throw taskWriteError('TASK_CATEGORY_RESOURCE_NOT_FOUND');
    return updated.rows[0];
  });
...
// deleteTaskCategoryIfUnused — port verbatim của task_delete_category_if_unused:
// advisory lock (chặn race với 1 Task mới đang tạo đúng lúc xóa) -> check
// còn dùng không -> DELETE nếu không dùng -> NOT_FOUND nếu đã bị xóa/không
// tồn tại. KHÔNG tự ý "soft delete"/set is_active=false thay thế — đó là
// setTaskCategoryActive(), một hành động RIÊNG main app tự chọn gọi khi cần
// (đúng nguyên tắc "chưa từng dùng -> xóa vật lý; đã dùng -> chỉ Ngừng dùng"
// nhưng quyết định "Ngừng dùng" khi TASK_CATEGORY_IN_USE là của main app,
// KHÔNG tự động fallback ở đây).
async function deleteTaskCategoryIfUnused(config, params) {
  ...
  return withTaskWriteTransaction(config, async (client) => {
    await client.query('SELECT pg_advisory_xact_lock(hashtextextended($1, 0))', [
      `task-category-delete|${normCode}`,
    ]);

    const inUse = await client.query('SELECT 1 FROM task.tasks WHERE category_code = $1 LIMIT 1', [normCode]);
    if (inUse.rowCount > 0) throw taskWriteError('TASK_CATEGORY_IN_USE');

    const deleted = await client.query('DELETE FROM task.categories WHERE category_code = $1 RETURNING category_code', [normCode]);
    if (deleted.rowCount === 0) throw taskWriteError('TASK_CATEGORY_RESOURCE_NOT_FOUND');

    return { deleted: true, category_code: normCode };
  });
}

// ---------------------------------------------------------------------------
// Gate 12 — Exception-grant CRUD. Dịch NGUYÊN VĂN từ createTaskPermissionGrant/
// revokeTaskPermissionGrant (api/_lib/task-core.js dòng 439-528).
//
// KHÔNG re-implement People Master lookup/scope-type-supported-cho-role nào
// ở đây — main app (requireTaskPermissionAdmin + normalizeExtendPeopleScope)
// đã resolve granteeEmployeeCode/peopleScope thành giá trị CUỐI CÙNG hợp lệ
// TRƯỚC khi gọi, giống đúng pattern actorContext đã resolve gửi xuống cho
// mọi operation khác trong file này. Ở đây chỉ presence/shape-check tối
// thiểu cho tham số đã nhận, KHÔNG tự tra cứu People Master (không có
// quyền truy cập bảng đó từ phf_hr).
//
// ATOMIC IMPROVEMENT (Option B, cùng tiền lệ đã duyệt cho removeTaskRelated/
// addTaskComment/removeTaskLink): bản gốc Supabase insert grant + insert
// history bằng 2 lời gọi riêng, kèm "compensation" thủ công (update lại
// is_active nếu insert history thất bại) vì PostgREST không có transaction
// thật. Ở đây dùng withTaskWriteTransaction() thật — insert grant + insert
// history trong CÙNG transaction, lỗi ở bước nào cũng ROLLBACK sạch, KHÔNG
// cần compensation logic nữa. Đây là cải thiện có chủ đích đã có tiền lệ
// được duyệt (S3B mục 6.4), KHÔNG phải business rule mới — kết quả cuối
// (grant tồn tại + history ghi lại đúng 'created') giữ nguyên.
//
// ⚠️ BLOCKER ĐÃ XÁC NHẬN LIVE (không phải suy đoán): task.permission_grants.
// created_by_employee_code và task.permission_grant_history.
// changed_by_employee_code đều NOT NULL trên phf_hr (xác nhận qua
// information_schema.columns, live, Gate 12 audit). createTaskPermissionGrant/
// revokeTaskPermissionGrant CHỈ được gọi bởi Admin (requireTaskPermissionAdmin
// trong task-core.js), và actorAuditColumns() cho Admin LUÔN ghi
// employee_code=null (actorContext.employeeCode luôn rỗng cho Admin, theo
// đúng thiết kế actor identity đã CLOSED ở S3B mục 6.2). Nghĩa là MỌI lời
// gọi createTaskPermissionGrant sẽ FAIL ở NOT NULL constraint của
// task.permission_grants.created_by_employee_code, và MỌI lời gọi
// revokeTaskPermissionGrant sẽ FAIL ở NOT NULL constraint của
// task.permission_grant_history.changed_by_employee_code — với schema ĐANG
// ÁP DỤNG hiện tại. task.categories đã ĐÚNG (2 cột audit employee_code đều
// nullable) — chỉ 2 bảng permission_grants*/grant_history* bị lệch.
// KHÔNG tự ý ALTER schema ở đây (ngoài phạm vi gate này, cần GO riêng) —
// code dưới đây port ĐÚNG hành vi dự kiến, sẽ hoạt động đúng NGAY KHI 2 cột
// này được sửa thành nullable (khớp đúng pattern categories đã có).
// ---------------------------------------------------------------------------
const TASK_PERMISSION_GRANT_TYPES = ['extend'];
const TASK_PERMISSION_GRANT_SCOPE_TYPES = ['employees', 'all_company'];

async function createTaskPermissionGrant(config, params) {
  const { granteeEmployeeCode, peopleScope, reason, actorAccountId, actorEmployeeCode } = params;

  const normGrantee = granteeEmployeeCode && String(granteeEmployeeCode).trim() !== '' ? String(granteeEmployeeCode).trim().toUpperCase() : null;
  if (normGrantee === null) throw taskWriteError('TASK_PERMISSION_GRANT_GRANTEE_REQUIRED');

  if (!peopleScope || typeof peopleScope !== 'object' || Array.isArray(peopleScope)) {
    throw taskWriteError('TASK_PERMISSION_GRANT_SCOPE_REQUIRED');
  }
  const scopeType = peopleScope.type && String(peopleScope.type).trim() !== '' ? String(peopleScope.type).trim().toLowerCase() : null;
  if (scopeType === null || TASK_PERMISSION_GRANT_SCOPE_TYPES.indexOf(scopeType) === -1) {
    throw taskWriteError('TASK_PERMISSION_GRANT_SCOPE_REQUIRED');
  }
  let scopeValues = [];
  if (scopeType === 'employees') {
    scopeValues = Array.isArray(peopleScope.values) ? peopleScope.values.map((v) => String(v).trim().toUpperCase()).filter(Boolean) : [];
    if (!scopeValues.length) throw taskWriteError('TASK_PERMISSION_GRANT_SCOPE_REQUIRED');
  }
  const normScope = { type: scopeType, values: scopeValues };

  const normReason = reason && String(reason).trim() !== '' ? String(reason).trim() : null;
  if (normReason === null) throw taskWriteError('TASK_PERMISSION_GRANT_REASON_REQUIRED');

  const normActorAccountId = actorAccountId && String(actorAccountId).trim() !== '' ? String(actorAccountId).trim() : null;
  const normActorEmployeeCode = actorEmployeeCode && String(actorEmployeeCode).trim() !== '' ? String(actorEmployeeCode).trim().toUpperCase() : null;
  if (normActorAccountId === null && normActorEmployeeCode === null) throw taskWriteError('TASK_PERMISSION_GRANT_ACTOR_REQUIRED');

  return withTaskWriteTransaction(config, async (client) => {
    const inserted = await client.query(
      `INSERT INTO task.permission_grants (
         grantee_employee_code, grant_type, people_scope, capabilities,
         effective_from, effective_to, reason, is_active,
         created_by_account_id, created_by_employee_code,
         updated_by_account_id, updated_by_employee_code, updated_at
       ) VALUES ($1, 'extend', $2::jsonb, '{}'::jsonb, now(), NULL, $3, true, $4, $5, $4, $5, now())
       RETURNING *`,
      [normGrantee, JSON.stringify(normScope), normReason, normActorAccountId, normActorEmployeeCode]
    );
    const grant = inserted.rows[0];

    await client.query(
      `INSERT INTO task.permission_grant_history (
         grant_id, changed_field, old_value, new_value, changed_by_employee_code, changed_by_account_id, reason
       ) VALUES ($1, 'created', NULL, $2::jsonb, $3, $4, $5)`,
      [grant.id, JSON.stringify(grant), normActorEmployeeCode, normActorAccountId, normReason]
    );
...
async function revokeTaskPermissionGrant(config, params) {
  const { grantId, reason, actorAccountId, actorEmployeeCode } = params;

  const normGrantId = grantId && String(grantId).trim() !== '' ? String(grantId).trim() : null;
  if (normGrantId === null) throw taskWriteError('TASK_PERMISSION_GRANT_ID_INVALID');
  const normReason = reason && String(reason).trim() !== '' ? String(reason).trim() : null;
  if (normReason === null) throw taskWriteError('TASK_PERMISSION_GRANT_REASON_REQUIRED');
  const normActorAccountId = actorAccountId && String(actorAccountId).trim() !== '' ? String(actorAccountId).trim() : null;
  const normActorEmployeeCode = actorEmployeeCode && String(actorEmployeeCode).trim() !== '' ? String(actorEmployeeCode).trim().toUpperCase() : null;
  ...
  return withTaskWriteTransaction(config, async (client) => {
    const found = await client.query('SELECT * FROM task.permission_grants WHERE id = $1 FOR UPDATE', [normGrantId]);
    if (found.rowCount === 0) throw taskWriteError('TASK_PERMISSION_GRANT_NOT_FOUND');
    const existing = found.rows[0];

    if (String(existing.grant_type).toLowerCase() !== 'extend') {
      throw taskWriteError('TASK_PERMISSION_REVOKE_TYPE_NOT_SUPPORTED');
    }
    if (existing.is_active !== true) throw taskWriteError('TASK_PERMISSION_GRANT_ALREADY_REVOKED');

    const updated = await client.query(
      `UPDATE task.permission_grants
          SET is_active = false, updated_by_account_id = $2, updated_by_employee_code = $3, updated_at = now()
        WHERE id = $1 AND is_active = true
        RETURNING *`,
      [normGrantId, normActorAccountId, normActorEmployeeCode]
    );
    ...
    await client.query(
      `INSERT INTO task.permission_grant_history (
         grant_id, changed_field, old_value, new_value, changed_by_employee_code, changed_by_account_id, reason
       ) VALUES ($1, 'is_active', 'true'::jsonb, 'false'::jsonb, $2, $3, $4)`,
      [normGrantId, normActorEmployeeCode, normActorAccountId, normReason]
    );

    return { revoked: true, grant_id: revoked.id, grantee_employee_code: revoked.grantee_employee_code };
  });
}

// ---------------------------------------------------------------------------
// Gate 5 — Attachment DB-layer. QUAN TRỌNG (kiến trúc đã CLOSED ở G5.4):
// đây là DB-LAYER THUẦN TÚY — KHÔNG nhận HTTP request stream, KHÔNG mở/ghi/
// đọc filesystem, KHÔNG require('fs'), KHÔNG gọi bất kỳ hàm nào trong
// lib/attachment-storage.js (module đó CHỈ dùng bởi orchestrator ở gate sau,
// tách biệt hoàn toàn). File này CHỈ import lib/attachment-policy.js để tái
// dùng MAX_FILE_SIZE/isAllowedMime() làm backstop validate — KHÔNG sửa
// policy file, KHÔNG duplicate giá trị.
//
// Postgres transaction (withTaskWriteTransaction) KHÔNG BAO GIỜ "giả vờ"
// atomic với filesystem — mọi function dưới đây CHỈ commit/rollback đúng
// phạm vi Postgres, đúng nguyên tắc đã CLOSED ở G5.2/G5.3.
//
// CHECKSUM_SHA256_RE: 64 hex char, khớp đúng độ dài SHA256 hex digest.
const CHECKSUM_SHA256_RE = /^[0-9a-f]{64}$/i;

// ---------------------------------------------------------------------------
// findTaskAttachmentByObjectKey — READ-ONLY, dùng cho idempotency replay
// lookup (orchestrator gate sau gọi TRƯỚC khi quyết định stream file — đúng
// G5.2 "SELECT replay-check TRƯỚC khi ghi file"). KHÔNG filesystem.
// ---------------------------------------------------------------------------
async function findTaskAttachmentByObjectKey(config, params) {
  const { storedObjectKey } = params;
  const objectKey = storedObjectKey && String(storedObjectKey).trim();
  if (!objectKey) throw taskWriteError('TASK_ATTACHMENT_OBJECT_KEY_REQUIRED');

  return withTaskWriteTransaction(config, async (client) => {
    const result = await client.query('SELECT * FROM task.attachments WHERE stored_object_key = $1 LIMIT 1', [objectKey]);
...
// KHÔNG stream, KHÔNG claim file, KHÔNG filesystem compensation — file VẬT
// LÝ đã được orchestrator ghi/publish XONG (qua lib/attachment-storage.js)
// TRƯỚC KHI gọi hàm này; hàm này chỉ ghi metadata cho 1 object đã tồn tại
// thật trên đĩa.
//
// sizeBytes: schema CHECK constraint thật là "size_bytes > 0" (migrations/
// phf_hr_task_foundation_v1.sql dòng 580 — task_attachments_size_ck), KHÔNG
// PHẢI ">=0" — validate theo ĐÚNG constraint DB thật, không theo diễn giải.
//
// UNIQUE CONFLICT (stored_object_key, SQLSTATE 23505): KHÔNG tự coi là
// success nếu chưa xác nhận winner — bắt lỗi, ROLLBACK đã tự xảy ra (xử lý
// bên trong withTaskWriteTransaction, connection đã release), rồi tự lookup
// winner qua findTaskAttachmentByObjectKey() bằng 1 CONNECTION MỚI HOÀN TOÀN
// (pool.connect() độc lập, KHÔNG liên quan gì transaction vừa rollback) —
// đúng pattern race-backstop đã CLOSED từ createDraftTask (Batch 3), verbatim
// tái dùng cho attachment. Đây CHÍNH LÀ cách "chứng minh transaction state
// sạch": lookup chạy trong transaction MỚI, không đọc lại state của
// transaction đã hỏng.
// ---------------------------------------------------------------------------
async function createTaskAttachmentMetadata(config, params) {
  ...
  if (!filename) throw taskWriteError('TASK_ATTACHMENT_FILENAME_REQUIRED');
  const objectKey = storedObjectKey && String(storedObjectKey).trim();
  if (!objectKey) throw taskWriteError('TASK_ATTACHMENT_OBJECT_KEY_REQUIRED');
  ...
  if (normalizedSize > MAX_FILE_SIZE) throw taskWriteError('TASK_ATTACHMENT_TOO_LARGE');
  ...
  try {
    ...
    if (err && err.code === '23505') {
      const winner = await findTaskAttachmentByObjectKey(config, { storedObjectKey: objectKey });
...
// ---------------------------------------------------------------------------
// removeTaskAttachment — logical remove ĐÚNG G5.1 quyết định #6: status
// 'active' -> 'pending_delete' + deleted_at + deleted_by_employee_code, ghi
// event 'attachment' action='remove' — atomic. KHÔNG BAO GIỜ DELETE row
// task.attachments, KHÔNG chạm filesystem (physical cleanup là job riêng
...
async function removeTaskAttachment(config, params) {
  ...
  return withTaskWriteTransaction(config, async (client) => {
    const current = await client.query(
      'SELECT id, status FROM task.attachments WHERE id = $1 AND task_id = $2 FOR UPDATE',
      [attachmentId, taskId]
    );
    if (current.rowCount === 0) throw taskWriteError('TASK_ATTACHMENT_NOT_FOUND');
    if (current.rows[0].status !== 'active') throw taskWriteError('TASK_ATTACHMENT_ALREADY_REMOVED');
    ...
    const payload = { action: 'remove', attachment_id: attachment.id };
...
// ---------------------------------------------------------------------------
// getTaskAttachmentForDownload — READ-ONLY, chỉ trả metadata attachment
// đang 'active' đúng task đã yêu cầu. KHÔNG filesystem, KHÔNG HTTP response —
// orchestrator gate sau tự dùng metadata này (stored_object_key) để gọi
// lib/attachment-storage.js đọc file thật. status != 'active' HOẶC taskId
// sai HOẶC attachmentId sai đều gộp chung 1 mã lỗi (KHÔNG tiết lộ đã từng
// tồn tại rồi bị xóa — đúng G5.2 "download route PHẢI kiểm status trước khi
// serve, trả 404 đồng nhất").
// ---------------------------------------------------------------------------
async function getTaskAttachmentForDownload(config, params) {
  ...
  return withTaskWriteTransaction(config, async (client) => {
    const result = await client.query(
      `SELECT * FROM task.attachments WHERE id = $1 AND task_id = $2 AND status = 'active' LIMIT 1`,
      [attachmentId, taskId]
    );
    if (result.rowCount === 0) throw taskWriteError('TASK_ATTACHMENT_NOT_FOUND');
