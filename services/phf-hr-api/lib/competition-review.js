...
// When a reviewer grant is revoked OR LOWERED, return their now-ineligible
// unprocessed active assignments to the pool and reassign among the
// remaining eligible reviewers. `tiers` scopes the sweep:
//   - full revoke               -> both tiers (default, unchanged behaviour)
//   - downgrade below level 2   -> 'primary_high' only (level-1 authority is
//     retained, so their primary_l1 assignments stay theirs)
async function returnAssignmentsForRevokedReviewer(config, params) {
  const tiers = params.tiers || ['primary_l1', 'primary_high'];
  return writeTx(config, async (client) => {
    const rows = await client.query(
      `SELECT * FROM competition.review_assignments
        WHERE campaign_id = $1 AND reviewer_account_id = $2 AND is_active AND status IN ('assigned','in_progress')
          AND tier = ANY($3::text[]) FOR UPDATE`,
      [params.campaignId, params.accountId, tiers]);
    const out = [];
    for (const a of rows.rows) {
      ...
      await histRow(client, a.submission_id, a.id, 'return_to_pool', a.reviewer_account_id, a.reviewer_employee_code, null, { reason: params.reasonCode || 'reviewer_revoked' }, null);
      const sub = await client.query('SELECT author_account_id, author_employee_code, status, current_level_order FROM competition.submissions WHERE id = $1', [a.submission_id]);
      if (!['submitted', 'needs_revision', 'approved'].includes(sub.rows[0].status)) continue;
      if (a.tier === 'primary_l1' && ['submitted', 'needs_revision'].includes(sub.rows[0].status)) {
        const re = await assignForSubmission(client, {
          submissionId: a.submission_id, campaignId: a.campaign_id,
          authorAccountId: sub.rows[0].author_account_id, authorEmployeeCode: sub.rows[0].author_employee_code, resubmit: false,
        });
        out.push({ submissionId: a.submission_id, tier: a.tier, reassignedTo: re ? re.reviewerAccountId : null });
      } else if (a.tier === 'primary_high') {
        const re = await ensureHighAssignment(client, {
          submissionId: a.submission_id, campaignId: a.campaign_id,
          authorAccountId: sub.rows[0].author_account_id, authorEmployeeCode: sub.rows[0].author_employee_code,
          currentLevelOrder: sub.rows[0].current_level_order,
        });
        out.push({ submissionId: a.submission_id, tier: a.tier, reassignedTo: re ? re.reviewerAccountId : null });
      }
    }
...
//
// C4.4 fix: self-view MUST count actual review_assignments for the acting
// identity directly — it must NOT depend on that identity holding a row in
// reviewer_grants. A Competition Admin (system admin, or an admin_grants
// holder) reviews via the "admin acts as unbounded reviewer" bypass in
// assertReviewerCanActOnLevel and never gets a reviewer_grants row; the old
// query built its result FROM reviewer_grants, so an admin who genuinely
// approved/rejected submissions still saw 0/0/0/0 — the review itself had
// committed correctly (review_assignments.status='completed',
// review_assignment_history, submission status all update in the SAME
// reviewAction transaction), only this READ was mis-scoped. A revoked/
// downgraded reviewer's past work must also stay visible to them for the
// same reason (history is real, independent of current grant state).
async function reviewerProductivity(config, actor, params) {
  ...
  return readTx(config, async (client) => {
    if (!wantAll) {
      // "assigned" is a historical volume ("Đã nhận" = ever received) ->
      // any review_assignments row for this identity, regardless of the
      // submission's current status. "pending"/"overdue" ("Đang chờ" /
      // "Quá hạn") MUST mean the same thing anonymousQueue means by
      // "actionable in your queue right now" — the queue's WHERE clause
      // requires the underlying submission to still be
      // status IN ('submitted','needs_revision'); an assignment row can
      // stay is_active/'assigned' after its submission was later approved
      // (e.g. a primary_high "possible upgrade" assignment auto-created by
      // ensureHighAssignment once a high reviewer's L1-approved submission
      // is offered to them) — that is invisible in the queue (approved !=
      // submitted/needs_revision) but was still being counted as "pending"
      // here, producing pendingCount=1 with an empty queue (found live on
      // PHF012). Joining submissions with the SAME status filter the queue
      // uses is the fix at the source, not a UI workaround.
      // "processed" must reflect ACTUAL completions by this identity, which
      // is NOT the same set: reviewAction lets an admin (or any reviewer at
      // a high-enough level) complete a submission that was auto-assigned to
      // someone else — completeAssignmentForReviewer then can't find a
      // matching assignment row for the acting identity and only logs a
      // review_assignment_history row (action='completed', actor_* =
      // the acting identity) without ever creating/owning an assignment row
      // for them. Counting "processed" from review_assignments alone
      ...
      const self = await client.query(
        `SELECT
                ...
                (SELECT COALESCE(count(DISTINCT rah.submission_id),0) FROM competition.review_assignment_history rah
                  JOIN competition.submissions s ON s.id = rah.submission_id
      ...
      return {
        ...
        maxLevelOrder: auth.isCompetitionAdmin ? null : auth.reviewerMaxLevel,
    ...
    const r = await client.query(
      `WITH rg AS (
       ...
       SELECT rg.account_id, rg.employee_code, rg.display_name, rg.max_level_order,
         ...
         LEFT JOIN competition.submissions s ON s.id = ra.submission_id
    ...
    const rows = r.rows.map((x) => ({
      ...
      displayName: x.display_name, maxLevelOrder: x.max_level_order,
      assigned: Number(x.assigned_count), processed: Number(x.processed_count),
      pending: Number(x.pending_count), overdue: Number(x.overdue_count),
    }));
