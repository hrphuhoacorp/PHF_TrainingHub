...
(async () => {
  ...
  // ===========================================================================
  {
    const client = makeFakeClient([
      ...
      { expect: SET_TIMEOUT, result: {} },
      {
        ...
        expect: /^SELECT t\.id, t\.task_code, t\.flow_type, t\.status, t\.title, t\.priority, t\.deadline, t\.category_code, t\.progress_percent, t\.progress_status, t\.created_by_employee_code, t\.created_by_account_id, t\.recurring_series_id, t\.is_cross_department, t\.source_department, t\.target_department, t.created_at, t.completed_at, t.row_version, pd\.proposal_status, pd\.recipient_employee_code, pd\.generated_task_id, pd\.reject_reason, pd\.cancel_reason, pd\.decided_by_employee_code, pd\.decided_at, EXISTS \( SELECT 1 FROM task\.proposal_decisions gpd WHERE gpd\.generated_task_id = t\.id \) AS proposal_generated FROM task\.tasks t LEFT JOIN task\.proposal_decisions pd ON pd\.proposal_task_id = t\.id WHERE flow_type = \$1 AND t\.created_by_employee_code = \$2 ORDER BY CASE WHEN t.status IN \('published', 'in_progress'\) THEN 0 ELSE 1 END, t.deadline ASC NULLS LAST, CASE t.priority WHEN 'khan_cap' THEN 0 WHEN 'quan_trong' THEN 1 ELSE 2 END, t.created_at DESC, t.id ASC LIMIT \$3 OFFSET \$4$/,
  ...
  // ===========================================================================
  {
    const client = makeFakeClient([
      ...
      {
        expect: /^SELECT t\.id, t\.task_code, t\.flow_type, t\.status, t\.title, t\.priority, t\.deadline, t\.category_code, t\.progress_percent, t\.progress_status, t\.created_by_employee_code, t\.created_by_account_id, t\.recurring_series_id, t\.is_cross_department, t\.source_department, t\.target_department, t.created_at, t.completed_at, t.row_version, pd\.proposal_status, pd\.recipient_employee_code, pd\.generated_task_id, pd\.reject_reason, pd\.cancel_reason, pd\.decided_by_employee_code, pd\.decided_at, EXISTS \( SELECT 1 FROM task\.proposal_decisions gpd WHERE gpd\.generated_task_id = t\.id \) AS proposal_generated FROM task\.tasks t LEFT JOIN task\.proposal_decisions pd ON pd\.proposal_task_id = t\.id WHERE flow_type = \$1 AND id = ANY\(\$2::uuid\[\]\) AND status <> 'draft' AND is_cross_department = true ORDER BY CASE WHEN t.status IN \('published', 'in_progress'\) THEN 0 ELSE 1 END, t.deadline ASC NULLS LAST, CASE t.priority WHEN 'khan_cap' THEN 0 WHEN 'quan_trong' THEN 1 ELSE 2 END, t.created_at DESC, t.id ASC LIMIT \$3 OFFSET \$4$/,
  ...
  // ===========================================================================
  {
    const client = makeFakeClient([
      ...
      { expect: SET_TIMEOUT, result: {} },
      {
        expect: /^SELECT t\.id, t\.task_code, t\.flow_type, t\.status, t\.title, t\.priority, t\.deadline, t\.category_code, t\.progress_percent, t\.progress_status, t\.created_by_employee_code, t\.created_by_account_id, t\.recurring_series_id, t\.is_cross_department, t\.source_department, t\.target_department, t.created_at, t.completed_at, t.row_version, pd\.proposal_status, pd\.recipient_employee_code, pd\.generated_task_id, pd\.reject_reason, pd\.cancel_reason, pd\.decided_by_employee_code, pd\.decided_at, EXISTS \( SELECT 1 FROM task\.proposal_decisions gpd WHERE gpd\.generated_task_id = t\.id \) AS proposal_generated FROM task\.tasks t LEFT JOIN task\.proposal_decisions pd ON pd\.proposal_task_id = t\.id WHERE flow_type = \$1 AND t\.created_by_employee_code = \$2 AND status IN \('published', 'in_progress'\) AND deadline < \$3 AND \(task_code ILIKE \$4 OR title ILIKE \$4\) ORDER BY CASE WHEN t.status IN \('published', 'in_progress'\) THEN 0 ELSE 1 END, t.deadline ASC NULLS LAST, CASE t.priority WHEN 'khan_cap' THEN 0 WHEN 'quan_trong' THEN 1 ELSE 2 END, t.created_at DESC, t.id ASC LIMIT \$5 OFFSET \$6$/,
