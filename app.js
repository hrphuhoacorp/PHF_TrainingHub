/* PHF Training Hub Local Prototype - app.js
   Bọc thêm lớp lưu dữ liệu local vào data.json qua server.js.
...
(function () {
  ...
  function collectEmployee() {
    ...
    const values = inputs.map(el => (el.value || '').trim()).filter(Boolean);
    const guessedName = text('input[name="fullName"]') || text('#fullName') || text('input[placeholder*="Họ" i]') || values[0] || 'Nhân viên demo PHF';
    const guessedBranch = text('select[name="branch"]') || text('#branch') || text('input[name="branch"]') || '';
    const guessedBirthday = text('input[type="date"]') || '';

    const normalizedName = guessedName.replace(/\s+/g, ' ').trim();
    const idSource = normalizedName && normalizedName !== 'Nhân viên demo PHF' ? normalizedName : employeeId;
    const nextId = idSource
      .toLowerCase()
  ...
  function detectTestResult() {
    ...
    const passed = /đạt|pass|mở bước|hoàn thành/i.test(resultText) && !/chưa đạt|không đạt|fail/i.test(resultText);
  ...
  function completedPagesFromDom() {
    return Array.from(document.querySelectorAll('.nav-step.done, .phf-nav-item.done, .active'))
  ...
  function updateSaveBadge(text) {
    ...
    if (!badge) {
      ...
      badge.style.cssText = 'position:fixed;left:16px;bottom:16px;z-index:99999;background:#064533;color:#fff;border-radius:999px;padding:9px 12px;font:700 12px Segoe UI,Arial;box-shadow:0 10px 24px rgba(5,63,49,.2);opacity:.92';
  ...
  window.fakeSave = function (msg) {
    saveLocal('manual');
    if (typeof originalFakeSave === 'function') originalFakeSave(msg || 'Đã lưu thử');
    else alert((msg || 'Đã lưu thử') + '.');
  };

  document.addEventListener('click', function (e) {
    const target = e.target.closest('button, [data-go], .phf-nav-item, .nav-step, .chip');
    if (!target) return;
    setTimeout(function () {
      const isTestButton = /grade|test|nộp|chấm|hoàn thành/i.test(target.id + ' ' + target.textContent);
      saveLocal(isTestButton ? 'test' : 'navigation');
    }, 350);
  });

  document.addEventListener('change', function () {
    clearTimeout(window.__phfLocalSaveTimer);
    window.__phfLocalSaveTimer = setTimeout(() => saveLocal('form-change'), 500);
  });

  loadLocal().then(() => saveLocal('page-open'));
})();
