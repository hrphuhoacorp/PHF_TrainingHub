...
/* SCROLL STABILITY (2026-09-03) — renderTaskRoot() rebuilds the whole Task
   subtree with root.innerHTML on every re-render, including LATE async ones
   (managed-scope probe resolving, reloadTaskDetail after a lifecycle action,
   retrySupplements, notification-driven refreshes…). While the old DOM is gone
   the browser clamps window scrollY toward 0, so a user reading half-way down
   a task gets thrown back to the top. We snapshot scrollY before the swap and
   restore it right after — but ONLY for a re-render of the SAME screen/task and
   NEVER during an intentional navigation (window.phfRenderTask sets
   phfTaskNavigating and does its own scrollTo(0,0) at the end of the nav). */
var phfTaskNavigating = false;
var phfTaskPendingScrollRestore = 0;
var phfTaskPendingScrollTries = 0;
var phfTaskLastRenderKey = '';
function phfTaskRenderKey(){
  return String((taskUiState && taskUiState.view) || '') + '|' + String((taskUiState && taskUiState.taskId) || '');
}
...
// TASK DETAIL — "Nội dung công việc" primary card (2026-09-03). Renders the
// canonical Task body (task.content) that the model already stores — no new
// field, no second source of truth. Given its own prominent, full-width card
// right under the hero so a viewer reads "what is the work" first, instead of
// the body being a small line lost among vitals/tech metadata. Empty body ->
// a plain hint, never a blank card. No permission/visibility change: the
// caller already gates the whole detail payload.
function taskDetailContentCardHtml(task){
  var body=String((task&&task.content)||'').trim();
  return '<section class="phft-detail-card phft-detail-content">'+
    '<header><h2>Nội dung công việc</h2></header>'+
    (body
      ? '<div class="phft-detail-content-body">'+esc(body)+'</div>'
      : '<p class="phft-detail-content-empty">Người giao chưa mô tả chi tiết cho công việc này. Trao đổi trực tiếp với người giao nếu cần làm rõ yêu cầu.</p>')+
  '</section>';
}
...
function renderTaskRoot(root){
  ...
  if(wantY>0){
    ...
    try{
      ...
      var sh=doc?Math.max(doc.documentElement?doc.documentElement.scrollHeight:0, doc.body?doc.body.scrollHeight:0):0;
      maxY=Math.max(0,sh-vh);
    }catch(e){}
    var target=(maxY>0)?Math.min(wantY,maxY):wantY;
    try{
      if(typeof window!=='undefined'&&typeof window.scrollTo==='function'&&Math.abs((window.pageYOffset||window.scrollY||0)-target)>1){
        window.scrollTo(0,target);
      }
    }catch(e){}
    // If the page is still shorter than where the user was (the brief cold-load
    // card, or layout not settled yet), the browser clamps the restore — keep
    // the intent and finish it on the next render that is tall enough.
    // Deterministic, no timers.
    var achieved=0;
    try{ achieved=(typeof window!=='undefined')?(window.pageYOffset||window.scrollY||0):0; }catch(e){}
    if(achieved<wantY-1&&phfTaskPendingScrollTries<4){
      phfTaskPendingScrollRestore=wantY; phfTaskPendingScrollTries+=1;
    }else{
      phfTaskPendingScrollRestore=0; phfTaskPendingScrollTries=0;
    }
  }else{
    phfTaskPendingScrollRestore=0; phfTaskPendingScrollTries=0;
  }
...
async function reloadTaskDetail(root){
  ...
  taskUiState.view='detail';taskUiState.detailError='';
  // Cold load -> full loading screen. REFRESH (we already hold a truthful
  // detail, e.g. after a lifecycle action) -> keep the current content on
  // screen and swap in the fresh copy when it lands. No loading flash, so the
  // reader's scroll position is not disturbed by a tall->short->tall bounce.
  if(!taskUiState.detail)taskUiState.detailLoading=true;
  renderTaskRoot(root);
...
window.phfRenderTask = async function(path){
  ...
  phfTaskNavigating = true;         // suppress renderTaskRoot scroll-restore for this nav
  phfTaskPendingScrollRestore = 0; phfTaskPendingScrollTries = 0; // drop stale restore intent from the previous screen
  try{
