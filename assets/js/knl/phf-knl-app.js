...
var foundationState={frameworks:[],detail:null,matrix:null,standards:null,preview:null,income:null,incomeTargets:[],incomeTargetsLoaded:false,incomeCanSelect:false,incomeIsAdmin:false,error:'',pendingNewGrades:[]};
...
async function loadFoundationVersion(versionId){var id=versionId||new URL(location.href).searchParams.get('version');if(!foundationState.frameworks.length){var list=await apiPost('listKnlFrameworks');foundationState.frameworks=list.frameworks||[];}if(!id){var first=(foundationState.frameworks[0]&&foundationState.frameworks[0].versions||[])[0];id=first&&first.id;}if(!id)return null;
  var previousId=foundationState.detail&&foundationState.detail.version&&foundationState.detail.version.id;if(id!==previousId)foundationState.pendingNewGrades=[];
  ...
  if(!foundationState.matrix.grades.length&&!foundationState.pendingNewGrades.length){var levelCols=orderedActive(foundationState.detail.columns).filter(function(c){return c.type==='level';});foundationState.pendingNewGrades=levelCols.map(function(c,index){var n=index+1;return{gradeCode:'B'+n,gradeNumber:n,label:'Bậc '+n,sortOrder:n};});}
  return id;}
function gradeMatrixHtml(){var d=foundationState.detail,m=foundationState.matrix;if(!d||!m)return noAccessSection('Chưa có Framework Version để cấu hình.');var levels=orderedActive(d.columns).filter(function(c){return c.type==='level';}),items=orderedActive(d.items),savedGrades=m.grades||[],pending=!savedGrades.length,grades=pending?foundationState.pendingNewGrades:savedGrades,byKey={};(m.requirements||[]).forEach(function(r){byKey[r.itemId+':'+r.gradeId]=r;});var warning=false;items.forEach(function(item){var prior=0;savedGrades.forEach(function(g){var r=byKey[item.id+':'+g.id],n=Number(r&&r.requiredLevelNumber||1);if(prior&&n<prior)warning=true;prior=n;});});
  var mutable=d.version.lifecycleStatus==='DRAFT'&&!d.version.isLocked;
  var addGradeBtn=pending&&mutable?'<button type="button" class="phfk-btn-secondary" data-grade-add>+ Thêm bậc</button>':'';
  return frameworkDomainNav('tieu-chuan-bac')+'<div class="phfk-page-head"><div><small>KNL · GRADE STANDARD</small><h1>Tiêu chuẩn bậc năng lực</h1></div></div><label class="phfk-field phfk-foundation-select"><span>Framework Version</span><select class="phfk-input" data-foundation-version>'+foundationVersionOptions()+'</select></label><section class="phfk-panel"><div class="phfk-section-head"><div><small>'+esc(d.framework.code+' · v'+d.version.versionNumber)+'</small><h2>Item × Bậc = Mức bắt buộc</h2></div><div class="phfk-mini-actions">'+addGradeBtn+'<button class="phfk-btn-primary" data-grade-save'+(!grades.length||!mutable?' disabled':'')+'>Lưu ma trận</button></div></div><p class="phfk-batch-note">Mỗi ô là yêu cầu độc lập; không tính trung bình. Số bậc B1..Bn và mức M1..Mn lấy động theo version.</p>'+(!savedGrades.length?'<p class="phfk-warning">Version chưa có grade definitions chính thức. Hệ thống gợi ý sẵn B1–B'+levels.length+' (khớp số mức M) và mặc định mỗi ô là M1 để Admin dễ chỉnh — đây chỉ là gợi ý client-side, chưa ghi vào hệ thống. Hãy tự đặt tên bậc, thêm/bớt bậc, chọn đúng mức yêu cầu từng ô rồi bấm "Lưu ma trận" mới thành chính thức.</p>':'')+(warning?'<p class="phfk-warning">Có bậc sau thấp hơn bậc trước. Hệ thống chỉ cảnh báo, không tự sửa nghiệp vụ.</p>':'')+(!grades.length?'':'<div class="phfk-dynamic-table-wrap"><table class="phfk-dynamic-table phfk-grade-table"><thead><tr><th>Hạng mục</th>'+grades.map(function(g){return'<th>'+esc(g.label||g.gradeCode)+(pending&&mutable?' <button type="button" class="phfk-mini-remove" data-grade-remove="'+esc(g.gradeCode)+'" title="Bỏ bậc này">×</button>':'')+'</th>';}).join('')+'</tr></thead><tbody>'+items.map(function(item){return'<tr><td><b>'+esc(item.name)+'</b></td>'+grades.map(function(g){var r=byKey[item.id+':'+g.id],selected=Number(r&&r.requiredLevelNumber||1);return'<td><select class="phfk-input" data-grade-cell="'+esc(item.id)+':'+esc(g.gradeCode)+'"'+(mutable?'':' disabled')+'>'+levels.map(function(l){return'<option value="'+esc(l.id)+'|'+l.levelNumber+'"'+(l.levelNumber===selected?' selected':'')+'>M'+l.levelNumber+'</option>';}).join('')+'</select></td>';}).join('')+'</tr>';}).join('')+'</tbody></table></div>')+'</section>'+(foundationState.error?'<p class="phfk-error">'+esc(foundationState.error)+'</p>':'');}
async function renderGradeMatrix(root,versionId){var body=root.querySelector('[data-knl-body]');try{var id=await loadFoundationVersion(versionId);body.innerHTML=gradeMatrixHtml();bindFrameworkDomainNav(root);
    ...
    var addGrade=root.querySelector('[data-grade-add]');if(addGrade)addGrade.onclick=function(){var n=foundationState.pendingNewGrades.length+1,label=prompt('Tên bậc B'+n+' (Admin tự đặt tên, không tự sinh):','Bậc '+n);if(label===null)return;foundationState.pendingNewGrades.push({gradeCode:'B'+n,gradeNumber:n,label:label||('Bậc '+n),sortOrder:n});renderGradeMatrix(root,id);};
    root.querySelectorAll('[data-grade-remove]').forEach(function(el){el.onclick=function(){var code=el.dataset.gradeRemove;foundationState.pendingNewGrades=foundationState.pendingNewGrades.filter(function(g){return g.gradeCode!==code;}).map(function(g,index){var n=index+1;return{gradeCode:'B'+n,gradeNumber:n,label:g.label,sortOrder:n};});renderGradeMatrix(root,id);};});
    var save=root.querySelector('[data-grade-save]');if(save)save.onclick=async function(){var grades=(foundationState.matrix.grades&&foundationState.matrix.grades.length)?foundationState.matrix.grades:foundationState.pendingNewGrades;if(!grades.length)return;var requirements=[];root.querySelectorAll('[data-grade-cell]').forEach(function(el){var key=el.dataset.gradeCell.split(':'),value=el.value.split('|');requirements.push({itemId:key[0],gradeCode:key[1],requiredColumnId:value[0],requiredLevelNumber:Number(value[1])});});try{await apiPost('saveKnlGradeMatrix',{versionId:id,grades:grades,requirements:requirements});foundationState.matrix=null;foundationState.pendingNewGrades=[];await renderGradeMatrix(root,id);}catch(e){foundationState.error=e.message;body.innerHTML=gradeMatrixHtml();}};
