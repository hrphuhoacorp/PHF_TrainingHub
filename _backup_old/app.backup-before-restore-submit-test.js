...
function detectTestResult() {
  ...
  const resultText = [
    ...
    getVisibleText('.result'),
    getVisibleText('.quiz-result'),
    getVisibleText('.score'),
    page
  ].filter(Boolean).join(' | ');

  if (!resultText) return null;

  const scoreMatch = resultText.match(/(\d{1,3})\s*\/\s*100|đạt\s*(\d{1,3})\s*\/\s*100|điểm\s*(\d{1,3})|(\d{1,3})\s*%/i);
  const score = scoreMatch
    ? Number(scoreMatch[1] || scoreMatch[2] || scoreMatch[3] || scoreMatch[4])
    : null;

  const passed =
    /đạt|pass|hoàn thành|mở bước/i.test(resultText) &&
