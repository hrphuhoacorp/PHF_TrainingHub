...
const { listReviewableEmployees } = require('./competition-people');
...
// ACTION_MAP passthrough: the reviewer list itself (with current
// max_level_order) is Company-PostgreSQL data (phf-hr-api), while the
// candidate people list is People Master (Supabase) — two different stores
// this layer is the only place already reading both. Authority is NOT
// re-implemented here: `competition.grant.listReviewers` is already
// requireCompetitionAdmin-gated server-side inside phf-hr-api, so calling it
// first both fetches the current grants AND fails closed (403) for anyone
// without Competition Admin authority, before any People Master read runs.
async function listReviewablePeople(session, campaignId) {
  const actor = await resolveCompetitionActor(session);
  const grants = await callCompetitionAction('competition.grant.listReviewers', actor, { campaignId });
  const grantByKey = new Map();
  for (const g of grants || []) {
    if (!g.isActive) continue;
    const key = (g.accountId || '') + '|' + (g.employeeCode || '');
    grantByKey.set(key, g.maxLevelOrder);
  }
  const people = await listReviewableEmployees();
  return people.map((p) => {
    const key = (p.accountId || '') + '|' + (p.employeeCode || '');
    const maxLevelOrder = grantByKey.has(key) ? grantByKey.get(key) : null;
    return Object.assign({}, p, { reviewerMaxLevel: maxLevelOrder });
  });
}
...
async function dispatchCompetitionAction(session, payload) {
  ...
  if (action === 'competitionListReviewablePeople') {
    const result = await listReviewablePeople(session, String((payload && payload.campaign_id) || ''));
