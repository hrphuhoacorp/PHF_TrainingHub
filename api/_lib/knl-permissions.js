...
// 2026-08-11 KNL Grade Promotion Proposal batch 2: thêm view_proposals — CỐ Ý
// tách khỏi view_people (mục 2 của yêu cầu, "Proposal Visibility KHÔNG dùng
// people_scope một cách máy móc"). view_proposals dùng proposalScope RIÊNG
// (lưu trong capabilities.proposalScope, cùng cơ chế đã dùng cho incomeScope —
// xem normalizeProposalScope()/normalizeGrant() bên dưới), KHÔNG đọc/ghi
// people_scope. propose/agree_proposal/approve giờ ĐÃ có workflow thật
// (lib/knl-grade-proposals.js) — enforce qua requirePropose/
// requireAgreeProposal/requireApprove bên dưới, dùng people_scope hiện có cho
// AUTHORITY (ai được xử lý cho ai) vì đây đúng là ý nghĩa gốc của people_scope
// ("phạm vi phụ trách"), khác với proposalScope (chỉ quyết định XEM danh sách
// đề xuất). manage_framework đã enforce từ trước (xem ghi chú cũ ở dưới).
// 2026-08-13 Dashboard KNL Gate 2 — dashboard_view: canonical gate cho quyền
// vào Dashboard KNL, THAY THẾ hoàn toàn cách gate tạm bằng tên preset
// (roleKeyFromPreset==='assistant') đã dùng ở Gate 1. Không cần capabilities
// column mới (jsonb, không ALTER TABLE) — capabilities() dưới đây tự nhận
// key mới qua CAPABILITY_KEYS. KHÔNG suy diễn từ preset/income_view/
// view_people: PHF cấp thủ công qua màn Phân quyền KNL hiện có (Admin đã có
// mặc định qua nhánh admin_recovery, xem resolveActorGrant()). dashboard_view
// CHỈ quyết định được vào Dashboard hay không — không quyết định phạm vi dữ
// liệu (đó vẫn là people_scope/incomeScope độc lập, xem lib/knl-dashboard.js).
const CAPABILITY_KEYS = ['access_knl', 'view_people', 'propose', 'agree_proposal', 'approve', 'manage_framework', 'manage_permissions', 'income_view', 'view_proposals', 'dashboard_view'];
...
const PRESETS = Object.freeze({
  NHAN_VIEN: {
    name: 'Nhân viên',
    capabilities: { access_knl: true, view_people: true, propose: false, agree_proposal: false, approve: false, manage_framework: false, manage_permissions: false, income_view: false, view_proposals: false, dashboard_view: false },
  ...
  CUSTOM: {
    name: 'Tùy chỉnh',
    capabilities: { access_knl: false, view_people: false, propose: false, agree_proposal: false, approve: false, manage_framework: false, manage_permissions: false, income_view: false, view_proposals: false, dashboard_view: false },
    peopleScope: { type: 'self', values: [], reservedEmployees: [] }
  }
});
 ...
 * (mã "PHF012" — Organization Master) — KHÔNG session.employeeId (internal
 * linked-employee id kiểu "hv-xxxxxxxxxx" của Training Hub, khác hệ mã).
 * resolved.identity.employeeCode từ actor() này được subjectMatchesScope()
 * dùng cho type:'self' (lib/knl-scope.js) ở nhiều nơi (view_people self-scope,
 * proposalScope self-scope, knl-competency self-view) — ưu tiên sai khiến
 * mọi so khớp self đều fail câm lặng. Cùng đúng pattern đã chốt ở
 * lib/knl-competency.js:actor() / lib/knl-foundation.js:actor() (2026-08-11),
 * áp dụng lại ở đây sau trace 2026-08-12 (case PHF012 grade-promotion). */
function actor(session) {
  return {
    id: text(session?.account?.id || session?.sub),
    name: text(session?.account?.name || session?.account?.email || session?.email),
    employeeCode: text(session?.employeeCode || session?.employee_code || session?.account?.employeeCode || session?.account?.employee_code).toUpperCase(),
...
// 2026-08-11 KNL Permission Clean: mở rộng thêm department/branch/title (đa
// giá trị, sourced từ employee_profiles qua lib/knl-people.js — không phải
// giá trị Admin tự gõ) để đáp ứng "Trợ lý GĐ phụ trách kinh doanh có thể được
// cấp xem nhiều phòng ban cùng lúc". Vẫn 1 type/lượt lưu (không kết hợp nhiều
// chiều cùng lúc trong 1 grant) — nếu cần kết hợp (vd nhiều phòng ban VÀ
// nhiều chi nhánh cùng lúc) thì đây là limitation cần PHF chốt thêm trước khi
// mở rộng thành mảng điều kiện, KHÔNG tự suy diễn ở đây.
const INCOME_SCOPE_TYPES = Object.freeze(new Set(['all_company', 'employees', 'department', 'branch', 'title']));
const INCOME_SCOPE_MULTI_VALUE_TYPES = Object.freeze(new Set(['employees', 'department', 'branch', 'title']));
...
function normalizeIncomeScope(value) {
  ...
  const values = INCOME_SCOPE_MULTI_VALUE_TYPES.has(type) ? normalizeScopeList(source.values) : [];
  if (INCOME_SCOPE_MULTI_VALUE_TYPES.has(type) && !values.length) {
    const label = { employees: 'Chọn nhân sự cụ thể', department: 'Theo phòng ban', branch: 'Theo chi nhánh', title: 'Theo chức danh' }[type];
    fail('Phạm vi "' + label + '" (Thu nhập) cần chọn ít nhất một giá trị.', 400, 'KNL_INCOME_SCOPE_VALUES_REQUIRED');
  }
  return { type, values };
}

/* PHF HR – KNL GRADE PROMOTION PROPOSAL batch 2, mục 2 — "Proposal Visibility
 * KHÔNG dùng people_scope một cách máy móc". proposalScope là khái niệm HOÀN
 * TOÀN RIÊNG với people_scope, lưu trong CHÍNH cột capabilities jsonb đã có
 * (không ALTER TABLE) dưới key `proposalScope`, đúng kỹ thuật đã dùng cho
 * incomeScope ở trên. Population tối thiểu = đúng 5 type của KNL_SCOPE_TYPES
 * (self/sales_all_branches/department/employees/all_company — mục 2 của yêu
 * cầu) — tái dùng luôn set này và hàm subjectMatchesScope() để CHẤM quyền
 * (semantics phù hợp: cùng là "nhân sự nào thuộc phạm vi ai"), nhưng GIÁ TRỊ
 * lưu trữ (proposalScope) độc lập hoàn toàn với people_scope — đổi cái này
 * không ảnh hưởng cái kia, và ngược lại. Không có reserve/restore (giống
 * incomeScope, không income_view thì không giữ lịch sử phục hồi). */
function normalizeProposalScope(value) {
  if (value == null) return null;
  const source = value && typeof value === 'object' ? value : {};
  const type = text(source.type).toLowerCase();
  if (!KNL_SCOPE_TYPES.has(type)) fail('Phạm vi xem Đề xuất nâng bậc không hợp lệ: ' + type, 400, 'KNL_PROPOSAL_SCOPE_INVALID');
  const needsValues = type === 'department' || type === 'employees';
  const values = needsValues ? normalizeScopeList(source.values) : [];
  if (needsValues && !values.length) {
    const label = { employees: 'Chọn nhân sự cụ thể', department: 'Theo phòng ban' }[type];
    fail('Phạm vi "' + label + '" (Đề xuất nâng bậc) cần chọn ít nhất một giá trị.', 400, 'KNL_PROPOSAL_SCOPE_VALUES_REQUIRED');
  }
  return { type, values };
}
...
async function resolveActorGrant(session) {
  ...
  if (a.role === 'admin') {
    return {
      ...
      capabilities: capabilities({ access_knl: true, view_people: true, propose: true, agree_proposal: true, approve: true, manage_framework: true, manage_permissions: true, income_view: true, view_proposals: true, dashboard_view: true }),
      peopleScope: { type: 'all_company', values: [], reservedEmployees: [] },
      row: null,
      identity: a
    };
...
function requireDashboardView(resolved) {
  if (!resolved.capabilities.dashboard_view) fail('Tài khoản chưa được cấp quyền xem Dashboard KNL.', 403, 'KNL_DASHBOARD_VIEW_DENIED');
}
// KNL Grade Promotion Proposal batch 2 — 4 gate riêng biệt, KHÔNG suy quyền
// lẫn nhau (mục 1/11 của yêu cầu): xem danh sách đề xuất, tạo đề xuất, xử lý
// Đồng ý/Không đồng ý ở tầng trung gian, và quyết định cuối của Admin.
function requireViewProposals(resolved) {
  if (!resolved.capabilities.view_proposals) fail('Tài khoản chưa được cấp quyền xem Đề xuất nâng bậc.', 403, 'KNL_VIEW_PROPOSALS_DENIED');
}
function requirePropose(resolved) {
  if (!resolved.capabilities.propose) fail('Tài khoản chưa được cấp quyền tạo Đề xuất nâng bậc.', 403, 'KNL_PROPOSE_DENIED');
}
function requireAgreeProposal(resolved) {
  if (!resolved.capabilities.agree_proposal) fail('Tài khoản chưa được cấp quyền xử lý (Đồng ý/Không đồng ý) Đề xuất nâng bậc.', 403, 'KNL_AGREE_PROPOSAL_DENIED');
}
function requireApprove(resolved) {
  if (!resolved.capabilities.approve) fail('Tài khoản chưa được cấp quyền quyết định cuối cùng Đề xuất nâng bậc.', 403, 'KNL_APPROVE_DENIED');
}

/* incomeScopeAllows — DI DỜI từ lib/knl-foundation.js (2026-08-12, batch
 * "canViewIncome per-row" cho màn Nhân sự KNL) để lib/knl-people.js dùng
 * được mà KHÔNG tạo circular require (knl-foundation.js đã require
 * knl-people.js để lấy loadKnlOrganizationRows — nếu knl-people.js require
 * ngược lại knl-foundation.js sẽ vòng lặp). knl-permissions.js không phụ
 * thuộc knl-people.js/knl-foundation.js nên là nơi trung lập đúng để đặt
 * logic permission thuần này. lib/knl-foundation.js tiếp tục re-export
 * đúng hàm này (không định nghĩa lại) để KHÔNG đổi public API cho các
 * consumer cũ (scripts/test-knl-income-batch1-2026-08.js,
 * scripts/phf-knl-foundation-production-access-verify.js). Logic giữ
 * NGUYÊN VẸN 100%, không đổi hành vi.
 * person: string (employeeCode, backward-compatible) HOẶC object đầy đủ
 * {employeeCode,department,branch,title}. Income scope hoàn toàn độc lập
 * với people_scope/reservedEmployees — không đọc field nào khác ngoài 4
 * field này. */
function incomeScopeAllows(resolved, person) {
  if (resolved.source === 'admin_recovery') return true;
  if (resolved.capabilities.income_view !== true) return false;
  const scope = resolved.row && resolved.row.capabilities && (resolved.row.capabilities.incomeScope || resolved.row.capabilities.income_scope);
  if (!scope || typeof scope !== 'object') return false;
  if (scope.type === 'all_company') return true;
  const p = typeof person === 'string' ? { employeeCode: person } : (person || {});
  const values = Array.isArray(scope.values) ? scope.values.map(v => text(v).toUpperCase()) : [];
  if (!values.length) return false;
  if (scope.type === 'employees') return values.includes(text(p.employeeCode).toUpperCase());
  if (scope.type === 'department') return values.includes(text(p.department).toUpperCase());
  if (scope.type === 'branch') return values.includes(text(p.branch).toUpperCase());
  if (scope.type === 'title') return values.includes(text(p.title).toUpperCase());
  return false;
}
...
function normalizeGrant(input = {}, existing = null) {
  ...
  /* view_proposals=true bắt buộc phải có proposalScope hợp lệ (đúng cơ chế
   * incomeScope ở trên) — view_proposals=false thì KHÔNG ghi proposalScope,
   * lượt lưu tới ghi đè toàn bộ cột capabilities nên proposalScope cũ (nếu
   * có) biến mất hẳn, không sót lại tham gia enforcement sau này. */
  if (normalizedCapabilities.view_proposals) {
    const proposalScopeValue = normalizeProposalScope(capabilitiesSource.proposalScope || capabilitiesSource.proposal_scope);
    if (!proposalScopeValue) fail('Vui lòng chọn phạm vi xem Đề xuất nâng bậc.', 400, 'KNL_PROPOSAL_SCOPE_REQUIRED');
    normalizedCapabilities.proposalScope = proposalScopeValue;
  }
  return {
...
module.exports = {
  ...
  requireViewProposals,
  requirePropose,
  requireAgreeProposal,
  requireApprove,
