...
const { SCOPE_TYPES, subjectMatchesScope } = require('./checklist-scope');
...
// subjectMatchesScope: SOURCE OF TRUTH duy nhất cho so khớp scope, dùng chung
// với lib/checklist-violations.js - xem lib/checklist-scope.js.
...
/*
 * UX-01 Batch 2 — nguồn quyền xem "Hồ sơ đánh giá" của người khác (view_monthly).
 * CỐ Ý khác 4 hàm chị em ở trên (getChecklistMonthlyReviewAccess/getChecklistReportAccess/
 * getChecklistExportAccess/getChecklistRoleWorkspace): KHÔNG có nhánh admin-bypass
 * (a.role==='admin' -> thấy tất cả) và KHÔNG fallback active[0] như getChecklistRoleWorkspace
 * (dòng ~255 phía trên). Đây là quyết định nghiệp vụ Batch 2: "không mở Admin âm thầm chỉ vì
 * role cao" — Admin muốn xem hồ sơ người khác qua UX này (nếu có) phải có grant view_monthly
 * thật như mọi tài khoản khác, không có đường tắt. allEmployees (chưa lọc scope) được trả kèm
 * để nơi gọi tự phân biệt 404 "không tồn tại" với 403 "ngoài phạm vi" — KHÔNG được lộ trường
 * này ra API response, chỉ dùng nội bộ server-side.
 */
async function getChecklistMonthlyViewAccess(session){
  ensureDb();
  const a=actor(session),today=new Date().toISOString().slice(0,10);
  const assignmentsRequest=supabase.from('checklist_employee_assignments').select('employee_id,employee_code,employee_name,department,title,branch,manager_id,manager_code,manager_name,employee_status,template_id,template_version,effective_date').neq('employee_status','Đã nghỉ việc').order('employee_name',{ascending:true}).limit(1000);
  if(!a.id&&!a.employeeCode)return {identity:a,canView:false,grant:null,allEmployees:[],people:[]};
  const filters=[];if(a.id)filters.push('account_id.eq.'+a.id);if(a.employeeCode)filters.push('employee_code.eq.'+a.employeeCode);
  const grantRequest=supabase.from(TABLE).select('id,account_id,employee_code,employee_name,preset_code,capabilities,view_scope,review_scope,record_scope,export_scope,effective_from,effective_to,reason,is_active,updated_at,updated_by_name').eq('is_active',true).lte('effective_from',today).or('effective_to.is.null,effective_to.gte.'+today).or(filters.join(',')).order('effective_from',{ascending:false}).order('updated_at',{ascending:false}).limit(20);
  const [assignmentResult,grantResult]=await Promise.all([assignmentsRequest,grantRequest]);
  if(assignmentResult.error)throw assignmentResult.error;if(grantResult.error)throw grantResult.error;
  const allEmployees=assignmentResult.data||[];
  // STRICT: chỉ nhận grant có capabilities.view_monthly===true. Không OR với
  // view_reports/view_violations/record_violation, không fallback active[0].
  const viewGrant=currentGrantRows(grantResult.data||[]).find(row=>row.capabilities?.view_monthly===true)||null;
  if(!viewGrant)return {identity:a,canView:false,grant:null,allEmployees,people:[]};
  const people=allEmployees.filter(row=>subjectMatchesScope(row,viewGrant.view_scope,a));
  return {identity:a,canView:true,grant:publicGrant(viewGrant),allEmployees,people};
}
async function getChecklistRoleWorkspace(session){
  ...
  const viewViolationsGrant=active.find(row=>row.capabilities?.view_violations===true) || null;
  return {
    role:'manager',
    identity:a,
    ...
    canViewViolations:Boolean(viewViolationsGrant),
    recordScope:recordGrant?(recordGrant.record_scope?.type?recordGrant.record_scope:{type:'none',values:[]}):{type:'none',values:[]},
    // 2026-08-16 (Trợ lý thẩm định Đi trễ): surface đúng reviewGrant/review_scope đã TÍNH SẴN ở
    // trên (dòng 293, dùng để gắn people[].canReview) làm field top-level — mirror y hệt pattern
    // canRecordViolation/recordScope ngay phía trên. KHÔNG phải capability/scope mới — chỉ lộ ra
    // giá trị đã có sẵn để client gate UI "Đi trễ" theo đúng quyền thẩm định hiện hữu, không phải
    // suy từ preset_code/role string.
    canReviewMonthly:Boolean(reviewGrant),
    reviewScope:reviewGrant?(reviewGrant.review_scope?.type?reviewGrant.review_scope:{type:'none',values:[]}):{type:'none',values:[]},
