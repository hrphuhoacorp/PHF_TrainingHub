...
function dueWithinCalendarDays(dueMs, nowMs, days) {
...
// Fallback CHỈ dùng khi operationTimingPolicy() không đọc được cấu hình (mất kết
// nối/chưa cấu hình) - không phải số nghiệp vụ độc lập. Giá trị khớp đúng
// boundedDays() fallback=3 đã có sẵn trong lib/checklist-tasks.js.
const DUE_SOON_FALLBACK_DAYS = 3;

/**
 * Phân loại "Tình trạng xử lý" của một bản ghi lỗi - NGUỒN CHUẨN DUY NHẤT,
 * dùng chung cho Tổng quan, Báo cáo ("Tình trạng xử lý ghi nhận lỗi", xem
 * getChecklistViolationWorkflowSummary trong lib/checklist-reports.js) và
 * Nhật ký lỗi (danh sách/badge + filter workflowStatus ở listChecklistViolations
 * ngay dưới đây). Không định nghĩa lại bucket ở nơi khác - mọi nơi cần biết
 * "chờ nhân viên/đang giải trình/quá hạn/sắp quá hạn/việc đang mở" đều phải
 * gọi hàm này, không tự tính lại bằng công thức riêng.
 * - waitingAdmin: tập con của inExplanation (task.status = waiting_admin) -
 *   tách riêng chỉ để phục vụ KPI "Báo Admin", không phải bucket workflowStatus mới.
 * - dueSoon: task còn mở (chưa completed/cancelled), chưa overdue, còn tối đa
 *   N ngày dương lịch đến due_at ("Sắp quá hạn") - N lấy TRỰC TIẾP từ
 *   operationTimingPolicy() (lib/checklist-tasks.js): employeeResponseDays khi
 *   đang chờ nhân viên, reviewerResponseDays khi đang giải trình/chờ Admin -
 *   không hardcode một con số tách rời khỏi chính sách đang cấu hình. Người
 *   gọi truyền `timingPolicy` (kết quả operationTimingPolicy() đã await 1 lần
 *   cho cả batch, không gọi lại trong vòng lặp).
 * - open: bản ghi còn "việc đang mở" = waitingEmployee || inExplanation || overdue.
 */
function classifyViolationWorkflowStatus(recordStatus, task, now = Date.now(), timingPolicy = null) {
  if (recordStatus === 'cancelled') {
    return { key: 'cancelled', waitingEmployee: false, inExplanation: false, waitingAdmin: false, overdue: false, dueSoon: false, open: false };
  }
  const waitingEmployee = !!task && (task.status === 'waiting_employee' || task.status === 'waiting_employee_result');
  const inExplanation = !!task && (task.status === 'waiting_reviewer' || task.status === 'waiting_admin');
  const waitingAdmin = !!task && task.status === 'waiting_admin';
  const isOpenTask = !!task && task.status !== 'completed' && task.status !== 'cancelled';
  const due = task ? Date.parse(task.due_at || '') : NaN;
  const overdue = isOpenTask && !Number.isNaN(due) && due < now;
  const policyReady = timingPolicy && timingPolicy.ready === true;
  const dueSoonDays = waitingEmployee
    ? (policyReady ? timingPolicy.employeeResponseDays : DUE_SOON_FALLBACK_DAYS)
    : (policyReady ? timingPolicy.reviewerResponseDays : DUE_SOON_FALLBACK_DAYS);
  const dueSoon = isOpenTask && !overdue && dueWithinCalendarDays(due, now, dueSoonDays);
  const open = waitingEmployee || inExplanation || overdue;
  const key = overdue ? 'overdue' : waitingEmployee ? 'waiting_employee' : inExplanation ? 'in_explanation' : 'official';
  return { key, waitingEmployee, inExplanation, waitingAdmin, overdue, dueSoon, open };
}
...
function sameDayRepeatKey(row) {
  return t(row.employee_code).toUpperCase() + '|' + t(row.criterion_code).toUpperCase() + '|' + t(row.occurred_date);
}

/*
 * Batch D2A: dem so ban ghi CHINH THUC (record_status='official', is_test=false)
 * trung employee_code+criterion_code+occurred_date - CO CHU Y khac voi
 * duplicateFingerprint() (hep hon: gom ca occurred_time/location/note nen bo sot
 * 2 lan cung loi khac gio/ghi chu). Dung cho nhan "Cung loi N lan trong ngay",
 * khong lien quan diem/workflow.
 * 1 query rieng cho ca trang (khong N+1): loc theo 3 tap gia tri xuat hien
 * trong trang roi group that trong JS (Supabase JS khong ho tro GROUP BY
 * truc tiep) - loc theo tung cot rieng la sieu tap (khong phai tich chinh xac
 * tung bo 3) nen co the tra du du lieu ngoai nhom can, nhung khong lam sai so
 * dem vi chi dung de group dung tung bo 3 thuc su ton tai.
 * Neu ket qua tho cham LIMIT (nguy co bi cat) tra Map rong de goi KHONG gan
 * nhan sai cho trang do, khong am tham tinh thieu.
 */
async function repeatSameDayCounts(officialRows) {
  if (!officialRows.length) return new Map();
  const employeeCodes = [...new Set(officialRows.map(row => t(row.employee_code).toUpperCase()).filter(Boolean))];
  const criterionCodes = [...new Set(officialRows.map(row => t(row.criterion_code).toUpperCase()).filter(Boolean))];
  const occurredDates = [...new Set(officialRows.map(row => t(row.occurred_date)).filter(Boolean))];
  if (!employeeCodes.length || !criterionCodes.length || !occurredDates.length) return new Map();
  const limit = 5000;
  const { data, error } = await supabase
    .from(TABLE)
    .select('employee_code,criterion_code,occurred_date')
    .eq('is_test', false)
    .eq('record_status', 'official')
    .in('employee_code', employeeCodes)
    .in('criterion_code', criterionCodes)
    .in('occurred_date', occurredDates)
    .limit(limit);
  if (error) throw error;
  const rows = data || [];
  if (rows.length >= limit) return new Map();
  const counts = new Map();
  rows.forEach(row => {
    const key = sameDayRepeatKey(row);
...
// checklist_permission_grants lưu scope company-wide dưới type 'all_company'
// (xem lib/checklist-permissions.js PRESETS.TRO_LY_GD). Trước AR-05, hàm
// normalizeViolationScopeType() ở đây đổi 'all_company' -> 'all' nhưng không
// đổi 'employees' (số nhiều, giá trị chuẩn) -> 'employee' (số ít, giá trị mà
// assignmentMatchesPermission() cũ tự đặt ra) - hai matcher lệch tên type,
// gây Silent Denial. AR-05: bỏ hẳn lớp đổi tên/matcher riêng này; scopeType
// ở đây giữ NGUYÊN type chuẩn ('all_company','employees',...) và việc so
// khớp nhân sự dùng chung subjectMatchesScope() - xem lib/checklist-scope.js
// (SOURCE OF TRUTH duy nhất, cũng là hàm mà lib/checklist-permissions.js
// dùng cho view_monthly/review_monthly/view_reports/export_data).
...
async function resolveViolationPermission(session, action = 'view') {
  ...
  const { data: rows, error } = await supabase
    ...
    .select('account_id,employee_code,preset_code,capabilities,view_scope,record_scope,effective_from,updated_at')
    .eq('is_active', true)
  ...
  const scopeSource = action === 'record' ? grant.record_scope : grant.view_scope;
  const resolvedScope = scopeSource && scopeSource.type
    ? scopeSource
    : { type: 'none', values: [] };
  return {
    allowed: true,
    ...
    canEditTest: false,
    canCancel: false,
    scopeType: t(resolvedScope.type) || 'none',
    scopeValues: Array.isArray(resolvedScope.values) ? resolvedScope.values : [],
    // presetCode: nguồn để hiển thị "vai trò/chức danh người ghi nhận" bằng dữ liệu THẬT
    // của tài khoản (vd PRESETS[presetCode].name ở lib/checklist-permissions.js) — KHÔNG
    // dùng để so khớp/hardcode logic quyền ở bất kỳ đâu, chỉ để hiển thị.
    presetCode: t(grant.preset_code),
    actorId: currentActor.id,
    actorEmployeeCode: currentActor.employeeCode
  };
}
...
async function permissionAssignmentRows(permission) {
  if (!supabase || permission.scopeType === 'all_company') return [];
  const { data, error } = await supabase
...
// So khớp nhân sự với scope qua subjectMatchesScope() dùng chung (xem
// lib/checklist-scope.js) - không tự viết matcher riêng ở đây nữa.
function permissionScopeIdentity(permission) {
  return { id: permission.actorId, employeeCode: permission.actorEmployeeCode };
}

async function permissionEmployees(permission) {
  if (permission.scopeType === 'all_company') {
    const { data, error } = await supabase
      .from(ASSIGNMENT_TABLE)
      .select('employee_code,employee_name,department,branch,manager_id,manager_code,employee_status')
      .neq('employee_status', 'Đã nghỉ việc')
  ...
  const scopeValue = { type: permission.scopeType, values: permission.scopeValues || [] };
  const identity = permissionScopeIdentity(permission);
  return rows.filter(row => subjectMatchesScope(row, scopeValue, identity));
}
...
async function requireViolationPermission(session, action, rows = []) {
  ...
  if (rows.length && permission.scopeType !== 'all_company') {
...
async function normalizeCanonical(row, session, mode, index, resolver, options = {}) {
  ...
  /* rawRequestId = request_id CLIENT gui len (chua ghep batch prefix - xem
     bien `requestId` phia duoi, chi tinh xong sau khi qua het cac buoc kiem
     tra ben tren). Dung tam gia tri nay lam dau hieu doi chieu dong cho client
     ngay ca khi fail() no ra TRUOC khi `requestId` chinh thuc duoc tinh. */
  const rawRequestId = t(row.requestId || row.request_id);
  /* Batch UX-QMP: moi fail() trong ham nay gan them {index, requestId} vao
     details() de client (Ghi nhan nhanh nhieu nhan vien) biet CHINH XAC dong
     nao trong batch that bai - KHONG doi message/code/statusCode da co (cac
     test hien huu assert nguyen van message/code phai tiep tuc pass), chi
     THEM truong details qua tham so thu 4 cua fail() (da ho tro san). */
  const rowDetails = (extra) => Object.assign({ index, requestId: rawRequestId }, extra || {});

  if (!submittedEmployeeCode && !submittedEmployeeId) {
    fail('Thiếu định danh hoặc mã nhân sự.', 400, 'CHECKLIST_EMPLOYEE_REQUIRED', rowDetails());
  }
  if (!criterionCode || !note) {
    fail('Thiếu tiêu chí hoặc mô tả lỗi.', 400, 'CHECKLIST_VIOLATION_REQUIRED', rowDetails());
  }
  if (note.length < 3) {
    fail('Nhận xét phải mô tả rõ sự việc.', 400, 'CHECKLIST_NOTE_TOO_SHORT', rowDetails());
  }
  ...
  if (
    ...
    fail(
      ...
      rowDetails({ submittedEmployeeCode, assignedEmployeeCode: employeeCode })
    );
  }
  if (
    submittedEmployeeId &&
    ...
    fail(
      ...
      409,
      'CHECKLIST_EMPLOYEE_IDENTITY_MISMATCH',
      rowDetails({ submittedEmployeeId, assignedEmployeeId: employeeId })
    );
  }
  ...
  const lateStandardPoints = null;
  const lateAdjustmentReason = '';
  const lateAdjustedBy = '';
  const lateAdjustedByName = '';
  const lateAdjustedAt = null;
  const lateManagerDecision = null;
  ...
  /* P0-2 fix (2026-08-15) — Đi trễ (DITRE) ở Step 1 KHÔNG được tạo official violation/
     checklist score/scoring/adjustment nào cả — chỉ ghi "quan sát của người quản lý"
     (Duyệt/Không duyệt) vào bảng riêng checklist_late_manager_observations, hoàn toàn tách
     biệt khỏi checklist_violation_records. Route qua recordManagerLateObservation() (lib/
     checklist-late-reconciliation-service.js) từ saveChecklistViolations() — ở đây CHỈ validate
     managerDecision bắt buộc (giữ nguyên mã lỗi CHECKLIST_LATE_MANAGER_DECISION_REQUIRED) và trả
     về sớm một payload "late observation intent" (đánh dấu bằng __lateObservation) thay vì
     official-violation-payload đầy đủ — KHÔNG tính điểm chuẩn/điểm áp dụng/lý do điều chỉnh nào
     cho Đi trễ nữa (những trường đó không còn ý nghĩa vì không insert vào checklist_violation_records).
     lateMinutes/giờ vào ca thực tế: recordManagerLateObservation() hiện CHƯA nhận/lưu field này
     (bảng checklist_late_manager_observations không có cột phút trễ) — KHÔNG tự ý thêm cột/schema
     mới ở đây, giữ nguyên phạm vi Step 1. */
  if (isLateCriterion(criterion)) {
    const lateDecision = t(row.managerDecision || row.manager_decision).toLowerCase();
    if (lateDecision !== 'approved' && lateDecision !== 'rejected') {
      fail(
        'Vui lòng chọn Duyệt hoặc Không duyệt cho Đi trễ.',
        400,
  ...
  const payload = {
    ...
    manager_decision: lateManagerDecision,
    occurred_date: occurredDate,
...
async function saveChecklistViolations(session, rows) {
  ...
  /* P0-1/P0-2 fix (2026-08-15) — permission check chạy TRƯỚC cho TOÀN BỘ rows (cả Đi trễ lẫn
     tiêu chí thường) qua đúng cơ chế capability/scope thật requireViolationPermission(session,
     'record', rows) - rows chỉ cần field employee_code (cả 2 loại payload đều có) để so khớp
     record_scope. KHÔNG hardcode role - actor có capability record + record_scope phủ nhân sự
     (Trưởng ca, Trưởng bộ phận, Trợ lý GĐ, Admin...) đều ghi nhận được, kể cả Đi trễ. Dòng nào
     ngoài phạm vi sẽ chặn CẢ BATCH ở đây, trước khi đụng DB (giữ nguyên contract atomic cũ). */
  await requireViolationPermission(session, 'record', normalized);

  // Tách 2 luồng: Đi trễ (DITRE) route qua recordManagerLateObservation() -> bảng
  // checklist_late_manager_observations, KHÔNG bao giờ tạo official violation/điểm. Tiêu chí
  // thường giữ nguyên luồng insert vào checklist_violation_records như cũ.
  const lateEntries = normalized.filter(row => row.__lateObservation);
  const payload = normalized.filter(row => !row.__lateObservation);

  if (modeInfo.mode === 'test' && payload.length) {
  ...
  /* Kiem tra KY THUAT thuan tuy (so sanh chinh request_id, khong doan theo noi
     dung): hai dong trong CUNG mot lan goi khong duoc trung request_id - day
     luon la loi frontend (vi request_id sinh theo batch+index nen khac nhau
     tu nhien), khong phai 2 nghiep vu doc lap. Postgres upsert onConflict
     khong xu ly duoc truong hop 2 dong trung key trong CUNG mot cau lenh
     insert (se bao "ON CONFLICT DO NOTHING command cannot affect row a second
     time"), nen phai chan som, ro rang, truoc khi cham DB. */
  const requestIdCounts = new Map();
  normalized.forEach(row => {
    if (!row.request_id) return;
    requestIdCounts.set(row.request_id, (requestIdCounts.get(row.request_id) || 0) + 1);
  });
  const collidedRequestIds = [...requestIdCounts.entries()].filter(([, n]) => n > 1).map(([id]) => id);
  if (collidedRequestIds.length) {
    fail(
      'Batch gửi lên có request_id trùng nhau trong cùng lần gửi - đây là lỗi kỹ thuật phía gửi dữ liệu, vui lòng thử lại.',
      400,
      'CHECKLIST_BATCH_REQUEST_ID_COLLISION',
      { requestIds: collidedRequestIds }
    );
  }

  /* Batch D1: duplicate_fingerprint (xem duplicateFingerprint()) van duoc tinh
     va luu tren moi row (trong normalizeCanonical) de lam du lieu ho tro
     audit/nhan "Cung loi N lan trong ngay" sau nay, nhung KHONG con duoc dung
     de quyet dinh co insert/tru diem/gui workflow hay khong - hai ghi nhan
     nghiep vu doc lap trung noi dung (Giam sat + Truong ca cung ghi, hoac
     sang/chieu cung loi thuc) phai duoc tao thanh 2 record rieng. Idempotency
     ky thuat DUY NHAT dua vao unique index tren request_id (upsert onConflict
     duoi day) - do la khoa on dinh client giu nguyen trong 1 lan submit, khac
     ban chat voi noi dung ban ghi.
     Migration di kem: scripts/PHF_CHECKLIST_VIOLATION_DUPLICATE_UNBLOCK_1.42.0.sql
     (drop uq_checklist_violation_active_fingerprint tren Production - CHUA chay). */
  const clean = payload.map(persistenceRow);

  let violationSavedRows = [];
  if (clean.length) {
    ...
    const newlyInsertedRequestIds = new Set((insertedData || []).map(row => row.request_id).filter(Boolean));

    let saved = insertedData || [];
    ...
    violationSavedRows = saved.map(row => {
      ...
      return {
        ...
        /* isNew=false nghia la request_id nay da ton tai TU TRUOC lan goi nay
           (retry idempotent that) - dung field nay de quyet dinh co ban notification/
           side-effect mot lan nua hay khong (xem api/data.js, server.js), KHONG
           dung payload.violations goc vi request retry van gui du input nhu cu. */
        isNew: newlyInsertedRequestIds.has(row.request_id),
  ...
  /* P0-2 — Đi trễ: gọi recordManagerLateObservation() (lib/checklist-late-reconciliation-service.js)
     cho từng dòng DITRE, ghi vào checklist_late_manager_observations, KHÔNG đụng
     checklist_violation_records. Require LAZY (trong hàm, không phải top-level của file) vì
     checklist-late-reconciliation-service.js đã require ngược lại lib/checklist-violations.js
     (requireViolationPermission/permissionEmployees/cancelChecklistViolation) — require ở top-level
     sẽ tạo circular dependency lúc module init (module.exports rỗng lúc load). */
  let lateObservationSavedRows = [];
  if (lateEntries.length) {
    const { recordManagerLateObservation } = require('./checklist-late-reconciliation-service');
    lateObservationSavedRows = await Promise.all(
      lateEntries.map(async entry => {
        const result = await recordManagerLateObservation(session, {
          employeeCode: entry.employee_code,
          occurredDate: entry.occurred_date,
          managerDecision: entry.manager_decision,
          note: entry.note,
        ...
        return {
          ...
          index: entry._index,
          requestId: (record && record.request_id) || entry.request_id || '',
          duplicateFingerprint: '',
          isNew: true,
          employeeCode: entry.employee_code || '',
          criterionCode: entry.criterion_code || '',
          occurredDate: entry.occurred_date || '',
          occurredTime: '',
          isLateObservation: true
        };
      })
    );
  }

  const savedRows = [...violationSavedRows, ...lateObservationSavedRows]
    .sort((a, b) => (Number.isInteger(a.index) ? a.index : 0) - (Number.isInteger(b.index) ? b.index : 0));
...
async function listChecklistViolations(session, input = {}) {
  ...
  const requestIds = Array.isArray(input.requestIds)
    ...
    : [];
  const publicEmployees = () => employees.map(row => ({
  ...
  if (permission.scopeType !== 'all_company' && !allowedCodes.length) {
    return { ...emptyResult(), employees: [] };
  }

  const search = t(input.query).replace(/[,()%]/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 100);

  /*
   * "Tình trạng xử lý" trên Nhật ký lỗi - dùng lại đúng bucket đã chốt ở
   * Dashboard "Tình trạng xử lý ghi nhận lỗi" (classifyViolationWorkflowStatus,
   * cùng nguồn với getChecklistViolationWorkflowSummary và với Tổng quan).
   * waiting_employee/in_explanation/waiting_admin/overdue/due_soon/open chỉ
   * thuộc bản ghi official, is_test=false nên cần một lượt lấy id khớp task
   * trước khi lọc trang chính - đúng mẫu truy vấn 2 bước đã dùng ở
   * getChecklistViolationWorkflowSummary (giới hạn 5000). waiting_admin/
   * due_soon/open phục vụ drill-down đúng số cho các KPI cùng tên ở Tổng quan
   * ("Báo Admin", "Sắp quá hạn", "Việc đang mở") - không phải bucket
   * workflowStatus hiển thị ở badge chính (badge chỉ dùng classification.key).
   */
  const workflowStatus = ['waiting_employee', 'in_explanation', 'waiting_admin', 'overdue', 'due_soon', 'open', 'official', 'cancelled'].includes(t(input.workflowStatus))
    ? t(input.workflowStatus)
    : '';
  let workflowMatchedIds = null;
  if (['waiting_employee', 'in_explanation', 'waiting_admin', 'overdue', 'due_soon', 'open'].includes(workflowStatus)) {
    let idQuery = supabase
      ...
      .eq('record_status', 'official')
      .limit(5000);
    if (permission.scopeType !== 'all_company') idQuery = idQuery.in('employee_code', allowedCodes);
    if (t(input.employeeCode)) idQuery = idQuery.eq('employee_code', t(input.employeeCode).toUpperCase());
    if (t(input.dateFrom)) idQuery = idQuery.gte('occurred_date', date(input.dateFrom));
    if (t(input.dateTo)) idQuery = idQuery.lte('occurred_date', date(input.dateTo));
    if (search) {
    ...
    if (candidateIds.length) {
      const [{ data: taskRows, error: taskError }, timingPolicy] = await Promise.all([
        supabase
          .from('checklist_violation_tasks')
          .select('violation_id,status,due_at')
          .in('violation_id', candidateIds)
          .limit(5000),
        workflowStatus === 'due_soon' ? operationTimingPolicy() : Promise.resolve(null)
      ]);
      if (taskError) throw taskError;
      const taskByViolation = new Map((taskRows || []).map(row => [row.violation_id, row]));
      matchedIds = candidateIds.filter(id => {
        const classification = classifyViolationWorkflowStatus('official', taskByViolation.get(id) || null, Date.now(), timingPolicy);
        if (workflowStatus === 'waiting_employee') return classification.waitingEmployee;
        if (workflowStatus === 'in_explanation') return classification.inExplanation;
        if (workflowStatus === 'waiting_admin') return classification.waitingAdmin;
        if (workflowStatus === 'due_soon') return classification.dueSoon;
        if (workflowStatus === 'open') return classification.open;
        return classification.overdue;
      });
    }
    if (!matchedIds.length) return emptyResult();
    workflowMatchedIds = matchedIds;
  }
  ...
  if (permission.scopeType !== 'all_company') query = query.in('employee_code', allowedCodes);
  ...
  if (workflowStatus === 'official') query = query.eq('record_status', 'official');
  if (workflowStatus === 'cancelled') query = query.eq('record_status', 'cancelled');
  if (workflowStatus) query = query.eq('is_test', false);
  if (workflowMatchedIds) query = query.in('id', workflowMatchedIds);
  ...
  const officialRows = records.filter(row => row.is_test !== true && row.record_status === 'official');
  const officialRowIds = officialRows.map(row => row.id);
  let rowTaskByViolation = new Map();
  if (officialRowIds.length) {
    const { data: rowTaskRows, error: rowTaskError } = await supabase
      .from('checklist_violation_tasks')
      .select('violation_id,status,due_at')
      .in('violation_id', officialRowIds)
      .limit(200);
    if (rowTaskError) throw rowTaskError;
    rowTaskByViolation = new Map((rowTaskRows || []).map(row => [row.violation_id, row]));
  }
  const repeatSameDayCountByKey = await repeatSameDayCounts(officialRows);
  const recordsWithWorkflow = records.map(row => {
    const isOfficial = row.is_test !== true && row.record_status === 'official';
    return {
      ...row,
      workflowStatus: row.is_test === true
        ? null
        : classifyViolationWorkflowStatus(row.record_status, rowTaskByViolation.get(row.id) || null).key,
      repeatSameDayCount: isOfficial ? (repeatSameDayCountByKey.get(sameDayRepeatKey(row)) || 1) : null
    };
  });

  return {
    records: recordsWithWorkflow,
...
/**
 * Trạng thái xử lý (task) của một bản ghi lỗi, cho modal "Chi tiết bản ghi"
 * trong Nhật ký lỗi - dùng lại đúng quyền 'view' của chính bản ghi (không có
 * logic quyền riêng, không suy quyền từ personal worklist của
 * lib/checklist-tasks.js). Trả về null nếu bản ghi chưa từng có task (nháp/
 * hủy trước khi tạo task, hoặc bản ghi cũ có trước khi có hệ thống task).
 */
async function getChecklistViolationTaskStatus(session, input = {}) {
  if (!supabase) fail('Supabase chưa được cấu hình.', 503, 'SUPABASE_NOT_CONFIGURED');
  const id = t(input.id);
  if (!id) fail('Thiếu mã bản ghi.', 400, 'CHECKLIST_VIOLATION_ID_REQUIRED');
  const record = await getRecord(id);
  await requireViolationPermission(session, 'view', [record]);
  const { data, error } = await supabase
    .from('checklist_violation_tasks')
    .select('id,status,due_at,completed_at,current_assignee_type,updated_at')
    .eq('violation_id', id)
    .order('created_at', { ascending: false })
    .limit(1)
    .maybeSingle();
...
module.exports = {
  ...
  getChecklistViolationTaskStatus,
