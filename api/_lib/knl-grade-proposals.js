...
const { getEmployeeCodesByAccountIds } = require('./auth');
const { resolveCompetencyStandardForCompensationGrade } = require('./knl-competency');
const { emitKnlNotificationSafe } = require('./knl-notifications');
...
// ---------------------------------------------------------------------------
// Đánh giá theo tiêu chí (Assessment V1, Phase 2) — bridge sang competency
// grade qua resolveCompetencyStandardForGradeCode() (lib/knl-competency.js).
// Compensation grade và competency grade là 2 hệ khác nhau (business decision
// đã chốt) — proposed compensation grade_code CHỈ là khóa mapping có kiểm
// chứng. Không resolve được mapping / grade không có requirements -> BLOCK
// tạo/gửi proposal ở đây, KHÔNG cho proposal rỗng lọt xuống DB.
// ---------------------------------------------------------------------------
const CRITERIA_MAPPING_ERROR_MESSAGES = {
...
function requireCriteriaMapping(mapping) {
  if (mapping.ok) return mapping;
  fail(CRITERIA_MAPPING_ERROR_MESSAGES[mapping.reason] || 'Không thể xác định tiêu chí năng lực cho bậc đề xuất.', 409, 'KNL_PROPOSAL_CRITERIA_' + String(mapping.reason || 'UNKNOWN').toUpperCase());
}

/* validateAssessment — đòi hỏi đánh giá ĐỦ VÀ ĐÚNG tất cả tiêu chí của
 * standard (không thiếu, không lạ) — người khởi tạo đánh giá gọn Đạt/Chưa đạt
 * + ghi chú bắt buộc khi Chưa đạt (business decision Assessment V1). Không
 * đọc/ghi bất kỳ field tiền lương nào — chỉ itemId/result/note. */
function validateAssessment(standardGroups, rawAssessment) {
  const allItemIds = new Set();
  (standardGroups || []).forEach(g => (g.items || []).forEach(it => allItemIds.add(it.id)));
  if (!allItemIds.size) fail('Bậc đề xuất chưa có tiêu chí năng lực nào được thiết lập.', 409, 'KNL_PROPOSAL_CRITERIA_NO_REQUIREMENTS');

  const list = Array.isArray(rawAssessment) ? rawAssessment : [];
  const byId = new Map();
  list.forEach(a => { const id = text(a && a.itemId); if (id) byId.set(id, a); });
  const missing = [...allItemIds].some(id => !byId.has(id));
  if (missing || byId.size !== allItemIds.size) fail('Vui lòng đánh giá đầy đủ tất cả tiêu chí của bậc đề xuất trước khi gửi.', 400, 'KNL_PROPOSAL_CRITERIA_INCOMPLETE');

  const resultByItemId = new Map();
  for (const id of allItemIds) {
    const a = byId.get(id);
    const result = text(a.result).toLowerCase();
    if (result !== 'met' && result !== 'not_met') fail('Kết quả đánh giá tiêu chí không hợp lệ (chỉ chấp nhận Đạt/Chưa đạt).', 400, 'KNL_PROPOSAL_CRITERIA_RESULT_INVALID');
    const note = text(a.note);
    if (result === 'not_met' && note.length < 3) fail('Tiêu chí "Chưa đạt" cần ghi chú/minh chứng (tối thiểu 3 ký tự).', 400, 'KNL_PROPOSAL_CRITERIA_NOTE_REQUIRED');
    resultByItemId.set(id, { result, note });
  }
  return resultByItemId;
}

function buildCriteriaSnapshot(mapping, resultByItemId, actorMeta) {
  const groups = mapping.standard.groups.map(g => ({
    id: g.id, name: g.name, sortOrder: g.sortOrder,
    items: g.items.map(it => {
      const r = resultByItemId.get(it.id);
      return {
        id: it.id, name: it.name, sortOrder: it.sortOrder,
        requiredLevelNumber: it.requiredLevelNumber, requiredColumnLabel: it.requiredColumnLabel, content: it.content,
        result: r.result, note: r.note
      };
    })
  }));
  return {
    frameworkVersionId: mapping.frameworkVersionId, competencyGradeId: mapping.competencyGradeId, gradeCode: mapping.gradeCode,
    framework: mapping.framework, groups,
    assessedAt: new Date().toISOString(), assessedById: actorMeta.id || null, assessedByName: actorMeta.name || null
  };
}

/* getGradePromotionCriteriaStandard — THUẦN READ, dùng cho UI hiển thị bước
 * "Đánh giá theo tiêu chí" trước khi submit (mirror getGradeOptionsForSubject
 * — tách khỏi createGradePromotionProposal để frontend load trước, backend
 * vẫn tự re-resolve + re-validate độc lập lúc tạo thật, hàm này không phải
 * nguồn duy nhất quyết định). requirePropose() giống các hàm hỗ trợ tạo khác
 * (getGradeOptionsForSubject/getGradePromotionApproverOptions). */
async function getGradePromotionCriteriaStandard(session, input = {}) {
  ensureDb();
  ...
  if (!proposedGradeId) fail('Thiếu bậc đề xuất.', 400, 'KNL_PROPOSAL_GRADE_REQUIRED');

  // "mapped" (KHÔNG dùng tên "ok"): tránh trùng nghĩa với field ok cấp
  // HTTP-response (api/data.js bọc {ok:true,...}) — mapped=false vẫn là một
  // response THÀNH CÔNG (200), chỉ mang nghĩa "chưa resolve được mapping",
  // không phải lỗi request.
  const mapping = await resolveCompetencyStandardForCompensationGrade(subjectEmployeeCode, proposedGradeId);
  if (!mapping.ok) return { mapped: false, reason: mapping.reason };
  return { mapped: true, framework: mapping.framework, gradeCode: mapping.gradeCode, groups: mapping.standard.groups };
}
...
// ---------------------------------------------------------------------------
// KNL NOTIFICATION (Phase N1) — side-effect SAU KHI transition thành công,
// KHÔNG BAO GIỜ throw ra ngoài (emitKnlNotificationSafe tự nuốt lỗi) và
// KHÔNG BAO GIỜ đổi response proposal. "Notification follows permission +
// workflow scope" (business rule đã chốt): recipient LUÔN resolve lại từ
// freshChain/grantsByCode sống — KHÔNG suy diễn thêm quyền nào ngoài đúng
// những gì resolveApprovalChain()/requireApprove() đã công nhận. TUYỆT ĐỐI
// không đưa field tiền lương vào title/message (Phase N1 chủ động không đưa
// income vào bất kỳ notification nào).
// ---------------------------------------------------------------------------
const GP_NOTIFY_TARGET_PATH = '/knl/de-xuat-nang-bac';

/* CORRECTION (audit): "Notification follows permission + workflow scope" —
 * KHÔNG tự union thêm toàn bộ account role==='admin' (đó là tự tạo authority
 * từ role, không phải từ workflow route). Nguồn duy nhất cho "ai là current
 * actor của tier cuối" là ĐÚNG grantsByCode — CÙNG 1 map đã dùng để dựng
 * toàn bộ chain ở resolveApprovalChain()/subjectIsApproverRole() phía trên,
 * không tạo thêm 1 nguồn dữ liệu song song (listHubAccountSummaries) có thể
 * lệch pha với workflow thật. Nếu 1 Admin (role='admin') CŨNG có 1 dòng
 * grant active với capabilities.approve===true thì họ nằm trong đúng
 * grantsByCode này và vẫn nhận — vì workflow route tới họ (qua grant), không
 * phải vì role Admin. Admin không có grant approve=true vẫn xử lý được tier
 * cuối bình thường qua admin_recovery (requireApprove() không đổi) — chỉ là
 * không được PING chủ động, đúng yêu cầu "không suy diễn quyền notify từ
 * role". */
async function resolveFinalApproverRecipients(grantsByCode) {
  const recipients = [];
  grantsByCode.forEach((grant, empCode) => {
    if (grant && grant.capabilities && grant.capabilities.approve === true && empCode) {
      recipients.push({ employeeCode: empCode });
    }
  });
  return recipients;
}

/* Recipient cho kết quả cuối (approved/rejected): người tạo + nhân sự được đề
 * xuất — creator resolve qua employeeCode CHUẨN hiện hành (không dùng
 * actor_employee_code có thể là internal id cũ), để emitKnlNotification()
 * tự dedupe đúng 1 dòng khi creator===subject (mục 8/4 batch). */
async function resolveOutcomeRecipients(proposal) {
  const creatorCodeByAccountId = proposal.created_by ? await getEmployeeCodesByAccountIds([proposal.created_by]) : {};
  const creatorCode = code(creatorCodeByAccountId[proposal.created_by] || '');
  const recipients = [];
  if (creatorCode || proposal.created_by) recipients.push({ accountId: proposal.created_by || undefined, employeeCode: creatorCode || undefined });
  recipients.push({ employeeCode: code(proposal.subject_employee_code) });
  return recipients;
}

async function notifyGradeProposalActionRequired(proposal, expected, grantsByCode, excludeEmployeeCode) {
  if (!expected) return;
  let recipients;
  if (expected.tier === 'final') recipients = await resolveFinalApproverRecipients(grantsByCode);
  else if (code(expected.employeeCode) && code(expected.employeeCode) !== code(excludeEmployeeCode)) recipients = [{ employeeCode: code(expected.employeeCode) }];
  else return;
  if (!recipients || !recipients.length) return;
  await emitKnlNotificationSafe('GRADE_PROPOSAL_ACTION_REQUIRED', {
    recipients, proposalId: proposal.id,
    title: 'Có đề xuất nâng bậc cần bạn xử lý',
    message: 'Có đề xuất nâng bậc cần bạn xử lý (' + text(proposal.subject_employee_name) + ').',
    targetPath: GP_NOTIFY_TARGET_PATH, priority: 'Cao',
    dedupeKey: 'GRADE_PROPOSAL_ACTION_REQUIRED|' + proposal.id + '|' + text(proposal.current_step_index)
  });
}

async function notifyGradeProposalOutcome(eventCode, proposal, title, message) {
  const recipients = await resolveOutcomeRecipients(proposal);
  await emitKnlNotificationSafe(eventCode, {
    recipients, proposalId: proposal.id, title, message,
    targetPath: GP_NOTIFY_TARGET_PATH, priority: 'Trung bình',
    dedupeKey: eventCode + '|' + proposal.id
  });
}
...
async function createGradePromotionProposal(session, input = {}) {
  ...
  // Đánh giá theo tiêu chí (Phase 2, bắt buộc) — bridge sang competency grade
  // qua mapping TƯỜNG MINH (knl_compensation_competency_grade_map), KHÔNG so
  // grade_code string (2 hệ độc lập, xem lib/knl-competency.js). BLOCK nếu
  // không resolve được mapping hoặc thiếu/sai đánh giá (không cho proposal
  // rỗng lọt xuống DB).
  const criteriaMapping = requireCriteriaMapping(await resolveCompetencyStandardForCompensationGrade(subjectEmployeeCode, proposedGrade.id));
  const criteriaResultByItemId = validateAssessment(criteriaMapping.standard.groups, input.assessment);
  const criteriaSnapshot = buildCriteriaSnapshot(criteriaMapping, criteriaResultByItemId, a);
  ...
  const created = rpcResult.data;

  // Notification (Phase N1): re-resolve đúng actor bước kế tiếp SAU khi ghi
  // thành công — KHÔNG dùng chain đã tính ở trên trực tiếp (chain là dự kiến
  // lúc tạo), dùng created.current_step_index (state thật vừa ghi) để lấy
  // đúng vị trí trong `chain`. Không gửi cho chính người vừa tạo nếu họ lại
  // resolve thành actor kế tiếp (auto-advance ở trên đã lo hầu hết trường hợp
  // này, giữ excludeEmployeeCode ở đây cho chắc).
  try { await notifyGradeProposalActionRequired(created, chain[created.current_step_index], grantsByCode, a.employeeCode); }
  catch (_e) { /* an toàn tuyệt đối: notification không bao giờ làm hỏng response tạo proposal đã thành công */ }

  return { proposal: publicProposal(created) };
}
...
async function processGradePromotionProposalStep(session, input = {}) {
  ...
  if (action === 'reject') {
    ...
    throwDb(rpcResult.error);
    const rejected = rpcResult.data;
    try { await notifyGradeProposalOutcome('GRADE_PROPOSAL_REJECTED', rejected, 'Đề xuất nâng bậc đã bị từ chối', 'Đề xuất nâng bậc cho ' + text(rejected.subject_employee_name) + ' đã bị từ chối bởi ' + text(a.name || 'người xử lý') + '.'); }
    catch (_e) { /* an toàn tuyệt đối: notification không bao giờ làm hỏng response reject đã thành công */ }
    return { proposal: publicProposal(rejected) };
  }
  ...
  const rpcResult = await db.rpc('knl_grade_promotion_transition', { p_proposal_id: proposal.id, p_expected_status: 'pending', p_expected_step_index: idx, p_patch: patch, p_steps: steps });
  throwDb(rpcResult.error);
  const updated = rpcResult.data;
  try {
    if (isFinalTier) {
      await notifyGradeProposalOutcome('GRADE_PROPOSAL_APPROVED', updated, 'Đề xuất nâng bậc đã được duyệt', 'Đề xuất nâng bậc cho ' + text(updated.subject_employee_name) + ' đã được duyệt.');
    } else {
      await notifyGradeProposalActionRequired(updated, freshChain[updated.current_step_index], grantsByCode, a.employeeCode);
    }
  } catch (_e) { /* an toàn tuyệt đối: notification không bao giờ làm hỏng response agree/approve đã thành công */ }
...
async function withdrawGradePromotionProposal(session, input = {}) {
  ...
  throwDb(rpcResult.error);
  const withdrawn = rpcResult.data;

  // Notification: những actor đã TRỰC TIẾP tham gia proposal (có mặt trong
  // audit trail knl_grade_promotion_proposal_steps.actor_id) — đọc lại toàn
  // bộ steps (kể cả step withdraw vừa ghi) để lấy danh sách, resolve
  // employeeCode CHUẨN hiện hành qua getEmployeeCodesByAccountIds() (cùng
  // pattern presentation identity đã dùng ở getGradePromotionProposalDetail),
  // KHÔNG dùng actor_employee_code persist (có thể là internal id cũ).
  try {
    const allStepsResult = await db.from(STEPS_TABLE).select('actor_id').eq('proposal_id', proposal.id);
    throwDb(allStepsResult.error);
    const actorIds = [...new Set((allStepsResult.data || []).map(s => s.actor_id).filter(Boolean))];
    const employeeCodeByAccountId = actorIds.length ? await getEmployeeCodesByAccountIds(actorIds) : {};
    const recipients = actorIds.map(id => ({ accountId: id, employeeCode: code(employeeCodeByAccountId[id] || '') || undefined }));
    if (recipients.length) {
      await emitKnlNotificationSafe('GRADE_PROPOSAL_WITHDRAWN', {
        recipients, proposalId: withdrawn.id,
        title: 'Đề xuất nâng bậc đã bị rút',
        message: 'Đề xuất nâng bậc cho ' + text(withdrawn.subject_employee_name) + ' đã bị người tạo rút lại.',
        targetPath: GP_NOTIFY_TARGET_PATH, priority: 'Trung bình',
        dedupeKey: 'GRADE_PROPOSAL_WITHDRAWN|' + withdrawn.id
      });
    }
  } catch (_e) { /* an toàn tuyệt đối: notification không bao giờ làm hỏng response withdraw đã thành công */ }

  return { proposal: publicProposal(withdrawn) };
}
...
/* resolvedEmployeeCode (param 2, optional): mã CHUẨN đọc lại từ
 * user_accounts.employee_code theo actor_id — ƯU TIÊN tuyệt đối so với
 * row.actor_employee_code đã persist (có thể là internal id cũ ghi TRƯỚC
 * identity fix 9bc4bae, xem getGradePromotionProposalDetail()). Không resolve
 * được thì để rỗng — KHÔNG fallback về giá trị persist cũ (đúng rule "không
 * hiện internal ID nếu không resolve được mã chuẩn"). audit gốc
 * (actor_employee_code) vẫn nguyên vẹn trong DB, hàm này chỉ đổi PRESENTATION. */
function publicStep(row, resolvedEmployeeCode) {
  return {
    stepIndex: row.step_index, actorEmployeeCode: resolvedEmployeeCode || '', actorName: row.actor_name || '',
...
/* Bulk-đọc employeeCode CHUẨN (PHFxxx) của người tạo qua user_accounts
 * (created_by trên proposal LUÔN là account id) + title của subject/creator
 * từ Organization Master — THUẦN presentation cho "Danh sách" (mục 3 batch
 * giám sát Admin), KHÔNG đổi permission/scope filter, KHÔNG đổi field nào
 * publicProposal() đã có.
 *
 * TRACE 2026-08-12 (batch visual cleanup): TRƯỚC đây lấy creator code từ
 * knl_grade_promotion_proposal_steps (step_index 0, actor_employee_code) —
 * đúng với proposal tạo SAU identity fix (9bc4bae), nhưng proposal tạo TRƯỚC
 * fix đó có actor_employee_code là internal id cũ (vd "HV-xxxx", ghi tại
 * thời điểm actor() còn bug) — hiện thẳng ra "Danh sách" là đúng nguyên nhân
 * PHF report ("Người đề xuất: Lê Vĩnh Thắng / HV-0934510194"). Đổi sang đọc
 * TRỰC TIẾP user_accounts.employee_code (qua getEmployeeCodesByAccountIds(),
 * lib/auth.js — nguồn duy nhất được phép đọc user_accounts) theo created_by
 * — luôn là mã CHUẨN, HIỆN HÀNH của account, không phụ thuộc thời điểm
 * proposal được tạo, không cần sửa/mutate audit step cũ nào. Nếu account
 * không có employee_code (hiếm/dữ liệu thiếu): để trống, KHÔNG fallback về
 * bất kỳ internal id nào (đúng rule "không hiện internal ID nếu không
 * resolve được mã chuẩn"). */
async function attachListPresentation(rows, orgByCode) {
  if (!rows.length) return [];
  const creatorCodeByAccountId = await getEmployeeCodesByAccountIds(rows.map(r => r.created_by));
  return rows.map(row => {
    const p = publicProposal(row);
    const creatorCode = code(creatorCodeByAccountId[row.created_by] || '');
    const subjectOrgRow = orgByCode.get(code(row.subject_employee_code));
    const creatorOrgRow = creatorCode ? orgByCode.get(creatorCode) : null;
    p.createdByEmployeeCode = creatorCode;
    p.subjectTitle = subjectOrgRow ? (subjectOrgRow.title || '') : '';
    p.createdByTitle = creatorOrgRow ? (creatorOrgRow.title || '') : '';
    return p;
  });
}
...
async function listVisibleGradePromotionProposals(session, filters = {}) {
  ...
  // Admin (đường cứu hộ) monitoring toàn công ty — READ đã có sẵn từ trước
  // (không phải thay đổi permission ở batch này), chỉ scope filter khác nhau
  // cho non-admin dưới đây (giữ nguyên logic cũ).
  const rows = resolved.source === 'admin_recovery' ? allRows : allRows.filter(row => {
  ...
  return { proposals: await attachListPresentation(rows, orgByCode) };
}
...
async function getGradePromotionProposalDetail(session, input = {}) {
  ...
  // steps load DI CHUYỂN lên trước permission gate (trước đây load sau) —
  // cần dữ liệu này để chấm "historical actor" bên dưới, không chỉ để trả về.
  ...
  /* Permission gap thật (phát hiện qua DEV rehearsal end-to-end trên
   * PHF-HR-DEV, 2026-08-19): người đã TRỰC TIẾP agree/reject/approve 1
   * proposal mất quyền xem lại chính proposal đó ngay khi workflow kết thúc
   * (isMyTurn tự về false vì current_step_index đã vượt chain), rơi xuống
   * yêu cầu view_proposals — một capability RỘNG hơn hẳn những gì họ thực sự
   * cần (chỉ xem lại 1 proposal họ đã tự tay xử lý). Business quyết định:
   * "đã hành động thật trên proposal này" (có mặt trong audit trail
   * knl_grade_promotion_proposal_steps.actor_id) => LUÔN được xem lại CHÍNH
   * proposal đó — vĩnh viễn, không phụ thuộc còn tới lượt hay không. ĐÂY LÀ
   * QUYỀN XEM 1 PROPOSAL CỤ THỂ, KHÔNG PHẢI proposalScope (không mở rộng
   * xem list/proposal khác), KHÔNG cấp thêm quyền sửa (agree/approve/reject
   * vẫn qua đúng workflow guard hiện có — proposal đã final thì mọi
   * processGradePromotionProposalStep() đều tự chặn KNL_PROPOSAL_ALREADY_FINAL
   * bất kể ai gọi), KHÔNG đổi capabilities.agree_proposal/approve. */
  const wasHistoricalActor = Boolean(a.id) && steps.some(s => s.actor_id === a.id);
  ...
  // Presentation identity (batch visual cleanup 2026-08-12): resolve mã CHUẨN
  // (PHFxxx) của creator + của TỪNG actor trong Lịch sử xử lý qua
  // user_accounts.employee_code (actor_id/created_by) — không dùng
  // actor_employee_code đã persist (có thể là internal id cũ ghi trước
  // identity fix 9bc4bae). Không mutate DB, chỉ đổi field trả về.
  const accountIds = [proposal.created_by, ...steps.map(s => s.actor_id)];
  const employeeCodeByAccountId = await getEmployeeCodesByAccountIds(accountIds);
  const publicProposalObj = publicProposal(proposal);
  publicProposalObj.createdByEmployeeCode = code(employeeCodeByAccountId[proposal.created_by] || '');

  return {
    proposal: publicProposalObj,
    steps: steps.map(s => publicStep(s, employeeCodeByAccountId[s.actor_id])),
    liveChain: chain, isMyTurn,
    // criteriaSnapshot: TRẢ RIÊNG ở detail (không phải publicProposal() dùng
    // chung cho list) — tránh phình payload các danh sách (mine/awaiting/
    // visible) vốn không cần nội dung tiêu chí. Đọc THẲNG từ DB row, không
    // qua bất kỳ hàm nào có thể ghi đè — đúng đặc tính "immutable sau khi
    // submit" (xem comment cột criteria_snapshot, 1.62.0 SQL).
    criteriaSnapshot: proposal.criteria_snapshot && Object.keys(proposal.criteria_snapshot).length ? proposal.criteria_snapshot : null
  };
}
