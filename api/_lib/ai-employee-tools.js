...
async function searchEmployees(session, args) {
  ...
  // titleSource/conflict: xem ghi chu ROLE_PRESET_HINTS trong org-directory.js.
  // 'permission_grant_preset' nghia la KHONG tim thay theo truong title, ket
  // qua duoi day lay tu quyen Checklist tuong ung lam TIN HIEU BO SUNG - noi
  // goi (lib/ai-tool-registry.js) PHAI gan evidence.note noi ro, khong duoc
  // coi day la truong chuc danh chinh thuc.
  return {
    asOf: result.asOf,
    total: result.total,
    employees: (result.people || []).map(publicEmployee),
    titleSource: result.titleSource || 'title_field',
    conflict: result.conflict ? {
      presetCode: result.conflict.presetCode,
      titleEmployees: (result.conflict.titlePeople || []).map(publicEmployee),
      presetEmployees: (result.conflict.presetPeople || []).map(publicEmployee)
    } : null
  };
}
