...
/* PHF Organization Directory - lop doc CANONICAL cho cau hoi co cau to
   ...
   Nguon du lieu: employee_profiles (Organization Master, tu Organization
   Master Cutover 1.50.7) - CUNG nguon KNL dang dung lam "CURRENT
   ORGANIZATION" (xem lib/knl-people.js; truoc 1.50.7 la checklist_employee_
   assignments). Day la duong doc THU BA, doc lap - KHONG dong den
...
async function loadDirectoryRows() {
  ...
  const { data, error } = await supabase.from(SOURCE_TABLE)
    .select('employee_id,employee_code,full_name,department,title,branch,manager_employee_code,employment_status')
    .neq('employment_status', 'inactive')
    .order('full_name', { ascending: true })
    .limit(2000);
  if (error) throw error;
  const all = data || [];
  const byCode = new Map(all.map(r => [text(r.employee_code).toUpperCase(), r]));
  cachedRows = all.map(r => {
    ...
    return {
      ...
      employee_status: r.employment_status === 'inactive' ? INACTIVE_STATUS : 'Đang làm việc'
    };
  });
  cachedAt = now;
  return cachedRows;
