...
/* Notification "Xem lỗi" -> modal chi tiết (batch violation-detail-modal).
   Nguồn permission KHÔNG dùng lại checklist_permission_grants (view_violations
   - đó là quyền DUYỆT NHIỀU bản ghi cho Nhật ký lỗi, dùng ở nơi khác) mà dùng
   ĐÚNG quan hệ actor<->record đã có sẵn cho getChecklistTaskHistory() ở trên
   (isSubject/isCreator/sameActor) - vì notification chỉ bao giờ gửi tới đúng
   nhân viên bị ghi lỗi hoặc người đang phải xử lý (current_assignee), không
   phải một người xem rộng ngẫu nhiên. Nhân viên luôn xem được lỗi của chính
   mình dù không có bất kỳ Checklist permission grant nào - cùng nguyên tắc
   "quyền nền cá nhân luôn được áp dụng" đã áp dụng cho ownAssignment/
   roleMonthlyHtml. Không leak: record không đủ điều kiện bị loại thẳng khỏi
   items, không throw riêng cho từng cái (một notification có thể gộp nhiều
   lỗi mà actor chỉ còn quyền xem một phần). */
async function getChecklistViolationDetail(session,input={}){
  if(!supabase)fail('Supabase chưa được cấu hình.',503,'SUPABASE_NOT_CONFIGURED');
  const a=actor(session);
  const requested=Array.isArray(input.violationIds)?input.violationIds:(input.violationId?[input.violationId]:[]);
  const ids=[...new Set(requested.map(t).filter(Boolean))].slice(0,20);
  if(!ids.length)fail('Thiếu mã lỗi cần xem.',400,'CHECKLIST_VIOLATION_DETAIL_ID_REQUIRED');
  const {data:violations,error}=await supabase.from('checklist_violation_records')
    .select('id,employee_id,employee_code,employee_name,template_id,template_version,template_name,criterion_code,criterion_name,criterion_group,points,occurred_date,occurred_time,location,note,record_status,is_test,created_by,created_by_name,created_at,cancelled_by_name,cancelled_at,cancel_reason')
    .in('id',ids);
  if(error)throw error;
  const rows=violations||[];
  if(!rows.length)return {items:[],deniedCount:0,totalRequested:ids.length};
  const violationIds=rows.map(row=>row.id);
  const {data:taskRows,error:taskError}=await supabase.from(TABLE)
    .select('id,violation_id,status,current_assignee_id,current_assignee_code,current_assignee_type,due_at,created_by,created_by_name,created_at')
  ...
  if(taskIds.length){
    const {data:historyRows,error:historyError}=await supabase.from('checklist_violation_task_history').select('*').in('task_id',taskIds).order('created_at',{ascending:true});
    if(historyError)throw historyError;
    (historyRows||[]).forEach(row=>{const list=historyByTask.get(row.task_id)||[];list.push(row);historyByTask.set(row.task_id,list);});
  }
  let deniedCount=0;
  const items=[];
  for(const violation of rows){
    const task=taskByViolation.get(violation.id)||null;
    const allowed=a.role==='admin'||isSubject(violation,a)||isCreator(violation,a)||(task&&sameActor(task,a));
    if(!allowed){deniedCount++;continue;}
    items.push({violation,task:task?publicTask(task):null,history:task?(historyByTask.get(task.id)||[]):[]});
  }
  return {items,deniedCount,totalRequested:ids.length};
}
module.exports={listChecklistTasks,transitionChecklistTask,getChecklistTaskHistory,getChecklistViolationDetail,operationTimingPolicy};
