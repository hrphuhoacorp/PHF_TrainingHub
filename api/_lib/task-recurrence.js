...
/*
 * PHF Task — công việc lặp (recurrence) — PURE CALCULATION ENGINE.
 *
 * Đây CHỈ là lớp tính toán ngày/kỳ (date math), KHÔNG đọc/ghi DB, KHÔNG gọi
 * Supabase, KHÔNG biết gì về task_tasks/task_permission_*. Không có side
 * effect nào trong file này — mọi hàm đều pure (input → output xác định).
 *
 * Lý do tách riêng: nghiệp vụ lặp (Tạo phiếu V1 mục 11-19) yêu cầu rất nhiều
 * quy tắc ngày-tháng-năm phức tạp (fallback cuối tháng, 29/02, catch-up sau
 * sự cố, chống trùng kỳ) — tách thành pure function cho phép test đầy đủ
 * bằng mock/unit test THẬT (không cần DB thật) trước khi có schema/scheduler
 * thật. Khi migration recurrence (xem scripts/PHF_TASK_RECURRENCE_*.sql,
 * CHƯA APPLY) và RPC sinh Task instance được triển khai, lớp gọi RPC đó sẽ
 * DÙNG các hàm ở đây để quyết định "kỳ tiếp theo là ngày nào", nhưng việc
 * ghi task_tasks/task_recurrence_occurrences vẫn nằm ở tầng khác.
 *
 * Biên trách nhiệm rõ ràng (đọc kỹ trước khi tích hợp):
 *   - "Primary còn active hay không" / "Related nào còn active" / "kỳ trước
 *     đã hoàn thành hay chưa" — KHÔNG phải việc của file này. Caller (tầng
 *     tích hợp DB thật) phải tự kiểm tra rồi mới quyết định có gọi
 *     generateOccurrence hay không. File này chỉ tính "ngày nào", không biết
 *     "có nên sinh không".
 *   - "Sửa lịch chỉ áp dụng kỳ tương lai" / "sửa instance không ảnh hưởng
 *     lịch gốc" — là quy tắc ghi dữ liệu (immutable snapshot), thuộc tầng
 *     task-core.js tương lai, không phải date-math.
 */
