...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if ((pathname === '/api/auth/accounts' || pathname === '/api/auth/accounts/list') && req.method === 'GET') {
      const acctAction = String(new URL(req.url, 'http://localhost').searchParams.get('action') || 'list').trim();
      if (acctAction === 'impersonate-candidates') {
        // Account Impersonation V1 — gộp vào /api/auth/accounts (mirror
        // đúng api/auth/accounts.js) thay vì route riêng, để không vượt giới
        // hạn Serverless Function của Vercel Hobby.
        ...
        const accountsForRole = await listAccountsForAdmin();
        const candidates = accountsForRole.filter(a => a.role === role && a.status === 'active').map(a => ({
    ...
    if (pathname === '/api/auth/accounts' && req.method === 'POST') {
      ...
      // impersonate-stop phải luôn an toàn/idempotent kể cả khi role hiệu lực
      // không còn là admin (đang giả lập) — KHÔNG đi qua requireWebOperatorSession.
      if(action==='impersonate-stop'){
        let stopSession=null;
        try{ stopSession=await readSession(req); }catch(_e){ stopSession=null; }
        res.setHeader('Set-Cookie', clearImpersonationCookieHeader());
        if(stopSession && stopSession.impersonating){
          await auditEmit(req, stopSession, {
            module:'account', action:'IMPERSONATION_STOP', result:'success',
            object_type:'account', object_id:stopSession.account.id, object_label:stopSession.account.employeeCode||stopSession.account.name||stopSession.account.id,
            actor:{ actor_account_id:stopSession.actor && stopSession.actor.id, actor_name:stopSession.actor && (stopSession.actor.name||stopSession.actor.email) }
          });
        }
    ...
    if (pathname === '/api/data') {
      ...
      if (req.method === 'POST') {
        ...
        /* Workstream B — Đi trễ BCC/ghi nhận từ bộ phận/Admin phê duyệt (Gap 3). Mọi hành động
           ...
           record_violation + record_scope bao phủ nhân sự đó, KHÔNG hardcode role ở router này;
           requireAdmin cho các thao tác BCC/duyệt/xuất Admin-only) — xem
           lib/checklist-late-reconciliation-service.js. Tên action CHUNG (Manager) là chính; tên
           cũ (ShiftLead) giữ lại làm bí danh tương thích ngược, gọi ĐÚNG hàm service giống hệt. */
        if (payload && payload.action === 'recordChecklistLateManagerObservation') {
          return sendJson(res, 200, {ok:true,...await recordManagerLateObservation(session, payload.input || {})});
        }
        if (payload && payload.action === 'recordChecklistLateShiftLeadObservation') {
          return sendJson(res, 200, {ok:true,...await recordShiftLeadLateObservation(session, payload.input || {})});
        }
        if (payload && payload.action === 'listChecklistLateManagerObservations') {
          return sendJson(res, 200, {ok:true,...await listManagerLateObservations(session, payload.input || {})});
        }
        ...
        if (payload && payload.action === 'reconcileChecklistLateBccImport') {
          return sendJson(res, 200, {ok:true,...await reconcileChecklistLateBccImport(session, payload.input || {})});
        }
        if (payload && payload.action === 'approveChecklistLateEvents') {
          return sendJson(res, 200, {ok:true,...await approveChecklistLateEvents(session, payload.decisions || [])});
        }
        if (payload && payload.action === 'createChecklistLateLinkedAdjustment') {
          return sendJson(res, 200, {ok:true,...await createChecklistLateLinkedAdjustment(session, payload.input || {})});
        }
        if (payload && payload.action === 'exportChecklistLateReconciliation') {
          return sendJson(res, 200, {ok:true,...await exportChecklistLateReconciliation(session, payload.filters || {})});
        }
      ...
      if(payload&&payload.action==='getKnlGradeMatrix')return sendJson(res,200,{ok:true,...await getKnlGradeMatrix(session,payload)});
      if(payload&&payload.action==='saveKnlGradeMatrix')return sendJson(res,200,{ok:true,...await saveKnlGradeMatrix(session,payload)});
      if(payload&&payload.action==='setKnlVersionEffectivity')return sendJson(res,200,{ok:true,...await setKnlVersionEffectivity(session,payload)});
      if(payload&&payload.action==='listKnlCompensationStandards')return sendJson(res,200,{ok:true,...await listKnlCompensationStandards(session)});
      if(payload&&payload.action==='previewKnlCompensationFoundation')return sendJson(res,200,{ok:true,...await previewKnlCompensationFoundation(session)});
      ...
      if(payload&&payload.action==='scheduleKnlCompensationVersion')return sendJson(res,200,{ok:true,...await scheduleKnlCompensationVersion(session,payload)});
      if(payload&&payload.action==='getKnlCompensationVersionAudit')return sendJson(res,200,{ok:true,...await getKnlCompensationVersionAudit(session)});
      if(payload&&payload.action==='listKnlEmployeeCompensationHistory')return sendJson(res,200,{ok:true,...await listKnlEmployeeCompensationHistory(session,payload)});
      if(payload&&payload.action==='listKnlEmployeeCompensationPeriods')return sendJson(res,200,{ok:true,...await listKnlEmployeeCompensationPeriods(session,payload)});
      ...
      if(payload&&payload.action==='getKnlEmployeeCompetencyGradeStandard')return sendJson(res,200,{ok:true,...await getKnlEmployeeCompetencyGradeStandard(session,payload)});
      ...
      if(payload&&payload.action==='getKnlGradePromotionApproverOptions')return sendJson(res,200,{ok:true,...await getGradePromotionApproverOptions(session,payload)});
      ...
      if(payload&&payload.action==='agreeKnlGradePromotionProposal')return sendJson(res,200,{ok:true,...await processGradePromotionProposalStep(session,{...payload,action:'agree'})});
      if(payload&&payload.action==='rejectKnlGradePromotionProposal')return sendJson(res,200,{ok:true,...await processGradePromotionProposalStep(session,{...payload,action:'reject'})});
      if(payload&&payload.action==='withdrawKnlGradePromotionProposal')return sendJson(res,200,{ok:true,...await withdrawGradePromotionProposal(session,payload)});
      if(payload&&payload.action==='listMyKnlGradePromotionProposals')return sendJson(res,200,{ok:true,...await listMyGradePromotionProposals(session)});
      ...
      if(payload&&payload.action==='getKnlSurveySetup')return sendJson(res,200,{ok:true,...await getKnlSurveySetup(session,payload)});
      if(payload&&payload.action==='saveKnlSurveyCampaign')return sendJson(res,200,{ok:true,...await saveKnlSurveyCampaign(session,payload.campaign||{})});
      if(payload&&payload.action==='openKnlSurveyCampaign')return sendJson(res,200,{ok:true,...await openKnlSurveyCampaign(session,payload)});
      if(payload&&payload.action==='closeKnlSurveyCampaign')return sendJson(res,200,{ok:true,...await closeKnlSurveyCampaign(session,payload)});
      if(payload&&payload.action==='listKnlSurveyCampaigns')return sendJson(res,200,{ok:true,...await listKnlSurveyCampaigns(session,payload)});
      if(payload&&payload.action==='getKnlSurveyTicket')return sendJson(res,200,{ok:true,...await getKnlSurveyTicket(session,payload)});
      if(payload&&payload.action==='saveKnlSurveyTicket')return sendJson(res,200,{ok:true,...await saveKnlSurveyTicket(session,payload)});
      if(payload&&payload.action==='getKnlSurveyResults')return sendJson(res,200,{ok:true,...await getKnlSurveyResults(session,payload)});
