-- PHF Checklist 1.38.52 · KPI tháng Marketing hoạt động trước khi tạo phiếu
...
begin
  ...
  v_template_key := case v_template
    when 'tbp-mkt-1.0' then 'tbp-marketing'
    when 'nv-media-1.0' then 'nv-marketing'
    else null
  end;
  ...
  where period_month=p_period_month
    ...
    and lower(trim(template_id))=v_template_key
  order by id
  limit 1
  for update;
  ...
  if v_form.id is not null then
    ...
    where lower(trim(template_key))=v_template_key
  ...
  if exists(
    ...
    where f.period_month=p_period_month
      ...
      and lower(trim(f.template_id))=v_template_key
      and (
