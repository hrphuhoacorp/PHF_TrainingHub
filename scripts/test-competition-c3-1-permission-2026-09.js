...
(async () => {
  ...
  for (const [ns, sysRole] of [['/hv', 'learner'], ['/ql', 'manager']]) {
    ...
    check(labels.includes('Tổng quan') && labels.includes('Bảng tin') && labels.includes('Bài của tôi') && labels.includes('+ Gửi bài dự thi') && labels.includes('Bảng xếp hạng & Kết quả'),
      ns + ' participant sees all 5 participation screens');
    check(!labels.includes('Chờ duyệt') && !labels.includes('Quản lý chương trình') && !labels.includes('Phân quyền xét duyệt') && !labels.includes('Chốt chương trình'),
  ...
  console.log('\n== 5. /hv WITH Competition Admin grant (non-system-admin) — admin screens appear + are reachable ==');
  {
    ...
    const labels = menuLabels(m.root);
    check(labels.includes('Quản lý chương trình') && labels.includes('Phân quyền xét duyệt') && labels.includes('Chốt chương trình'),
  ...
  console.log('\n== 6. /ql WITHOUT Competition Admin grant — admin screens absent + deep link redirected ==');
  {
    ...
    const labels = menuLabels(m.root);
    check(!labels.some((l) => /Quản lý chương trình|Phân quyền xét duyệt|Chốt chương trình/.test(l)), '/ql reviewer WITHOUT admin grant sees no admin menu items');
