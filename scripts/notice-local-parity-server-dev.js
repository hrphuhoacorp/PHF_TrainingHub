 ...
 *     -> Notice bridge (api/_lib/notice-bridge.js, flag ON in
 ...
 *        claude-phf; container phf-hr-e2e-throwaway-…; schema notice.*
 *        from Batch B)
 *
 ...
 * this file's only job is proving the Notice path).
 *
...
function die(m) { console.error('NOTICE_LOCAL_PARITY_ABORT: ' + m); process.exit(1); }
...
const API_PORT = Number(process.env.PHF_LOCAL_PARITY_API_PORT || 18933); // distinct from Task's 18931 so both can run side by side
const APP_PORT = Number(process.env.PORT || 3000);
const API_BASE = 'http://127.0.0.1:' + API_PORT;
const ATTACH_ROOT = fs.mkdtempSync(path.join(os.tmpdir(), 'phf-notice-local-parity-attach-'));
...
(async () => {
  console.log('[notice-local-parity] starting phf-hr-api child on ' + API_BASE + ' -> PG ' + dbEnv.PHF_HR_DB_HOST + ':' + dbEnv.PHF_HR_DB_PORT + '/' + dbEnv.PHF_HR_DB_NAME);
  const apiEnv = Object.assign({}, process.env, supaEnv, {
    ...
    // Development access lock (Batch 01 §25). Value = comma list of employee
    // codes / account ids allowed to reach the unfinished management surface
    // while NOTICE_DEV_ACCESS_ALLOW is set. GO-LIVE = unset it (no code change).
  ...
  children.push(api);
  api.on('exit', (c) => { console.error('[notice-local-parity] phf-hr-api child exited ' + c); stopAll(c || 1); });
  ...
  console.log('[notice-local-parity] phf-hr-api /healthz = 200');

  console.log('[notice-local-parity] starting Main App on http://127.0.0.1:' + APP_PORT + '  (Notice bridge ON only, Task flags left at default)');
  ...
  children.push(app);
  app.on('exit', (c) => { console.error('[notice-local-parity] Main App exited ' + c); stopAll(c || 1); });
  ...
  console.log('LOCAL_URL            = http://127.0.0.1:' + APP_PORT + '/admin/thong-bao  (or /hv/thong-bao, /ql/thong-bao)');
  console.log('SUPABASE (org/auth)  = PHF-HR-DEV (' + DEV_HOST + ')  [read-only, real People Master]');
  console.log('NOTICE_BACKEND  = SERVER BRIDGE (server.js -> notice-bridge.js -> local phf-hr-api ' + API_BASE + ')');
  console.log('NOTICE_DB       = throwaway PostgreSQL ' + dbEnv.PHF_HR_DB_HOST + ':' + dbEnv.PHF_HR_DB_PORT + '/' + dbEnv.PHF_HR_DB_NAME + ' (container ' + (dbEnv.PHF_HR_E2E_CONTAINER || '?') + ')');
  console.log('FLAGS                = PHF_NOTICE_BRIDGE_ENABLED=true (process env only). No PHF_TASK_* flag touched.');
...
})().catch((e) => { console.error('[notice-local-parity] fatal: ' + (e && e.stack || e)); stopAll(1); });
