...
assert(assignments.includes('position:org.position')&&assignments.includes('position:r.position'),'Checklist projection must preserve the canonical position field both ways.');
assert(service.includes("row.position=text(item.position)")&&service.includes("organization:'employee_profiles (People Master"),'Employee service must map organization from employee_profiles.');
assert(!service.includes('source.assignments.forEach'),'Checklist assignments must not create or override People Master rows.');
