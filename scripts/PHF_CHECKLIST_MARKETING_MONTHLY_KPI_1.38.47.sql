-- PHF Checklist 1.38.47 · KPI tháng động riêng Marketing
...
begin
  ...
  if v_form.id is null then
    return jsonb_build_object('ok',false,'code','CHECKLIST_MARKETING_KPI_FORMS_NOT_FOUND','message','Chưa có phiếu tháng cho mẫu này.');
  end if;
  if trim(coalesce(v_form.template_version,'')) <> trim(coalesce(p_template_version,'')) then
  ...
  if v_updated=0 then
    raise exception 'CHECKLIST_MARKETING_KPI_FORMS_NOT_FOUND';
  end if;
