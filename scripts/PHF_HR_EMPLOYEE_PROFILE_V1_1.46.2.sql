-- PHF HR Employee Profile V1 / 1.46.2
-- Additive only. Run once in Supabase Production SQL Editor.
-- Reuses the five Employee Master tables introduced by 1.46.0.

begin;

-- Baseline HSNS fields that are not present in Employee Master 1.46.0.
-- This table is already RLS-enabled and revoked from anon/authenticated.
alter table public.employee_private_profiles
  ...
  add column if not exists personal_tax_code text not null default '',
  add column if not exists social_insurance_code text not null default '';

-- Organization remains owned by checklist_employee_assignments.
-- title = Chức danh; position = Chức vụ. Permission presets are neither.
  ...
  loop
    ...
    then
      ...
      insert into public.checklist_employee_assignment_history(
        ...
        coalesce(item->>'title',''),coalesce(item->>'position',''),coalesce(item->>'branch',''),
...
--   and table_name in ('employee_private_profiles','checklist_employee_assignments','checklist_employee_assignment_history')
--   and column_name in ('nationality','ethnicity','personal_tax_code','social_insurance_code','position')
